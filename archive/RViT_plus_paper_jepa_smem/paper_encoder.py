...
try:
    from .paper_heads import JEPAStructuredHead
except ImportError:  # script / flat import
    from paper_heads import JEPAStructuredHead   # type: ignore[no-redef]
...
class CrossAttentionXH(nn.Module):
    ...
    mem_dim sizes the feedback projections (W_kh/W_vh); it defaults to d_mem, but the
    softmax-memory variant feeds a wider categorical memory (n_heads*proto_dim) so it is set
    independently there."""
    def __init__(self, d_token: int = D_TOKEN, d_mem: int = D_MEM, mem_dim: int = None) -> None:
        super().__init__()
        mem_dim = d_mem if mem_dim is None else int(mem_dim)
        self.W_q = nn.Linear(d_token, d_token, bias=False)
        self.W_kx = nn.Linear(d_token, d_token, bias=False)
        self.W_kh = nn.Linear(mem_dim, d_token, bias=False)
        self.W_vx = nn.Linear(d_token, d_token, bias=False)
        self.W_vh = nn.Linear(mem_dim, d_token, bias=False)
...
class RecurrentViTxLSTM(nn.Module):
    """Multiplicative/FiLM-SA ViT + a recurrent cell. With cell='xlstm' (default) it carries the
    paper xLSTM state (H,C,N,M) and reads out H; with cell='softmax_head' it carries ONLY a
    softmax-head cell state C (a 1-tuple), feeds C to the attention, and reads out C. Either readout
    is (B,4,1024), flattened to 4096 for the heads."""

    def __init__(self, d_token: int = D_TOKEN, d_mem: int = D_MEM, n_patch: int = N_PATCH,
                 feedback: str = "multiplicative", two_lstm: bool = False,
                 cell: str = "xlstm", mem_heads: int = 4,
                 softmax_memory: bool = False, mem_softmax_heads: int = 4,
                 mem_softmax_dim: int = 256) -> None:
        super().__init__()
        self.n_patch, self.d_token, self.d_mem = n_patch, d_token, d_mem
        self.feedback = feedback
        self.cell = cell                                          # "xlstm" | "softmax_head"
        # SOFTMAX-MEMORY mode: the externalised recurrent memory is S = per-head-softmax(mem_head(H1)),
        # a categorical (mem_softmax_heads × mem_softmax_dim) vector per patch. The xLSTM still carries
        # its CONTINUOUS cell (H,C,N,M) internally — S is what feeds the cross-attention K/V and the
        # readout. mem_head is the SAME structured head the JEPA loss distills (single owner here).
        self.softmax_memory = bool(softmax_memory)
        if self.softmax_memory and feedback != "crossattn1":
            raise ValueError("softmax_memory is implemented for feedback='crossattn1' only")
        if self.softmax_memory and cell != "xlstm":
            raise ValueError("softmax_memory requires cell='xlstm' (it carries the continuous xLSTM cell)")
        self.mem_softmax_heads, self.mem_softmax_dim = int(mem_softmax_heads), int(mem_softmax_dim)
        self.mem_dim = (self.mem_softmax_heads * self.mem_softmax_dim) if self.softmax_memory else d_mem
        self.mem_head = JEPAStructuredHead(d_mem, n_heads=self.mem_softmax_heads,
                                           proto_dim=self.mem_softmax_dim) if self.softmax_memory else None
        ...
        if feedback in ("crossattn", "crossattn1"):
            ...
            self.attn = CrossAttentionXH(d_token, d_mem, mem_dim=self.mem_dim)   # Q(X), K=K(X,mem), V=V(X,mem)
        ...
        self.readout_dim = n_patch * self.mem_dim                # flattened readout (mem_dim per patch)

    def _z(self, B, device, dtype):
        return torch.zeros(B, self.n_patch, self.d_mem, device=device, dtype=dtype)

    def _softmax_mem(self, H):
        """Continuous cell H (B,N,d_mem) → categorical memory S (B,N,mem_dim): per-head softmax
        (τ=1) of the structured head's logits, flattened. S is the externalised memory used
        EVERYWHERE (cross-attn K/V + readout); the JEPA loss distils the same head's logits."""
        logits = self.mem_head(H)                                # (B,N,heads,dim)
        return torch.softmax(logits, dim=-1).flatten(-2)         # (B,N,heads*dim)

    def init_states(self, batch_size: int, device=None, dtype=torch.float32) -> State:
        z = lambda: self._z(batch_size, device, dtype)
        if self.softmax_memory:
            # state = (continuous xLSTM (H,C,N,M), S) ; S init = UNIFORM softmax (1/dim per head)
            s_unif = torch.full((batch_size, self.n_patch, self.mem_dim),
                                1.0 / self.mem_softmax_dim, device=device, dtype=dtype)
            return ((z(), z(), z(), z()), s_unif)
    ...
    def cont_cell(self, state):
        """The CONTINUOUS cell output the JEPA head reads (== the readout in every mode EXCEPT
        softmax_memory, where the readout is S but the head must see the continuous cell H1)."""
        if self.softmax_memory:
            return state[0][0]                                   # ((H1,C,N,M), S) → H1
        if self.cell == "softmax_head":
            return state[0]                                      # (C,) → C
        if self.two_lstm:
            return state[1][0]                                   # (s1, s2) → H2 (the readout cell)
        return state[0]                                          # (H1,C,N,M) → H1

    def forward_step(self, X: torch.Tensor, state, return_attn: bool = False,
                     attn_clamp=None):
        # softmax_memory (crossattn1): the externalised memory S (a categorical per-head softmax)
        # feeds the cross-attention K/V AND is the readout; the xLSTM keeps its CONTINUOUS cell
        # internally (its recurrence uses H_prev, not S) and produces H1 → S1 = softmax(mem_head(H1)).
        if self.softmax_memory:
            xlstm_state, S_prev = state
            H_prev, C_prev, N_prev, M_prev = xlstm_state
            Z, attn = self.attn(X, S_prev, return_attn=return_attn)          # cross-attn memory = S_prev
            H1, C1, N1, M1 = self.lstm(Z, H_prev, C_prev, N_prev, M_prev)    # xLSTM uses CONTINUOUS H_prev
            S1 = self._softmax_mem(H1)                                       # externalised categorical memory
            return ((H1, C1, N1, M1), S1), S1, attn                         # heads read S1
