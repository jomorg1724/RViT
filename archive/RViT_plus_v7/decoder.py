"""
Actor and critic decoders for RViT+ v7 — 1D-CONV style.
...
def _to_conv_input(recurrent_states: List[torch.Tensor]) -> torch.Tensor:
    """Top recurrent state H2 (B, N, d_mem) → (B, d_mem, N): features as conv
    channels, tokens as the conv axis."""
    return recurrent_states[-1].transpose(1, 2).contiguous()
