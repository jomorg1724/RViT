...
from RViT_plus_v2.attention import FeedbackTransformer
from RViT_plus_v2.memory import GridCellRNN_LSTM
...
def test_feedback_transformer():
    print("\n[2] FeedbackTransformer (V2: per-channel spatial attention) ---")
    B, D, H, W = 2, 64, 12, 12
    # Two feedback sources at d_model channels each → c_feedback = 2 * D.
    ft = FeedbackTransformer(d_model=D, n_heads=4, c_feedback=2 * D, n_feedback=2)
    ...
    _check("FT.attn shape (B, C, H, W) — per-channel spatial attention",
           tuple(out["attn"].shape) == (B, D, H, W))
    ...
    # gates pass through sigmoid → values in [0, 1].
    _check("FT gates are in [0, 1]",
           bool((out["q_gate"] >= 0).all() and (out["q_gate"] <= 1).all()))

    # Per-channel spatial-softmax weights sum to 1 over (H, W) within each channel.
    print("\n[3] FT per-channel spatial attention weights ---")
    a = out["attn"].reshape(B, D, H * W)
    sums = a.sum(dim=-1)              # (B, C)
    _check("attn sums to 1 per channel",
           torch.allclose(sums, torch.ones_like(sums), atol=1e-5),
           detail=f"channel-sum mean={sums.mean().item():.4f}")
    _check("attn values are non-negative (softmax output)",
           bool((out["attn"] >= 0).all()))
...
def test_gridcell_rnn():
    ...
    _check("Cell.attn shape (B, C, H, W) — V2 per-channel spatial attention",
           tuple(out["attn"].shape) == (B, st_ch, H, W))
