"""
Training entry point for V10 — Multi-Layer Feedback Transformer with the v8 H1-RESIDUAL encoder on the ViZDoom
deathmatch arena.

Usage:
    .venv/bin/python v10_VizdoomArena/train_rl.py                 # full run (MPS auto)
    .venv/bin/python v10_VizdoomArena/train_rl.py --iters 50 --warmup-iters 5
    .venv/bin/python v10_VizdoomArena/train_rl.py --device cpu --seg-len 16
"""
...
from v10_VizdoomArena.config.loader import (
    DEFAULT_CONFIG_PATH, cfg_get, load_checkpoint_weights, load_config,
    print_resolved_config,
)
from v10_VizdoomArena.env import FEAT_DIM, FEAT_GROUPS, N_ACTIONS, VizdoomArenaEnv
from v10_VizdoomArena.model import V10ArenaModel
from v10_VizdoomArena.trainer import PACConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--checkpoint-dir", default=cfg_get(
        ...
        os.path.expanduser("~/rvit_plus_checkpoints/v10_vizdoom_arena")))
    ...
    model = V10ArenaModel(**model_kwargs).to(device)
    ...
    print(f"[diag] patch {args.patch_size}px → {model.n_tokens} tokens "
          ...
          f"L0 (perception) keys = {model.encoder.n_keys_for(0)}, "
          f"layout = {model.encoder.key_layout(0)}; "
          f"L1+ (consolidation) keys = {model.encoder.n_keys_for(1)}, "
          f"layout = {model.encoder.key_layout(1)}")
    ...
    final_path = os.path.join(args.checkpoint_dir, "v10_final.pt")
