...
Usage:
    .venv/bin/python RViT_plus_jump_softmax/train_rl.py
    .venv/bin/python RViT_plus_jump_softmax/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_jump_softmax.config.loader import (
...
from RViT_plus_jump_softmax.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    model = RViTPlusJumpModel(**model_kwargs).to(device)

    print(f"[diag] JUMP model: 4-patch MLP→d_model={args.d_model} (+CLS) | FiLM-affine perception "
          f"transformer ({args.tx_heads}h) | MEMORY transformer (self-attn over [Z‖H1], H1 on residual) "
          f"→ d_mem={args.d_mem} | readout LN[Z_cls‖H1_cls]=({args.d_model}+{args.d_mem})")
    print(f"[diag] patch_size={args.patch_size} → n_tokens={model.n_tokens} (CLS + {model.patch_embed.n_patches} patches)")
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_jump_softmax_rl_final.pt")
