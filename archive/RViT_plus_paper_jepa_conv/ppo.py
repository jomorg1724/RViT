...
class PPOConfig:
    ...
    # ── Bottom-up image-VAE auxiliary (OFF by default). SECOND backward pass per update, on Z
    #    with the recurrent memory DETACHED. Keep recon_coef < 1 so it stays weaker than the RL loss. ──
    recon_coef: float = 0.0                 # weight on the image-VAE reconstruction loss (set >0 to enable)
    recon_beta: float = 1e-3                # KL weight inside the VAE loss (small → recon-dominant)
...
def ppo_update(
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_jepa": 0.0, "loss_recon": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0,
    ...
    z_teacher = z_teacher_h1 = tau_t = None
    if use_jepa:
        jepa_teacher.eval()
        with torch.no_grad():
            ...
            if hasattr(jepa_teacher, "jepa_head_h1") and "cell_seq_h1" in jout:   # DUAL JEPA (two-lstm)
                z_teacher_h1 = jepa_teacher.jepa_logits_h1(jout["cell_seq_h1"])
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        if use_jepa:
            def _jepa_term(z_student, z_teach, center):
                if cfg.jepa_temporal:                            # student@t predicts teacher@t+1
                    zt, zs, vmask = z_teach[:, 1:], z_student[:, :-1], (m[:, 1:] * m[:, :-1])
                else:                                            # same-time ablation
                    zt, zs, vmask = z_teach, z_student, m
                # softmax over proto_dim (last) PER (token, head) → independent structured distributions
                p_t = torch.softmax((zt - center) / tau_t, dim=-1)              # centered + sharpened (no grad)
                logp_s = torch.log_softmax(zs / cfg.jepa_tau_student, dim=-1)   # student
                ce = -(p_t * logp_s).sum(-1)                                    # (B, Tj, 4, n_heads)
                vm = vmask[..., None, None].expand_as(ce)                       # mask frames, broadcast tokens+heads
                return (ce * vm).sum() / vm.sum().clamp(min=1.0)
            loss_jepa = _jepa_term(model.jepa_logits(out["cell_seq"]), z_teacher, model.jepa_center)
            if z_teacher_h1 is not None:                         # DUAL JEPA on the feedback memory H1
                loss_jepa = loss_jepa + _jepa_term(model.jepa_logits_h1(out["cell_seq_h1"]),
                                                   z_teacher_h1, model.jepa_center_h1)
        ...
        # ── Second backward pass: bottom-up image-VAE (recurrent memory detached). Accumulates
        #    into the SAME .grad buffers as the RL pass, so the single optimizer.step() below applies
        #    both objectives. Weighted by recon_coef (< 1) so it stays weaker than the RL loss. ──
        if getattr(model, "vae_aux", False) and cfg.recon_coef > 0.0:
            recon_loss, _recon_mse, _recon_kl = model.recon_loss(batch.observations, beta=cfg.recon_beta)
            (cfg.recon_coef * recon_loss).backward()
            accumulated["loss_recon"] += float(recon_loss.detach().item())
    ...
    if use_jepa:
        with torch.no_grad():
            ...
            if z_teacher_h1 is not None:                         # DUAL JEPA: center the H1 head too
                model.jepa_center_h1.mul_(cfg.jepa_center_momentum).add_(
                    z_teacher_h1.mean(dim=(0, 1)), alpha=1.0 - cfg.jepa_center_momentum)

    n_evals = max(accumulated["n_loss_evals"], 1.0)
    LOSS_KEYS = {"loss_policy", "loss_value", "loss_entropy", "loss_contrastive",
                 "loss_jepa", "loss_recon", "loss_total", "approx_kl"}
    result = {}
    ...
    *,
    ...
    if cfg.jepa_coef > 0.0:
        ...
        print(f"[setup] JEPA self-distillation: {'temporal t→t+1' if cfg.jepa_temporal else 'same-time'} "
              ...
              f"(4 tokens → {4*cfg.jepa_n_heads} independent softmaxes), coef={cfg.jepa_coef}, "
    ...
    for it in range(n_iterations):
        ...
        if _metrics_path is not None:
            if _metrics_w is None:
                ...
                _metrics_f = open(_metrics_path, "w", newline="")
                _metrics_w = csv.DictWriter(_metrics_f, fieldnames=list(log.keys()),
                                            extrasaction="ignore")
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            torch.save({"iter": it, "model_state_dict": model.state_dict(),
                        "model_kwargs": model_kwargs,      # self-describing: burst-resume rebuilds the exact arch
                        "theta": float(getattr(env, "theta", float("nan")))}, ckpt_path)
            print(f"[checkpoint] saved to {ckpt_path}  (θ={getattr(env,'theta',float('nan')):.1f})")
