"""
Encoder for RViT+ v12 — a VQ-VAE-style DISCRETE CODEBOOK bottleneck built out of
attention, SIMPLIFIED: the value codebook is now a STATIC LEARNABLE LOOKUP TABLE
(a plain nn.Parameter), not the self-referential recurrent H1 of the first v12.
...
vs the first v12: H1 (the values) was a recurrent state updated by LSTM(Z); here it
...
class VQAttnEncoder(nn.Module):
    ...
    def __init__(
        ...
        if int(n_lstm) != 1:
            raise ValueError("v12 (static codebook) needs n_lstm=1 (only the key-memory H2 is recurrent).")
