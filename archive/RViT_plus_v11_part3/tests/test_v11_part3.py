"""
Smoke tests for RViT+ v11_part3 — single-head dual-stream with a SPLIT readout,
SWAPPED vs v11_part2:

  Tμ SALIENCE: Q=X, K=V=[H1‖H2], residual=X  → CRITIC   (v11_part2 fed the actor)
  TQ TOP-DOWN: Q=X, K=V=H2,      residual=H2 → ACTOR    (v11_part2 fed the critic)
  H1=LSTM1(X), H2=LSTM2(Z_sal).

Run:  .venv/bin/python -m RViT_plus_v11_part3.tests.test_v11_part3
"""
...
from RViT_plus_v11_part3.env import ChangeDetectionEnv
from RViT_plus_v11_part3.tx_lstm_encoder import DualStreamEncoder
from RViT_plus_v11_part3.model import RViTPlusV11Part3Model
from RViT_plus_v11_part3.ppo import (
...
def _model(**over):
    ...
    kw.update(over)
    return RViTPlusV11Part3Model(**kw).to(DEVICE)
...
def test_stream_shapes_and_routing():
    """Tμ attends over [H1‖H2] (2N keys), TQ over H2 (N keys). (Encoder identical to
    v11_part2 — only the readout is swapped.)"""
    m = _model().eval()
    ...
    with torch.no_grad():
        ...
        assert attn[0].shape == (2, 1, 100, 200), f"salience attn {tuple(attn[0].shape)}, expected (2,1,100,200)"
        assert attn[1].shape == (2, 1, 100, 100), f"top-down attn {tuple(attn[1].shape)}, expected (2,1,100,100)"
        ...
        assert not torch.allclose(attn[0], attn_h1[0]) and torch.allclose(attn[1], attn_h1[1]), \
            "H1 should move only salience weights (TQ reads H2 only)"
        ...
        assert not torch.allclose(attn[0], attn_h2[0]) and not torch.allclose(attn[1], attn_h2[1]), \
            "H2 should move BOTH stream weights (in both K/V banks)"
    print("✓ routing: Tμ reads [H1‖H2] (2N keys), TQ reads H2 (N keys); H1→sal only, H2→both")
...
def test_salience_grounded_topdown_gating_only():
    ...
    print("✓ salience stays image-grounded; top-down is gating-only")


def test_split_readout_swapped():
    """The decisive v11_part3 proof (SWAPPED vs v11_part2): the ACTOR reads ONLY Z_td
    (top-down), the CRITIC reads ONLY Z_sal (salience). Changing Z_td moves the actor
    but NOT the critic; changing Z_sal moves the critic but NOT the actor."""
    m = _model().eval()
    B = 4
    ...
    with torch.no_grad():
        ...
        # perturb salience (Z_sal) → critic changes, actor UNCHANGED (actor reads Z_td)
        al2, q2, _, _ = m._run_heads([torch.randn(B, 100, 64), zt])
        assert torch.allclose(al, al2), "actor moved when only Z_sal changed — it must read Z_td only"
        assert not torch.allclose(q, q2), "critic ignores Z_sal — should read it"
        # perturb top-down (Z_td) → actor changes, critic UNCHANGED (critic reads Z_sal)
        al3, q3, _, _ = m._run_heads([zs, torch.randn(B, 100, 64)])
        assert not torch.allclose(al, al3), "actor ignores Z_td — should read it"
        assert torch.allclose(q, q3), "critic moved when only Z_td changed — it must read Z_sal only"
    print("✓ split readout SWAPPED: actor←Z_td (TQ), critic←Z_sal (Tμ); each ignores the other")
...
if __name__ == "__main__":
    ...
    for t in tests:
        t()
    print(f"\nAll {len(tests)} v11_part3 smoke tests passed (incl. the SWAPPED split-readout proof).")
