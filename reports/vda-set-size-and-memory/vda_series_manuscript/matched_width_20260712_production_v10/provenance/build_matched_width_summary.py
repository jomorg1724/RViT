...
def trial_contract(shards: dict[tuple[str, str, int], dict[str, np.ndarray]]) -> dict[str, int]:
    ...
    if len(psychometric) != 1 or len(clamp) != 1 or len(decoder) != 1:
        raise ValueError(
            "sample-size contract differs across shards: "
            f"psychometric={psychometric}, decoder={decoder}, clamp={clamp}"
        )
    return {
        "psychometric_trials_per_point": psychometric.pop(),
        "decoder_samples_per_shard": decoder.pop(),
...
def quiescent_regular_files(root: Path, *, stable_checks: int = 32, interval: float = 0.25) -> set[Path]:
...
def build(input_root: Path, output_root: Path) -> Path:
    ...
    sidecar = {
        ...
        "clamp": {"intervention": "additive bias on cued routing-key logits at change time", "baseline_key_logit_bias": 0.0, "trials_per_cell": trials["clamp_trials_per_cell"]},
