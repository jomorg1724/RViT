"""
V6ArenaModel — multi-layer feedback transformer actor-critic for the ViZDoom
arena (deathmatch).
...
Pipeline per frame (no convolution anywhere):
    ...
    frame x_t (B,3,60,80) float [0,1]
       ...
       │  MultiLayerFeedbackEncoder.forward_step      → L per-token LSTM memories;
       │     each layer cross-attends Q=patch over [X ++ H₁..H_L ++ state]
       ├─► ActorDecoder([H₁..H_L])   (CLS transformer) → logits (B, A)
       └─► CriticDecoder([H₁..H_L])  (CLS transformer) → Q (B, A, n_quantiles)
                                                        → V via derive_V
...
class V6ArenaModel(nn.Module):
    ...
    def __init__(
        ...
        dec_kwargs = dict(
            n_tokens=self.n_tokens, n_states=enc_layers, d_mem=d_mem,
