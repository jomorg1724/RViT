...
Usage:
  .venv/bin/python -m RViT_plus_v9.analysis.deepdive.exp5_value_entropy \
...
def main(argv=None):
    ...
    nL = model.encoder.tx_layers           # v9: cross-attn blocks (=1)
    nH = model.encoder.n_heads             # v9: heads per block (=8)
    n_lstm = model.encoder.n_lstm
    N = model.n_tokens
    Sk = (1 + (n_lstm - 1)) * N            # 2N keys: [patch ++ H2] (H1 = query/residual)
    sweep = []
