"""
Smoke tests for RViT+ v8_part2 — v8 with MEMORY-ONLY K/V: patch tokens are
queries only; keys/values are [H1 ++ H2]. Visual information reaches the
recurrent state ONLY through query-side gating over the mnemonic codebook.

Run:  .venv/bin/python -m RViT_plus_v8_part2.tests.test_v8_part2
"""
...
from RViT_plus_v8_part2.env import ChangeDetectionEnv
from RViT_plus_v8_part2.tx_lstm_encoder import TxLSTMEncoder
from RViT_plus_v8_part2.model import RViTPlusV5Part2Model
from RViT_plus_v8_part2.ppo import (
...
def test_encoder_cross_attends_over_memory():
    """Cross-attention: N patch QUERIES attend over [H1 ++ H2] = n_lstm·N keys
    (memory-only — no patch in K/V). Weights are (B, heads, N, 2N), and carried
    memory actually changes the output."""
    m = _model().eval()
    ...
    print("✓ cross-attention: N=100 queries over 2N=200 memory keys (H1++H2); no patch in K/V")



def test_h1_residual_no_visual_bypass():
    """THE v8_part2 property: with the attention output silenced (out_proj zeroed),
    the encoder's recurrent update must be INDEPENDENT of the visual input.
    With memory-only K/V there is no visual value stream at all — only query-side
    gating can carry frame information, and silencing out_proj blocks that too."""
    import copy
    m = _model().eval()
    ...
    with torch.no_grad():
        ...
        # attention intact -> different frames produce different states (query gating)
        ...
        assert not torch.allclose(rec_c[0], rec_d[0]), \
            "intact attention should transmit visual differences via query gating"
    print("✓ memory-only K/V: silenced attention blocks ALL visual flow; gating transmits it")
...
if __name__ == "__main__":
    ...
    for t in tests:
        t()
    print(f"\nAll {len(tests)} v8_part2 smoke tests passed (memory-only K/V + no-bypass proof).")
