...
\toprule
\textbf{Artifact class} & Newly authored VDA-series manuscript; not a rebuild of the earlier upgraded-paper manuscript.\\
...
\begin{abstract}
A spatial cue can make an agent report ``change'' more often because discrimination improves or because the decision criterion becomes more liberal. Response curves alone cannot separate those explanations. This audit asks which behavioral, representational, and model-intervention claims survive when each result is tied to an explicit value-directed-attention task, checkpoint, and evidence class. Deterministic products establish fourteen task specifications, source-derived panels document the inspected implementation, and eight historical behavioral batteries are attributed only to their preserved aggregate cache. A first checkpoint-recomputed wave establishes VDA4 and VDA9 psychometrics and descriptive routing weights. A second battery extends the same d128 VDA4/VDA9 checkpoints, adds VDA1, and compares them with separately trained d256 checkpoints. Here d128 and d256 mean 128 or 256 coordinates in each location's recurrent xLSTM state. Across five admitted checkpoint pairs, the observed d256-minus-d128 point estimates do not share one direction. Absolute held-out decoder scores show strong change-occurrence readout at t6; realized-location readout remains above its target-specific chance level but varies materially across tasks and checkpoints. An assigned cued-key logit bias changes model $d'$ within evaluated checkpoints, but the displayed cross-checkpoint difference remains associative. Criterion is retained in the battery but is not evaluated in this manuscript. The results do not estimate a population width effect, training-seed variation, human capacity, or biological mechanism.
\end{abstract}
...
Regenerated archive figure & Present from aggregate NPZ (M3) & What checksum-recorded arrays display & Independent checkpoint rerun, uncertainty, or seed replication\\
...
Model intervention & Present as additive cued-key logit-bias clamps within admitted checkpoints & Model-internal sensitivity and SDT change under the defined dose regime & Causal effect of width, achieved attention allocation, or equivalence to biological microstimulation\\
...
Computationally, each RGB frame is divided into patch vectors. The previous recurrent state changes how current patch information is routed; shared recurrent rules then update one state per patch location. The actor converts the combined state into wait/change scores. The critic shapes training, and the auxiliary JEPA output can shape training when enabled with nonzero loss weight; neither is a behavioral measurement. In implementation terms these stages are the visual tokens, feedback router, spatial xLSTM, actor, quantile-regression critic, and optional auxiliary JEPA head. The cache does not establish whether JEPA was enabled for any historical run.
...
The first wave does not retroactively validate the aggregate M3 cache. It is a separate recomputation from one independently selected iteration-19999 checkpoint for each task--routing label: historical VDA4 and VDA9 crossed with \env{affine\_ew} and \env{crossattn1}. Its immutable 65-file tree binds executable source, dependencies, checkpoints, runtime, deterministic NPZ caches, and rendered products. The manifest SHA-256 is \texttt{b627814d\ldots2950677}; its read-only audit and independent artifact review passed. Full publication mechanics are reported in Methods.

The focused trial uses a red cue at S1 and a valid change at S1. The historical environments remain fully occupied: four locations in a $2\times2$ VDA4 grid and nine locations in a $3\times3$ VDA9 grid. The bottom-right invalid comparator is therefore S4 for VDA4 and S9 for VDA9. Figures~\ref{fig:first-wave-env-vda4} and \ref{fig:first-wave-env-vda9} make that geometry explicit.
...
Each attention product averages 96 deterministic evaluation trials. Rows preserve every spatial query patch; columns preserve logical timesteps t0--t6. Values are nonnegative and sum to approximately one across spatial keys before reduction. Affine attention exposes one key per spatial location. Cross-attention exposes image and recurrent-memory key groups; corresponding image and memory keys are paired and summed by location. All four figures use one fixed $[0,1]$ scale. Green and orange borders mark cue and change onset, respectively.

These maps establish where this checkpoint's routing weights were concentrated under the focused condition. They do not establish that the concentration caused the response, improved discriminability, or implements biological attention. In particular, the first wave does not include the registered invalid-change attention contrast; M4C and M4D therefore remain partial.

\begin{landscape}\thispagestyle{plain}\begin{center}
\includegraphics[width=0.74\linewidth]{first_wave_attention_vda4_affine_ew.pdf}
\captionof{figure}{\textbf{VDA4 affine attention.} \eclass{Checkpoint-recomputed descriptive measurement; 96 trials.} Four query rows and seven logical timesteps are retained. The spatial-key baseline is 0.250. Event alignment and shared scaling permit within-figure description, not a causal routing claim.}\label{fig:first-wave-attn-vda4-affine}
\vspace{0.5em}
\includegraphics[width=0.74\linewidth]{first_wave_attention_vda4_crossattn1.pdf}
\captionof{figure}{\textbf{VDA4 cross-attention.} \eclass{Checkpoint-recomputed descriptive measurement; 96 trials.} Image and memory keys are paired and summed by spatial location before plotting. The four query rows, seven logical timesteps, and fixed $[0,1]$ scale match Figure~\ref{fig:first-wave-attn-vda4-affine}.}\label{fig:first-wave-attn-vda4-cross}
\end{center}\end{landscape}

\begin{figure}[p]\centering
\includegraphics[width=0.98\linewidth]{first_wave_attention_vda9_affine_ew.pdf}
\caption{\textbf{VDA9 affine attention.} \eclass{Checkpoint-recomputed descriptive measurement; 96 trials.} Nine query rows and seven logical timesteps are retained. The spatial-key baseline is $1/9=0.111\ldots$. All cells share the same $[0,1]$ scale.}\label{fig:first-wave-attn-vda9-affine}
\end{figure}

\begin{figure}[p]\centering
\includegraphics[width=0.98\linewidth]{first_wave_attention_vda9_crossattn1.pdf}
\caption{\textbf{VDA9 cross-attention.} \eclass{Checkpoint-recomputed descriptive measurement; 96 trials.} Image and memory keys are paired and summed by spatial location. The complete $9\times7$ query--time layout prevents a single cued row from being mistaken for the model's spatial attention state.}\label{fig:first-wave-attn-vda9-cross}
\end{figure}

\clearpage
\subsection{Three-panel psychometrics with finite-trial intervals}
Each psychometric point uses 300 evaluation trials. The ordinate is the probability of a qualifying change response: the first change declaration at logical frame 5 or later, divided by all evaluation trials. Panels A and B evaluate S1 and bottom-right changes over displayed cue proportions of 25\%, 50\%, 75\%, and 100\%. Panel C forces valid and bottom-right changes at 100\% displayed validity. That panel is a location intervention, not a naturally sampled 100\%-validity trial. Wilson 95\% intervals describe finite evaluation-trial uncertainty for one checkpoint; they do not estimate training-seed variation.

Across the four checkpoint families, change-response probability generally rises with orientation change. Forced-valid point estimates generally exceed forced-invalid point estimates at intermediate magnitudes. The VDA9 cross-attention invalid curve remains below one at $30^\circ$ in both the natural-invalid and forced-invalid panels. Its pointwise agreement across those panels passed cache and visual audits; it is retained as checkpoint behavior rather than ``corrected'' into saturation. These are finite-trial results from one checkpoint per family. Cross-family differences remain descriptive because weights, training histories, task geometry, and token count are not jointly controlled.

\begin{landscape}\thispagestyle{plain}\begin{center}
\includegraphics[width=0.84\linewidth]{first_wave_psychometric_vda4_affine_ew.pdf}
\captionof{figure}{\textbf{VDA4 affine psychometrics.} \eclass{Checkpoint-recomputed evaluation; 300 trials per point.} Panels separate valid S1 changes, bottom-right S4 changes, and the forced-location contrast. Bands are Wilson evaluation-trial intervals for one checkpoint.}\label{fig:first-wave-psych-vda4-affine}
\vspace{0.7em}
\includegraphics[width=0.84\linewidth]{first_wave_psychometric_vda4_crossattn1.pdf}
\captionof{figure}{\textbf{VDA4 cross-attention psychometrics.} \eclass{Checkpoint-recomputed evaluation; 300 trials per point.} The all-trial denominator and forced S1-versus-S4 intervention match Figure~\ref{fig:first-wave-psych-vda4-affine}. Separate checkpoints preclude a causal affine--cross-attention contrast.}\label{fig:first-wave-psych-vda4-cross}
\end{center}\end{landscape}

\begin{landscape}\thispagestyle{plain}\begin{center}
\includegraphics[width=0.84\linewidth]{first_wave_psychometric_vda9_affine_ew.pdf}
\captionof{figure}{\textbf{VDA9 affine psychometrics.} \eclass{Checkpoint-recomputed evaluation; 300 trials per point.} The invalid location is S9, not a geometry-free ``opposing'' label. Wilson bands quantify finite evaluation-trial uncertainty only.}\label{fig:first-wave-psych-vda9-affine}
\vspace{0.7em}
\includegraphics[width=0.84\linewidth]{first_wave_psychometric_vda9_crossattn1.pdf}
\captionof{figure}{\textbf{VDA9 cross-attention psychometrics.} \eclass{Checkpoint-recomputed evaluation; 300 trials per point.} The lower S9 response at large changes is reproduced in the natural-invalid and forced-invalid panels. It is a verified single-checkpoint result, not a replicated routing effect or evidence of a nine-item capacity boundary.}\label{fig:first-wave-psych-vda9-cross}
\end{center}\end{landscape}
...
Each psychometric point contains 300 forced valid-location change trials at displayed validity 0.75. At $14^\circ$, the absolute d128/d256 response probabilities are 0.697/0.653 for VDA1 affine, 0.520/0.730 for VDA1 cross-attention, 0.663/0.777 for VDA4 affine, 0.617/0.487 for VDA4 cross-attention, and 0.640/0.597 for VDA9 affine. Thus the clearest contrast is a $+0.210$ VDA1 cross-attention difference, whereas VDA4 cross-attention differs by $-0.130$---a 21-percentage-point increase versus a 13-percentage-point decrease. Table~\ref{tab:absolute-behavior} reports every absolute constituent curve behind Figure~\ref{fig:matched-width}A. Across the five admitted d128/d256 pairs, the point-estimate differences do not share one direction. No interval is shown for these cross-checkpoint differences, and the design cannot separate architecture, training-run variation, and finite evaluation noise.
...
\captionof{figure}{\textbf{Matched-width checkpoint summary.} \eclass{Checkpoint-recomputed descriptive and model-intervention evidence.} Ordinates are d256 minus d128 for separately trained iteration-19999 checkpoints. Panel A shows valid-location response differences at displayed validity 0.75 (300 trials per point); Table~\ref{tab:absolute-behavior} gives the absolute constituent curves. Panel B shows t6 foldwise PCA-128 balanced-accuracy differences. Zero, not chance, is Panel B's plotted reference because the ordinate is d256 minus d128; chance values describe the unplotted absolute scores. Binary change occurrence uses all 900 trials (chance 0.5), realized location uses 450 change-present trials (chance 0.25 for VDA4 and $1/9$ for VDA9), and cued versus uncued location uses the same 450 trials (chance 0.5). For scale, VDA9 affine d256 realized-location balanced accuracy is 0.499 versus $1/9=0.111$ chance (d128: 0.654). Structurally chance t0 points are excluded; native warning localization is process-log-derived rather than hash-bound and is omitted conservatively. Panels C and D show the checkpoint contrast in within-checkpoint $d'$ change relative to zero bias (250 trials per cell); Table~\ref{tab:clamp-constituents} gives every within-checkpoint constituent. Affine routing biases the cued image key; cross-attention biases that image key and its memory key, so equal biases are not calibrated cross-routing doses or achieved-attention allocations. VDA1 invalid effects are undefined. VDA2 is blocked by a four-token d128 versus two-token d256 geometry mismatch. VDA9 cross-attention is omitted because d256 fails the registered acceptance threshold. Aggregate counts do not preserve trial identities; paired intervals are unavailable.}\label{fig:matched-width}
\end{center}\end{landscape}
...
\item audit allocation semantics before M4 and use shared scales and event alignment;
...
Figure code was developed under focused regression tests. The integrated M1--M4 plus matched-width revision passed a 161-test scientific gate. That suite covers cache and checkpoint identity, exact inventory, symlink and special-file rejection, integer metadata, attention semantics, target-specific decoder support and chance levels, sample-size conservation, undefined decoder estimands, competence gating, and read-only audit regressions. The ten previously admitted production figures retain their approved visual records; the matched-width PNG and an independently rasterized PDF were inspected before placement. The manuscript was compiled with XeLaTeX, diagnostics were scanned for overfull boxes, missing glyphs, and undefined references, and every final PDF page was rendered and inspected. The build manifest records source and artifact hashes.
