...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_v11_part2", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusV11Part2Model:
    """Construct the v11 dual-stream model exactly as train_rl.py does, from config."""
    m = cfg["model"]
    rl = m["rl"]
    model = RViTPlusV11Part2Model(
...
def load_checkpoint(model: RViTPlusV11Part2Model, ckpt_path: str, device: torch.device) -> int:
...
STREAM_NAMES = ["salience", "topdown"]   # attn[0]=salience (KV=[H1‖H2]), attn[1]=top-down (KV=H2)
...
def batched_attention_rollout(
    ...
    """Run B trials forced to wait and pull v11_part2's TWO dual-stream attentions.

    The image queries X attend OVER a memory bank: Tμ salience over [H1‖H2] (2N
    keys), TQ top-down over H2 (N keys). The spatially-meaningful per-head saliency
    is the attention each MEMORY POSITION receives (mean over the N image queries);
    for the salience stream we SUM the H1- and H2-key attention at each position so
    both streams yield an N-position (grid_h, grid_w) map. Heads kept separate.
    Returned index 0 = salience (→ actor), 1 = top-down (→ critic).

    Returns (mean_attn[s] : (T, n_heads, gh, gw), obs_example : (T,50,50,3)).
    """
    B = len(envs)
    model.eval()
    states = model.init_states(B, device=device)
    ...
    while t <= T and not done.all():
        ...
        for s in range(n_streams):
            ...
            if sal.shape[-1] == 2 * N:                    # salience: fold [H1‖H2] → per position
                sal = sal[..., :N] + sal[..., N:]
