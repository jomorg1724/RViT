"""
Forced-trial attention analysis on the RViT+ model.
...
Then we roll the trained model on this single trial and produce, per encoder
layer, a figure showing every timestep's attention signals.

For each layer we plot the SELECTED V-channels — i.e. the channels with
non-trivial weight in the (now-collapsed) channel softmax. Each such channel
of V is a (H, W) tensor; we show it next to (a) the raw input, (b) the
combined spatial residual `A · V` RMS, (c) the recurrent state `C_ℓ` RMS.
...
def _topk_channels_per_head(A_flat: torch.Tensor, n_heads: int, topk: int = 1) -> List[int]:
    """Given (C, 1, 1) channel-softmax weights, return a flat list of the top-k
    global channel indices, with k picks per head. Uses the SAME tensor each
    timestep so we get a stable channel set per layer.
    """
    C = A_flat.numel()
    d_head = C // n_heads
    flat = A_flat.reshape(n_heads, d_head)
    chosen: List[int] = []
    for h in range(n_heads):
        idx_in_head = torch.topk(flat[h], topk).indices.tolist()
        for i in idx_in_head:
            chosen.append(h * d_head + i)
    return chosen
...
def plot_layer_full_episode(
    ...
    *,
    ...
    Columns per row:
        0          : input frame
        1..K       : V·A contribution from the top-k channels per head
                     (one column per selected channel; channels picked once
                      at t = press_step (or last valid frame) so the set is
                      stable across rows).
    ...
    # Pick the timestep for selecting which channels to highlight. The softmax
    # is essentially constant across t for layers 1 and 2 (collapsed); for
    # layer 3 it does flicker, so we use the last frame.
    ref_t = T - 1
    A_ref = rollout["attn_per_frame"][ref_t][0][layer_idx].squeeze(0)  # (C, 1, 1)
    selected = _topk_channels_per_head(A_ref, n_heads, topk_per_head)
    K = len(selected)
    ...
    for t in range(T):
        ...
        # Columns 1..K — selected V-channel maps (the things that get weighted
        # by A and added to Z's residual). We re-derive V·A per channel by
        # gathering: weighted_channel_map = A_flat[c] * (the c-th channel of V)
        # Since we don't store V separately per channel after the FT block, we
        # pull from attn_spatial_per_frame which IS (A·V).
        spatial_t = rollout[spatial_seq_key][t][0][layer_idx].squeeze(0)   # (C, H, W)
        A_flat_t = rollout["attn_per_frame"][t][0][layer_idx].squeeze(0).reshape(-1)
        for k_idx, c in enumerate(selected):
            ax = fig.add_subplot(gs[t, 1 + k_idx])
            chan_map = _to_numpy(spatial_t[c])
            vmax = float(np.abs(chan_map).max()) + 1e-8
            ax.imshow(chan_map, cmap="seismic", vmin=-vmax, vmax=vmax)
            if t == 0:
                head = c // (spatial_t.shape[0] // n_heads)
                ax.set_title(f"h{head} c{c}\nA={float(A_flat_t[c]):.2f}", fontsize=7)
            ax.axis("off")
    ...
    fig.suptitle(
        f"{title_prefix} layer {layer_idx+1}: top-{topk_per_head} channels/head + spatial residual + state\n"
...
def plot_specialist_full_episode(
    ...
    *,
    ...
    """C₃ specialist (actor or critic) — all timesteps."""
    import matplotlib.pyplot as plt
    from matplotlib import gridspec
    ...
    if len(spec_seq) == 0 or not isinstance(spec_seq[0], list) or len(spec_seq[0]) == 0:
        ...
        return
    # spec_seq[t][0] is the dict
    ref_t = T - 1
    A_ref = spec_seq[ref_t][0][branch].squeeze(0)                      # (C, 1, 1)
    selected = _topk_channels_per_head(A_ref, n_heads, topk_per_head)
    K = len(selected)
    ...
    for t in range(T):
        ...
        # Selected channels of the specialist's A·V.
        spatial_t = spec_seq[t][0][f"{branch}_spatial"].squeeze(0)     # (C, H, W)
        A_flat_t  = spec_seq[t][0][branch].squeeze(0).reshape(-1)
        for k_idx, c in enumerate(selected):
            ax = fig.add_subplot(gs[t, 1 + k_idx])
            chan_map = _to_numpy(spatial_t[c])
            vmax = float(np.abs(chan_map).max()) + 1e-8
    ...
    fig.suptitle(
        f"{branch.upper()} specialist (cell3_{branch}) attention — every timestep\n"
        f"trial: cue=top-left red prop=1.0, change@t={rollout['change_step']}, "
        f"press@t={rollout['press_step']}",
        fontsize=10, y=1.0,
    )
...
def write_summary(rollout: dict, path: str) -> None:
    ...
    with open(path, "w", newline="") as f:
        ...
        w.writerow(["t", "action", "reward", "is_change", "is_press",
                    ...
                    "L1_softmax_ent", "L2_softmax_ent", "L3_softmax_ent"])
        for t in range(T):
            ...
            for L in range(3):
                ...
                A = A_flat.reshape(4, A_flat.numel() // 4).clamp(min=1e-12)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--topk-per-head", type=int, default=2,
                    help="How many channels per head to show as separate columns.")
    ...
    for L in range(3):
        plot_layer_full_episode(
            ...
            title_prefix=f"AE/shared (prop={args.proportion}, cue={args.cue_position}/{args.cue_color})",
