"""
RL trainer for V6 — PAC actor loss + distributional QR-DQN critic +
...
try:
    ...
    from model import V6ArenaModel  # type: ignore[no-redef]
...
class SegmentCollector:
    ...
    def collect(
        self, model: V6ArenaModel, n_segments: int, random_actions: bool = False,
...
def pac_update(
    ...
    init_states = V6ArenaModel.tensors_to_states(
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ckpt_path = os.path.join(checkpoint_dir, "v6_latest.pt")
            torch.save({
                "iter": it,
