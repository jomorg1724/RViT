...
try:
    ...
    from model import RViTPlusV8Part3Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # v8_part3 additions:
    # pred_loss_coef — weight on the comparator's SELF-SUPERVISED next-Z prediction
    #   loss (reward-independent, so the forward model / surprise signal learns even
    #   when RL is saturated on the ~1-2 bit task). 0 disables.
    # harbor_coef — small penalty on P(wait) probability mass (ACTOR phase only),
    #   closing the always-wait safe harbor so a collapse surfaces instead of hiding.
    #   Kept tiny; monitor premature_rate as a guardrail. 0 disables.
    pred_loss_coef: float = 0.1
    harbor_coef:    float = 0.01
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV8Part3Model] = None,
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_pred": 0.0, "loss_harbor": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0,
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        # ── comparator self-supervised predictor loss + always-wait-harbor close ──
        aux_seq = out.get("aux_pred_seq")
        loss_pred = logits_t.new_zeros(())
        if aux_seq is not None and cfg.pred_loss_coef > 0.0:
            aux_seq = torch.where(m.bool(), aux_seq, torch.zeros_like(aux_seq))
            loss_pred = (aux_seq * mw).sum() / denom_mw
        loss_harbor = logits_t.new_zeros(())
        if train_actor and cfg.harbor_coef > 0.0:
            p_wait = torch.softmax(logits_t, dim=-1)[..., 0]            # P(wait) (B,T); action 0 = wait
            loss_harbor = (p_wait * mw).sum() / denom_mw

        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive
                + cfg.pred_loss_coef * loss_pred + cfg.harbor_coef * loss_harbor)
        ...
        accumulated["loss_pred"]    += float(loss_pred.detach().item())
        accumulated["loss_harbor"]  += float(loss_harbor.detach().item())
    ...
    LOSS_KEYS = {"loss_policy", "loss_value", "loss_entropy", "loss_contrastive",
                 "loss_pred", "loss_harbor", "loss_total", "approx_kl"}
    result = {}
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v8_part3_rl_latest.pt")
