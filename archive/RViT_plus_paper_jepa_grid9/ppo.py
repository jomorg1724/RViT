...
def ppo_update(
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_jepa": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0,
    ...
    # ── JEPA / DINO teacher (separate EMA net), evaluated ONCE per update ──
    use_jepa = jepa_teacher is not None and cfg.jepa_coef > 0.0
    z_teacher = tau_t = None
    if use_jepa:
        jepa_teacher.eval()
        with torch.no_grad():
            jout = jepa_teacher.forward_rl_sequence(batch.observations, return_cell=True)
            z_teacher = jepa_teacher.jepa_logits(jout["cell_seq"])           # (B,T,4,K) teacher prototype logits
        frac = min(float(it) / max(1, cfg.jepa_tau_teacher_warmup_iters), 1.0)
        tau_t = cfg.jepa_tau_teacher_start + frac * (cfg.jepa_tau_teacher_end - cfg.jepa_tau_teacher_start)

    for _epoch in range(cfg.n_epochs):
        out = model.forward_rl_sequence(batch.observations, return_cell=use_jepa)
        ...
        if use_jepa:
            z_s = model.jepa_logits(out["cell_seq"])             # (B,T,4,n_heads,proto_dim) student logits
            ...
            # softmax over proto_dim (last) PER (token, head) → independent structured distributions
            p_t = torch.softmax((zt - model.jepa_center) / tau_t, dim=-1)        # centered + sharpened (no grad)
            logp_s = torch.log_softmax(zs / cfg.jepa_tau_student, dim=-1)        # student
            ce = -(p_t * logp_s).sum(-1)                                         # (B, Tj, 4, n_heads)
            vm = vmask[..., None, None].expand_as(ce)                            # mask over frames, broadcast tokens+heads
            loss_jepa = (ce * vm).sum() / vm.sum().clamp(min=1.0)
        ...
        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive + cfg.jepa_coef * loss_jepa)
    ...
    if use_jepa:
        with torch.no_grad():
            # one centre per (token, head, proto) → mean over batch+time only (keep tokens & heads)
            model.jepa_center.mul_(cfg.jepa_center_momentum).add_(
                z_teacher.mean(dim=(0, 1)), alpha=1.0 - cfg.jepa_center_momentum)

    n_evals = max(accumulated["n_loss_evals"], 1.0)
    LOSS_KEYS = {"loss_policy", "loss_value", "loss_entropy", "loss_contrastive",
                 "loss_jepa", "loss_total", "approx_kl"}
    result = {}
    ...
    *,
    ...
    if cfg.jepa_coef > 0.0:
        ...
        print(f"[setup] JEPA self-distillation: {'temporal t→t+1' if cfg.jepa_temporal else 'same-time'} "
              ...
              f"teacher EMA={cfg.jepa_ema_decay}, τs={cfg.jepa_tau_student} "
