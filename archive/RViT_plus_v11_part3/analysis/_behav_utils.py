...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_v11", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusV11Model:
    """Construct the v11 dual-stream model exactly as train_rl.py does, from config."""
    m = cfg["model"]
    rl = m["rl"]
    model = RViTPlusV11Model(
...
def load_checkpoint(model: RViTPlusV11Model, ckpt_path: str, device: torch.device) -> int:
