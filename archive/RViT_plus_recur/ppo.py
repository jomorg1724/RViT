...
try:
    from .model import RViTPlusRecurModel
except ImportError:  # pragma: no cover
    from model import RViTPlusRecurModel  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # VAE KL penalty on the recurrent latent (recur variant). SMALL by design — a strong KL
    # over-regularises the state. Only applied if the model exposes kl_seq.
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusRecurModel] = None,
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_kl": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0,
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        # ── VAE KL penalty on the recurrent latent (recur variant) ──
        loss_kl = logits_t.new_zeros(())
        if cfg.kl_beta > 0.0 and out.get("kl_seq") is not None:
            loss_kl = (out["kl_seq"] * m).sum() / m.sum().clamp(min=1.0)   # masked mean per valid step
            accumulated["loss_kl"] = accumulated.get("loss_kl", 0.0) + float(loss_kl.detach().item())

        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive + cfg.kl_beta * loss_kl)
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v9_rl_latest.pt")
