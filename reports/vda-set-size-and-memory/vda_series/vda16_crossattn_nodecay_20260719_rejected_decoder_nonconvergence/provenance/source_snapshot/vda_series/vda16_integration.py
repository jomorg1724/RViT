...
This module intentionally treats VDA16 cross-attention/d128/no-decay as one
descriptive checkpoint cell.  It does not extend the frozen matched-width
factorial battery, which would require affine and d256 VDA16 checkpoints plus
...
def validate_checkpoint_admission(
    ...
    *,
    ...
    return {
        ...
        "claim_boundary": (
            "single seed and one cross-attention/d128/no-decay checkpoint; no affine, "
            "width, decay, seed-population, or causal set-size contrast"
        ),
    }
...
def build_training_figure(
    ...
    figure.suptitle(
        f"{DISPLAY_NAME}\n"
        f"status: {summary['status']} · final θ={summary['final_theta_degrees']:.1f}° · "
    ...
    *,
    ...
    for condition_index, condition in enumerate(condition_names):
        ...
        for dose_index, dose in enumerate(matched.CLAMP_DOSE_PARAMETERS):
            ...
            for timestep in range(7):
                ...
                mass = raw[:, :, [cue, model.n_tokens + cue]].sum(dim=-1).mean(dim=-1)
    ...
    metadata = {
        ...
        "attention_mass_definition": (
            "sum of image-key and corresponding recurrent-memory-key mass for S1, "
...
def validate_achieved_clamp_attention(
    ...
    *,
    ...
    del expected_device  # device-independent numerical cache
    with np.load(path, allow_pickle=False) as payload:
        expected_keys = {
            "metadata_json",
            "conditions",
        ...
        np.testing.assert_allclose(
            payload["clamp_dose_parameters"], (0.0, 0.25, 0.5, 0.75, 1.0)
        )
        np.testing.assert_allclose(
            payload["clamp_key_logit_biases"], (-6.0, -3.0, 0.0, 3.0, 6.0)
        )
    return metadata
...
def build_achieved_clamp_attention_figure(
    ...
    *,
    ...
    figure.suptitle(
        f"{DISPLAY_NAME}\nAchieved image+memory cued-key mass (mean ± trial-level SEM)",
        fontweight="bold",
    )
    return _save_figure(figure, Path(output_stem))
...
def build_manuscript_summary_figure(
    ...
    *,
    ...
    figure.suptitle(
        f"{DISPLAY_NAME}\nstatus: {competence_status} · single-checkpoint descriptive evidence",
        fontweight="bold",
    )
    figure.text(
        0.5,
        -0.01,
        "No affine VDA16, d256 VDA16, decay contrast, or training-seed uncertainty is available.",
        ha="center",
        fontsize=9,
        color="#7A1F1F",
    )
...
def build_method_comparison_figure(
    ...
    """Add VDA16 to the descriptive d128 routing-family manuscript matrix."""
    ...
    cells[("vda16", "crossattn1")] = _load_npz(vda16_shard_path)
    ...
    for feedback in ("affine_ew", "crossattn1"):
        ...
        for task in tasks:
            ...
            values["clamp_effect"].append(
                float(data["clamp_dprime_valid"][-1] - data["clamp_dprime_valid"][0])
            )
        ...
        for axis, key in zip(
            axes.flat,
            ("cue_benefit", "change_decode", "cued_decode", "clamp_effect"),
            strict=True,
        ):
            axis.plot(set_sizes, values[key], **style)
    titles = (
        "A  Valid − invalid response rate\n(V=.75, nearest Δ=18°)",
        "B  Final-timestep change decoder",
        "C  Final-timestep cued-change decoder",
        "D  Valid d′ clamp contrast\n(+6 minus −6 logit bias)",
    )
    ylabels = ("Δ response probability", "balanced accuracy", "balanced accuracy", "Δd′")
    for axis, title, ylabel in zip(axes.flat, titles, ylabels, strict=True):
        axis.set_title(title, loc="left", fontweight="bold")
        ...
        axis.grid(alpha=0.2)
        axis.legend(frameon=False)
    axes[0, 0].text(1, 0.02, "undefined", rotation=90, va="bottom", fontsize=8)
    axes[1, 0].text(1, 0.02, "undefined", rotation=90, va="bottom", fontsize=8)
    for axis in axes.flat:
        axis.annotate(
            "VDA16 affine unavailable",
            xy=(16, 0),
            ...
            ha="right",
            fontsize=8,
            color=ROUTING_COLORS["affine_ew"],
        )
    figure.suptitle(
        "Descriptive d128 VDA task sequence with the new VDA16 cross-attention checkpoint\n"
        "One checkpoint per shown cell; this is not a controlled or replicated set-size effect",
        fontweight="bold",
    )
    return _save_figure(figure, Path(output_stem))
