...
@pytest.mark.parametrize("task", ["vda1", "vda2", "vda4", "vda9"])
def test_decode_sampling_replays_exactly_from_seed_and_config(task):
    first = D.sample_conditions(task, n=144, seed=991)
...
@pytest.mark.parametrize(
    ("task", "minimum"),
    [("vda1", 16), ("vda2", 16), ("vda4", 48), ("vda9", 128)],
)
...
def test_decode_main_writes_small_configured_result_without_changing_legacy_bytes(
    ...
    saved = D.main(["--out", str(requested_output), "--seed", "17", "--n", "128"])

    assert saved == output.resolve()
    assert output.is_file()
    assert len(load_calls) == 8
    assert len(collect_calls) == 8
    assert all(n == 128 and seed == 17 for _, n, seed in collect_calls)
...
def test_decode_main_rejects_paths_resolving_to_legacy_before_loading_models(
    ...
    with pytest.raises(ValueError, match="legacy"):
        D.main(["--out", str(legacy_alias), "--n", "128"])
...
def test_decode_main_rejects_hard_link_to_legacy_before_loading_models(
    ...
    with pytest.raises(ValueError, match="legacy"):
        D.main(["--out", str(legacy_hard_link), "--n", "128"])
