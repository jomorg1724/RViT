"""
RViTPlusV11Part3BroadcastModel — conv-free, RL-only recurrent attention model with PARALLEL
DUAL-STREAM attention and a SPLIT readout. v11_part3_broadcast is v11_part2 with the readout
destinations SWAPPED: the TQ TOP-DOWN stream feeds the ACTOR and the Tμ SALIENCE
stream feeds the CRITIC (v11_part2 had Tμ→actor, TQ→critic). The two transformers
are otherwise IDENTICAL to v11_part2 (see tx_lstm_encoder.py).

  ENCODER (DualStreamEncoder), SINGLE head each — unchanged from v11_part2:
       • Tμ SALIENCE:  Q=X, K=V=[H1 ‖ H2], residual=X → Z_sal  (grounded image stream).
       • TQ TOP-DOWN:  Q=X, K=V=H2,        residual=H2 → Z_td   (v8-style gating-only).
  ...
  DECODERS (SPLIT — the ONLY change vs v11_part2): the actor's 1D-CONV head reads
     ONLY the TQ top-down output Z_td; the critic's reads ONLY the Tμ salience output
     Z_sal. Each ingests a single stream (d_model channels), arranged (B,d,N) and run
...
Pipeline (no convolution stem, no PC/JEPA, no decoder/VAE):
    ...
    frame x_t (B,3,50,50)
       ...
       ├─► ActorDecoder([Z_td])   : (B,d,N)→Conv1d stack→flatten→logits (B, n_actions)
       └─► CriticDecoder([Z_sal]) : (B,d,N)→Conv1d stack→flatten→Q (B, n_actions, n_quantiles)
                                                          → V via derive_V
...
class RViTPlusV11Part3BroadcastModel(nn.Module):
    ...
    def _run_heads(
        ...
        # SPLIT readout (v11_part3_broadcast SWAPS v11_part2): rec = [Z_sal, Z_td].
        # TQ top-down (Z_td) → ACTOR; Tμ salience (Z_sal) → CRITIC.
        z_sal, z_td = recurrent_states[0], recurrent_states[1]
        actor_logits = self.actor_head([z_td])
        q_dist = self.critic_head([z_sal])
