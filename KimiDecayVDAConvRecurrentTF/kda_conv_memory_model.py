...
gives a 4x4-pixel patch per stimulus cell; n_channels=128; heads x d_v = C.
"""
...
def _tokens_to_nchw(t: torch.Tensor, H: int, W: int) -> torch.Tensor:
    """(B,H*W,C) -> (B,C,H,W)."""
    B, N, C = t.shape
...
def _two_stream_token_attn(Q, Kx, Kh, Vx, Vh, scale: float, split: bool = False,
    ...
    # AMP: QK^T + softmax in fp32 (256-token logits overflow fp16).
    q, kx, kh = q.float(), kx.float(), kh.float()
    vx, vh = vx.float(), vh.float()
    N = H * W
...
class KDAVisualAccumulator(nn.Module):
    ...
    def __init__(self, c: int = 128, heads: int = 4, head_dim: int = 32,
    ...
    def step(self, X_t: torch.Tensor, state, update_memory: bool = True,
        ...
        if self.v_mode in ("learned", "learned_z", "learned_all"):
            ...
            if self.v_mode == "learned_z":
                ...
                # rewritten EVERY step — no tick gating. Mnemonic noise kept
                # on the H1 write.
                H1 = Z
                if self.memory_noise_std > 0.0:
                    H1 = H1 + self.memory_noise_std * torch.randn_like(H1)
            elif update_memory:
