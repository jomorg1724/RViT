...
Usage:
    .venv/bin/python RViT_plus_v12_part2/train_rl.py
    .venv/bin/python RViT_plus_v12_part2/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v12_part2.config.loader import (
...
from RViT_plus_v12_part2.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--n-lstm", type=int, default=cfg_get(cfg, "model.n_lstm", 1),
                   help="Must be 1 — only the key-memory H2 is a true LSTM state.")
    p.add_argument("--codebook-decay", type=float, default=cfg_get(cfg, "model.codebook_decay", 0.9),
                   help="λ for the codebook-deviation leaky integrator D_t=λ·D_{t-1}+energy "
                        "(<1 ⇒ the codebook relaxes back to the static learned table).")
    ...
    p.add_argument("--energy-coef", type=float, default=cfg_get(cfg, "ppo.energy_coef", 0.01),
                   help="Weight on the codebook-deviation maintenance cost mean‖D_t‖². "
                        "Higher → stronger pull back to the static codebook. 0 disables.")
    ...
    print_resolved_config(cfg, used_keys=[
        ...
        "ppo.value_coef", "ppo.entropy_coef", "ppo.energy_coef", "ppo.qr_kappa",
    ...
    model_kwargs = dict(
        ...
        codebook_decay=args.codebook_decay, temperature=args.temperature,
    )
    model = RViTPlusV12Part2Model(**model_kwargs).to(device)
    ...
    print(f"[diag] d_model={args.d_model} d_mem={args.d_mem} tx[{args.tx_heads}h×{args.tx_layers}L] "
          f"+ 1×LSTM (H2) + adaptive codebook[decay={args.codebook_decay}, energy_coef={args.energy_coef}] "
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v12_part2_rl_final.pt")
