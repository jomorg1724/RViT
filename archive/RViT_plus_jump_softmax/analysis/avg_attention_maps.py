...
Usage:
    .venv/bin/python RViT_plus_jump_softmax/analysis/avg_attention_maps.py \
...
from RViT_plus_jump_softmax.analysis import _behav_utils as bu
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--checkpoint", default="RViT_plus_jump_softmax/checkpoints/rvit_plus_rl_latest.pt")
    ap.add_argument("--config", default=None)
    ap.add_argument("--out-dir", default="RViT_plus_jump_softmax/analysis/figures")
    ...
    # v11: TWO parallel attention STREAMS (salience reads H1, top-down reads H2).
    # The per-head map is the attention each MEMORY KEY receives (mean over image
    # queries), reshaped to the grid — i.e. which memory location the image reads.
    stream_names = ["salience", "topdown"]
    n_streams = len(stream_names)
    per_stream_strips: List[List[np.ndarray]] = [[] for _ in range(n_streams)]   # [s] -> list over conds of (T,H,gh,gw)
    ...
    for side, ring in conds:
        ...
        mean_attn, obs_ex = bu.batched_attention_rollout(model, envs, obs0, device, n_layers=n_streams)
        obs_examples.append(obs_ex)
        for s in range(n_streams):
            per_stream_strips[s].append(mean_attn[s])     # (T, H, gh, gw)
        rng_txt = ", ".join(f"{stream_names[s]}=[{m.min():.3f},{m.max():.3f}]" for s, m in enumerate(mean_attn))
        print(f"  [{label}] done  (per-head ranges: {rng_txt})")

    # Per-stream, per-condition outputs (heads kept separate).
    for s in range(n_streams):
        sname = stream_names[s]
        for ci, (label, strip4) in enumerate(zip(cond_labels, per_stream_strips[s])):
            safe = label.replace(" ", "").replace("·", "_").replace("(", "").replace(")", "")
            plot_head_strip(
                strip4, obs_examples[ci],
                os.path.join(args.out_dir, f"avg_attn_{sname}_{safe}_{suffix}.png"),
                title=f"{sname} stream  ·  per-HEAD attention-onto-memory  ·  {label}  ·  n={args.n_trials}",
                change_frame=args.change_time,
            )
            plot_head_alpha(
                strip4,
                os.path.join(args.out_dir, f"avg_attn_alpha_{sname}_{safe}_{suffix}.png"),
                layer_idx=s, change_frame=args.change_time, change_index=args.change_index,
                cond_label=f"{sname} · {label}",
            )
    ...
    for s in range(n_streams):
        save[stream_names[s]] = np.stack(per_stream_strips[s], axis=0)  # (n_cond, T, H, gh, gw)
