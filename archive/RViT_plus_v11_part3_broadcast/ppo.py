...
try:
    from .model import RViTPlusV11Part3BroadcastModel
except ImportError:  # pragma: no cover
    from model import RViTPlusV11Part3BroadcastModel  # type: ignore[no-redef]
...
def collect_episodes(
    model: RViTPlusV11Part3BroadcastModel,
    env,
    n_episodes: int,
    device: torch.device,
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV11Part3BroadcastModel] = None,
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v11_part3_broadcast_rl_latest.pt")
