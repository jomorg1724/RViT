#!/usr/bin/env bash
# Motion-ZK DMC + KDA + token QK^T two-stream attention. Fresh from collection 0.
# fp32 (no AMP): token+KDA grads NaN under fp16 on this recipe; A40 48GB holds batch 32.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_ROOT="${RUN_ROOT:-/workspace/runs/motionzk_kda_token_c128_seed0}"
DEVICE="${DEVICE:-cuda}"
mkdir -p "$RUN_ROOT/checkpoints"
exec "$HERE/.venv/bin/python" -u "$HERE/pretrain_kda_convmem.py" \
  --task motion_zk --label valid \
  --attn-mode token \
  --T 20 --min-change-time 12 --max-change-time 12 \
  --noise 5.0 --memory-noise-std 0.0 \
  --accum-mode kda --accum-decay 0.5 --kda-heads 4 --kda-head-dim 32 \
  --n-channels 128 --map-size 16 --proto-dim 256 \
  --frame-window 1 --frame-stride 1 --mem-every 3 \
  --jepa-coef 1.0 --change-coef 1.0 \
  --jepa-ema-decay 0.996 --jepa-tau-student 0.1 \
  --jepa-tau-teacher-start 0.03 --jepa-tau-teacher-end 0.05 --jepa-tau-warmup 300 \
  --jepa-center-momentum 0.9 --jepa-sinkhorn-iters 3 \
  --jepa-var-coef 1.0 --jepa-cov-coef 0.01 \
  --theta-start 65.0 --curr-threshold 0.85 --curr-step 3.0 --curr-floor 8.0 \
  --n-trials 500000 --collection-size 1024 --epochs 5 --batch-size 32 \
  --lr 0.0003 --grad-clip 1.0 --seed 0 \
  --device "$DEVICE" \
  --save-every 1 --log-every 1 \
  --checkpoint-dir "$RUN_ROOT/checkpoints"
