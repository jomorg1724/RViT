...
def configure_review_fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict:
    ...
    artifact_root = vda_root / "figures" / "first_wave"
    artifact_root.mkdir(parents=True)
    (artifact_root / "PRODUCTION_MANIFEST.json").write_text("{}\n", encoding="utf-8")
    return {
...
def write_reviews(candidate: dict, mutate=None) -> None:
    records = []
    artifact_manifest = M.VDA_ROOT / "figures" / "first_wave" / "PRODUCTION_MANIFEST.json"
    for index, (role, verdict) in enumerate(VERDICTS.items(), start=1):
        ...
        if mutate is not None:
            mutate(role, review)
        path = M.QA_ROOT / f"{M.REVIEW_PREFIX}_{role}.json"
...
def test_visual_qa_validation_accepts_current_schema_and_rejects_stale_digest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    visual_qa = tmp_path / "visual_qa.md"
    monkeypatch.setattr(M, "VISUAL_QA", visual_qa)
    visual_qa.write_text(
        "\n".join(
            [
                ...
                "PDF SHA-256: `pdf`",
                "Physical pages: `56`",
                "Ordered render-set SHA-256: `render`",
                "The rendered 56-page candidate passed full visual QA.",
            ]
        ),
        encoding="utf-8",
    )
    M.validate_visual_qa("source", "pdf", 56, "render")
    with pytest.raises(RuntimeError, match="stale or non-approving"):
        M.validate_visual_qa("source", "pdf", 56, "stale")
