...
try:
    from .model import RViTPlusSetsize9V12Model
except ImportError:  # pragma: no cover
    from model import RViTPlusSetsize9V12Model  # type: ignore[no-redef]
...
def collect_episodes(
    model: RViTPlusSetsize9V12Model,
    env,
    n_episodes: int,
    device: torch.device,
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusSetsize9V12Model] = None,
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_setsize9_v12_rl_latest.pt")
