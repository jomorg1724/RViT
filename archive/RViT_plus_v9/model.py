"""
RViTPlusV9Model — conv-free, RL-only recurrent attention model with MEMORY-QUERY attention: the carried H1 memory queries [X ++ H2] (2N keys), the image enters ONLY as attended values, and the skip path stays H1 (see tx_lstm_encoder.py).
...
class RViTPlusV9Model(nn.Module):
