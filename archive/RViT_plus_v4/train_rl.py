"""
RL training entry point for RViT+ v4 on Posner change-detection.

v4 is conv-free and RL-only: a patch embedding feeds a 2-layer MEMORY-AS-TOKENS
...
Usage:
    .venv/bin/python RViT_plus_v4/train_rl.py
    .venv/bin/python RViT_plus_v4/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v4.config.loader import (
...
from RViT_plus_v4.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    # ── Model architecture (conv-free patch + feedback transformer) ──
    ...
    p.add_argument("--enc-heads", type=int, default=cfg_get(cfg, "model.enc_heads", 4))
    p.add_argument("--enc-layers", type=int, default=cfg_get(cfg, "model.enc_layers", 2))
    p.add_argument("--n-FR", type=int, default=cfg_get(cfg, "model.n_FR", 1))
    p.add_argument("--dec-heads", type=int, default=cfg_get(cfg, "model.dec_heads", 4))
    ...
    p.add_argument("--head-hidden", type=int, default=cfg_get(cfg, "model.head_hidden", 128),
                   help="Hidden width of each decoder's MLP readout head (>=2 FF layers decode the CLS).")
    p.add_argument("--head-layers", type=int, default=cfg_get(cfg, "model.head_layers", 2),
                   help="Number of Linear layers in the decoder MLP readout head (min 2).")
    ...
    print_resolved_config(cfg, used_keys=[
        ...
        "model.patch_size", "model.patch_hidden", "model.d_model", "model.d_mem", "model.enc_heads",
        "model.enc_layers", "model.n_FR", "model.dec_heads", "model.dec_layers",
        "model.head_hidden", "model.head_layers",
    ...
    model_kwargs = dict(
        ...
        enc_heads=args.enc_heads, enc_layers=args.enc_layers, n_FR=args.n_FR,
    ...
    model = RViTPlusV4Model(**model_kwargs).to(device)
    ...
    print(f"[diag] patch_size={args.patch_size} → n_tokens={model.n_tokens} "
          ...
          f"decoder reads {model.enc_layers}×{model.n_tokens} + 1 CLS tokens)")
    print(f"[diag] d_model={args.d_model} d_mem={args.d_mem} enc[{args.enc_heads}h×{args.enc_layers}L] "
          f"dec[{args.dec_heads}h×{args.dec_layers}L] head[{args.head_layers}L×{args.head_hidden}] n_FR={args.n_FR}")
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v4_rl_final.pt")
