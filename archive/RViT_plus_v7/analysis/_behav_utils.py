...
and the forced-trial mechanism are IDENTICAL; the v5-specific pieces are:
  * build_model     → constructs RViTPlusV7Model from the v5 config
...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_v7", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusV7Model:
    """Construct the v7 model exactly as train_rl.py does, from the config."""
    m = cfg["model"]
    rl = m["rl"]
    model = RViTPlusV7Model(
...
def load_checkpoint(model: RViTPlusV7Model, ckpt_path: str, device: torch.device) -> int:
...
# ── forced trial specs (v7 semantics: cued-SIDE task + distractor control) ──
...
class ForcedTrialSpec:
    ...
    v7 ``change_index_mode`` semantics: targets live on the CUED side only.
      'cued'   → the cued quadrant itself (S1 for cue-left / S4 for cue-right)
      'uncued' → the OTHER quadrant of the SAME side (S2 / S3) — the v7
                 within-side analog of an invalid trial
      int      → explicit quadrant index (no side check)
    Distractor control (None = leave whatever reset() sampled):
      distractor_true       0/1 — force a distractor on/off
      distractor_time       frame at which the distractor change appears
      distractor_index_mode 'top'/'bottom' (of the uncued side) or explicit int
      distractor_mag        |Δθ| of the distractor (sign randomized)
    """
    ...
    distractor_true: Optional[int] = None
    distractor_time: Optional[int] = None
    distractor_index_mode: object = None
    distractor_mag: Optional[float] = None
...
def reset_with_spec(env: ChangeDetectionEnv, spec: ForcedTrialSpec, rng: np.random.Generator) -> np.ndarray:
    ...
    from RViT_plus_v7.env import SIDE_QUADRANTS
    env.reset()
    ...
    if mode is not None and int(env.change_true) == 1:
        if mode == "cued":
            env.change_index = side["primary"]
        elif mode == "uncued":
            env.change_index = side["secondary"]      # same SIDE, other quadrant
        elif isinstance(mode, int):
            env.change_index = int(mode)
        else:
            raise ValueError(f"bad change_index_mode: {mode!r}")
    elif int(env.change_true) == 1:
        # No explicit mode: (re)sample the target onto the cued side per the
        # (possibly overridden) proportion — keeps side-consistency under any
        # combination of cue/change_true/proportion overrides.
        env.change_index = (side["primary"] if rng.random() < env.proportion
                            else side["secondary"])
    # ── distractor overrides (uncued side) ──
    if spec.distractor_true is not None:
        env.distractor_true = int(spec.distractor_true)
    if spec.distractor_time is not None:
        env.distractor_time = int(spec.distractor_time)
    if spec.distractor_mag is not None:
        sign = 1.0 if rng.random() < 0.5 else -1.0
        env.distractor_change = float(sign * spec.distractor_mag)
    dmode = spec.distractor_index_mode
    if dmode is not None:
        top, bottom = sorted(side["distractor"])      # smaller index = top quadrant
        if dmode == "top":
            env.distractor_index = int(top)
        elif dmode == "bottom":
            env.distractor_index = int(bottom)
        elif isinstance(dmode, int):
            env.distractor_index = int(dmode)
        else:
            raise ValueError(f"bad distractor_index_mode: {dmode!r}")
    else:
        # keep the distractor on the (possibly new) uncued side
        env.distractor_index = int(rng.choice(side["distractor"]))
    env.t = 0
...
def batched_attention_rollout(
    ...
    """Run B trials forced to wait; per layer, take the memtok attention
    (B, heads, 3N, 3N), keep the columns over the N image-patch keys, and reduce
    to a PER-HEAD per-patch saliency — mean over QUERIES only, NOT over heads
    (heads can attend to opposite locations, so head-averaging cancels real
    structure), reshaped to (grid_h, grid_w) and averaged over trials.

    Returns (mean_attn[L] : (T, n_heads, gh, gw), obs_example : (T,50,50,3)).
    """
    B = len(envs)
    model.eval()
    states = model.init_states(B, device=device)
    ...
    while t <= T and not done.all():
        ...
        for L in range(n_layers):
            aw = attn[L]                                  # (B, heads, 3N, N_k...)
            patch_keys = aw[:, :, :, :N]                  # (B, heads, 3N_q, N_k) → attention TO patch tokens
            sal = patch_keys.mean(dim=2)                  # mean over QUERIES only → (B, heads, N_k)
        ...
        t += 1
    mean_attn = [np.stack(frames, axis=0) for frames in per_layer_frames]
