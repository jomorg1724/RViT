#!/usr/bin/env python3
"""Build a provenance-closed VDA16 routing-family manuscript evidence shard."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feedback",
        choices=("affine_ew", "crossattn1"),
        default="crossattn1",
        help="routing family bound to the completed checkpoint",
    )
    parser.add_argument(
        "--training-source-root",
        type=Path,
        default=ROOT,
        help="checkpoint-bound producer snapshot used for admission hash checks",
    )
    parser.add_argument(
        "--companion-vda16-bundle",
        type=Path,
        help="optional admitted bundle for the other VDA16 routing family",
    )
    parser.add_argument(
        "--device",
        choices=("cpu", "cuda"),
...
def _write_manuscript_fragments(
    ...
    routing_name = "affine element-wise" if feedback == "affine_ew" else "cross-attention"
    attention_definition = (
        "self-attention key mass"
        if feedback == "affine_ew"
        else "image-key plus corresponding recurrent-memory-key mass"
    )
    markdown = f"""# VDA16 {routing_name} manuscript integration
...
- Architecture: {routing_name}, d_mem=128, memory decay=1.0, 4×4/16 tokens
...
This is a single seed and a single {routing_name}/d128/no-decay checkpoint.
...
"""
    ...
    tex = rf"""\subsection{{VDA16 {routing_name} checkpoint}}
\label{{sec:vda16_{feedback}}}

We evaluated one VDA16 {routing_name} checkpoint ($d_\mathrm{{mem}}=128$,
...
\begin{{figure*}}[t]
  ...
  \caption{{VDA16 {routing_name} analysis. (A) Matched-bank response rates.
...
\begin{{figure*}}[t]
  ...
  series. {attention_definition.capitalize()} is averaged over query tokens.
...
def _runtime(device: str) -> dict[str, str]:
    ...
    record = {
        ...
        "deterministic_algorithms_enabled": str(
    ...
    if torch.cuda.is_available():
        ...
        record.update(
            {
                "cuda_device_index": str(index),
                "cuda_device_name": str(properties.name),
                "cuda_compute_capability": (
                    f"{properties.major}.{properties.minor}"
                ),
                "cuda_total_memory_bytes": str(properties.total_memory),
            }
        )
    return record
...
def build(args: argparse.Namespace) -> Path:
    ...
    integration.configure_feedback(args.feedback)
    ...
    admission = integration.validate_checkpoint_admission(
        ...
        launch_manifest,
        metrics_path,
        project_root=args.training_source_root,
    )
    ...
    shard_path = data_dir / f"vda16_{args.feedback}_d128_nodecay_seed0.npz"
    ...
    attention_cache = data_dir / f"attention_vda16_{args.feedback}.npz"
    ...
    attention_meta = first_wave.validate_attention_cache(
        attention_cache,
        expected_task="vda16",
        expected_feedback=args.feedback,
    ...
    psychometric_cache = data_dir / f"psychometric_vda16_{args.feedback}.npz"
    ...
    companion_evidence: dict[str, Any] | None = None
    if args.companion_vda16_bundle is not None:
        companion_root = args.companion_vda16_bundle.resolve()
        companion_manifest_path = audit_completed_output(companion_root)
        companion_manifest = json.loads(companion_manifest_path.read_text(encoding="utf-8"))
        companion_feedback = str(companion_manifest["scope"]["feedback"])
        companion_candidates = list(
            (companion_root / "data").glob(
                f"vda16_{companion_feedback}_d128_nodecay_seed0.npz"
            )
        )
        if len(companion_candidates) != 1:
            raise RuntimeError("companion VDA16 shard inventory is ambiguous")
        companion_freeze = provenance_dir / "companion_vda16"
        companion_freeze.mkdir(parents=True)
        frozen_companion_manifest = companion_freeze / "MANIFEST.json"
        frozen_companion_shard = companion_freeze / companion_candidates[0].name
        shutil.copy2(companion_manifest_path, frozen_companion_manifest)
        shutil.copy2(companion_candidates[0], frozen_companion_shard)
        companion_training = companion_manifest["checkpoint_admission"]["training"]
        companion_evidence = {
            "feedback": companion_feedback,
            "source_bundle": str(companion_root),
            "source_manifest_sha256": integration.sha256_file(companion_manifest_path),
            "frozen_manifest": str(frozen_companion_manifest.relative_to(output_root)).replace("\\", "/"),
            "frozen_manifest_sha256": integration.sha256_file(frozen_companion_manifest),
            "frozen_shard": str(frozen_companion_shard.relative_to(output_root)).replace("\\", "/"),
            "frozen_shard_sha256": integration.sha256_file(frozen_companion_shard),
            "training": companion_training,
        }
        companion_kwargs = {
            "companion_vda16_shard_path": frozen_companion_shard,
            "companion_vda16_feedback": companion_feedback,
            "companion_competence_status": companion_training["status"],
            "companion_final_theta": companion_training["final_theta_degrees"],
        }
    ...
    manifest = {
        ...
        "companion_vda16_evidence": companion_evidence,
