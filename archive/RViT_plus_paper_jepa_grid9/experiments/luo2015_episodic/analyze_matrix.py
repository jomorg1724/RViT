...
def balanced_trial_bank(
    *,
    ...
    """Build one deterministic, session-consistent bank shared by every policy.

    One environment instance fixes the two orientations at each location for the
    complete bank. Global NumPy state is restored so analysis cannot perturb
    training or another evaluation.
    """
    ...
    try:
        ...
        environment = _env("luo2015_sensitivity", float(magnitude))
...
def _press_times_batched(model, videos: torch.Tensor, batch_size: int) -> np.ndarray:
    outputs = [
        press_times(model, videos[start:start + batch_size])
