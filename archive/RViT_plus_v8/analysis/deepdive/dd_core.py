...
from RViT_plus_v8.analysis import _behav_utils as bu  # noqa: E402
...
DEFAULT_CKPT = "/Users/jonathanmorgan/rvit_plus_checkpoints/v8/v8_analysis_snapshot.pt"
...
def cross_attn_forward_step(
    ...
    For each ``_CrossAttnBlock`` (there are tx_layers of them, default 1):
        ...
        Z      = res + drop(a)     # v8 H1-RESIDUAL: res = raw Hs[0] for block 0
    ...
    for li, blk in enumerate(encoder.blocks):
        ...
        a, aw = manual_cross_attention(blk.attn, q_in, kv, sb, n_patch_keys=N, value_edit=ve)
...
def _selfcheck() -> None:
    ...
    # zeroing the patch value stream must CHANGE the output (the straw carries content)
    blind = lambda li, attn, v, out_patch, out_mem: (torch.zeros_like(out_patch), out_mem)  # noqa: E731
    ...
    print(f"[selfcheck] encoder rec max|Δ|={d_enc:.2e}  (Hs/Cs Δ={d_state:.2e})  "
          ...
          f"identity-hook Δ={d_hook:.2e}  zero-patch-hook Δ={d_blind:.2e}")
    ...
    assert d_blind > 1e-3, "zeroing the patch value stream did not change the output?!"
