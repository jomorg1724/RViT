...
from model import ConvRViTModel                                         # noqa: E402
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train the fully-convolutional recurrent ViT")
...
def main() -> None:
    ...
    model = ConvRViTModel(**model_kwargs).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[conv] task={args.task} tokens={model.n_tokens} particles={n_quantiles} "
    ...
    ckpt_dir = (args.checkpoint_dir or cfg_get(cfg, "run.checkpoint_dir", "")
                or os.path.expanduser(f"~/rvit_plus_checkpoints/conv_{args.task}"))
    ...
    if init_mode in ("warm_start", "resume"):
        ...
        if not ckpt_path:
            prefer = [os.path.join(ckpt_dir, "rvit_plus_rl_latest.pt"),
                      os.path.join(ckpt_dir, f"rvit_conv_{args.task}_final.pt")]
        ...
        print(f"[conv] {init_mode} LOADED {ckpt_path}\n[conv]   -> {info}")
    else:
        print("[conv] init_mode=fresh (random init)")
    ...
    print(f"[conv] {iters} iters × {eps} episodes/iter = {iters*eps:,} episodes; γ={ppo_cfg.gamma}")
    ...
    final = os.path.join(ckpt_dir, f"rvit_conv_{args.task}_final.pt")
    torch.save({"iter": iters, "model_state_dict": model.state_dict(),
                "model_kwargs": model_kwargs, "task": args.task}, final)
    print(f"[conv] saved {final}; iters logged={len(history)}")
