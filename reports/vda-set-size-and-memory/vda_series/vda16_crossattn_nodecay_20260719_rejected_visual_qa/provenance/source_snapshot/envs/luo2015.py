...
simultaneous sample Gabors occupy opposite locations; after a delay, one location is
tested with the same or the other of two session-fixed orientations. A declaration to
...
class LuoMaunsell2015Env(BaseChangeDetectionEnv):
    ...
    def __init__(self, session: str = "sensitivity",
                 ...
                 base_orientations: dict[int, float] | None = None, **kw):
        ...
        if base_orientations is None:
            self.base_orientations = {
                loc: float(np.random.uniform(0.0, 180.0)) for loc in self.LOC
            }
        else:
            if set(base_orientations) != set(self.LOC):
                raise ValueError(f"base_orientations must have keys {self.LOC}")
            self.base_orientations = {
                loc: float(base_orientations[loc]) for loc in self.LOC
            }
    ...
    def reset(self):
        ...
        # Each location has a fixed session pair {base_i, base_i+theta}; the sampled
        # member is randomized independently on each trial.
        self.samp, hi = {}, {}
        for i in self.LOC:
        ...
        if self.change_true:                          # test = the OTHER orientation (differs by theta)
            self.test_ori = self.base_orientations[self.test_loc] + (
                0.0 if hi[self.test_loc] else theta
            )
        else:                                         # test = same as the sample at that location
            self.test_ori = self.samp[self.test_loc]
        self.second_test_ori = self.base_orientations[self.test_loc] + (
            0.0 if hi[self.test_loc] else theta
        )
    ...
    def training_state_dict(self) -> dict:
        ...
        state["environment_config"].update({
            ...
            "base_orientations": dict(self.base_orientations),
