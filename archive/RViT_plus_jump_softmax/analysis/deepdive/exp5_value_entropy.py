...
Usage:
  .venv/bin/python -m RViT_plus_jump_softmax.analysis.deepdive.exp5_value_entropy \
...
from RViT_plus_jump_softmax.analysis import _behav_utils as bu  # noqa: E402,F401
from RViT_plus_jump_softmax.analysis.deepdive import dd_core as dd  # noqa: E402
...
def main(argv=None):
    ...
    summary = {"iter": it, "gamma": gamma, "change_time": args.change_time,
               "arch": "v11 (parallel dual-stream salience+top-down cross-attn + 2 LSTMs + 1D-conv heads on [Z_sal++Z_td])"}
    ...
    # ── Part 3: attention → value/entropy (all-heads SALIENCE-key bias sweep) ──
    # v11 has no patch keys; the grounded stimulus-evidence pathway is the SALIENCE
    # stream (image queries reading recent sensory memory H1). Biasing its keys is
    # the closest analog of v5's "stimulus-evidence gain" lever.
    sweep = []
    for b in args.biases:
        bias = None
        if b != 0:
            bias = dd.make_attn_bias(model, device, stream="sal", head=None,
                                     region="all", value=float(b))
    ...
    fig.suptitle(f"v11 dual-stream · Value function appropriateness · change@t={Tg} · "
