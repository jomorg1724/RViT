"""
RViT+ "jump" model — a FiLM-affine feedback transformer + a HIERARCHICAL STACK OF 3
memory-transformers, designed around a manifold/jump-diffusion view of working memory.
...
# ── memory update as a transformer (H1 on the RESIDUAL = drift/integration path) ─
class MemoryTransformer(nn.Module):
    """The working memory is itself a transformer. We self-attend over the UNION of the new
    percept Z and the prior memory H1 (both serve as keys AND queries), and write the result
    back onto H1 with a RESIDUAL:  H1 ← H1 + Attn([Z‖H1])[H1 part] + FFN.

    The residual on H1 is the crucial integration path (additive drift; gradient highway over
    time) that the earlier renormalised MLP lacked — evidence accumulates, and a frame where H1
    attends hard to a changed Z yields a large write (a jump). No gates. Token-mixing IS allowed
    now (the memory attends across patches + to the percept), unlike the per-token RNN."""
    def __init__(self, in_dim=512, d_mem=1024, n_heads=8, n_tokens=5, drop=0.1):
        ...
        self.norm_in = nn.LayerNorm(d_mem)
        self.attn = nn.MultiheadAttention(d_mem, n_heads, dropout=drop, batch_first=True)
        self.norm_ff = nn.LayerNorm(d_mem)
        self.ffn = nn.Sequential(nn.Linear(d_mem, 4 * d_mem), nn.GELU(),
                                 nn.Dropout(drop), nn.Linear(4 * d_mem, d_mem))
        self.drop = nn.Dropout(drop)
        # anti-explosion: LayerScale (small writes) + a learned leaky/OU rate (bounds the residual
        # stream to a steady state ≈ write/λ instead of growing ~linearly with T). Both are
        # constants, NOT input-dependent gates; the OU leak is the mean-reverting drift.
        self.ls_attn = nn.Parameter(torch.full((d_mem,), 0.1))
        self.ls_ffn = nn.Parameter(torch.full((d_mem,), 0.1))
    ...
    def forward(self, inp, H_prev, return_attn=False):
        ...
        h = H_prev + self.type_emb[1] + self.pos_emb                   # memory tokens (B,N,d_mem)
        seq = self.norm_in(torch.cat([z, h], dim=1))                    # (B,2N,d_mem); pre-LN
        out, aw = self.attn(seq, seq, seq, need_weights=return_attn, average_attn_weights=True)
        write = self.ls_attn * self.drop(out[:, N:])                   # LayerScale → small writes
        lam = self.decay.clamp(0.02, 0.9)                            # FLOOR 0.02: leak can't train to 0 (=unbounded accumulator)
        H = (1.0 - lam) * H_prev + write                              # leaky/OU residual (bounded, integrates)
        H = H + self.ls_ffn * self.ffn(self.norm_ff(H))
...
class RViTPlusJumpModel(nn.Module):
    def __init__(self, in_channels=3, image_h=50, image_w=50, patch_size=25,
                 ...
                 init_action_bias=None, seq_len=29, drop=0.1, n_mem=3, **_ignore):
        super().__init__()
