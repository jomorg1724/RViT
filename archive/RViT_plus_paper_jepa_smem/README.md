# RViT+ — SOFTMAX-MEMORY JEPA variant (`_smem`)

**This variant (built 2026-06-29):** the JEPA student's per-head softmax `S` (jepa_heads ×
jepa_proto_dim, e.g. 4×256 = 1024-d per patch) is the **recurrent memory used everywhere** —
it feeds the cross-attention K/V, is the actor/critic readout, AND is the JEPA distillation
target (the loss distils the same head's pre-softmax logits). The xLSTM keeps its CONTINUOUS
cell internally (its recurrence runs on the continuous `H`, not `S`) and produces `S` each
step via `encoder.mem_head` (the single owner of the structured head). Enabled by
`--softmax-memory`; implemented for `--feedback crossattn1 --cell xlstm` only. `--d-mem` sizes
ONLY the internal continuous cell — the externalized memory width is jepa_heads×jepa_proto_dim
regardless. Requires `--jepa-coef > 0` (the distillation is what shapes the memory).

Canonical run (no-VAE from pixels + frame-repeat×5, matching the model that reached ~0.87):

```
python RViT_plus_paper_jepa_smem/train_rl.py --device mps --init-mode fresh \
  --T 7 --frame-repeat 5 --min-change-time 5 --max-change-time 5 \
  --cell xlstm --feedback crossattn1 --vae-color --jepa-coef 0.5 \
  --softmax-memory --d-mem 1024 --checkpoint-dir ~/rvit_plus_checkpoints/paper_jepa_smem
```
