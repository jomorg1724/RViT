...
def decoder_support_contract(
    shards: dict[tuple[str, str, int], dict[str, np.ndarray]],
) -> dict[str, Any]:
    """Derive target-specific support and balanced-accuracy chance levels from labels."""
    contracts: dict[str, Any] = {}
    label_keys = {
        "change": "decoder_sample_label_change",
        "change_location": "decoder_sample_label_change_location",
        "cued_change": "decoder_sample_label_cued_change",
    }
    for task in TASKS:
        task_contract: dict[str, Any] = {}
        for estimand, key in label_keys.items():
            if task == "vda1" and estimand != "change":
                task_contract[estimand] = {
                    "defined": False,
                    "effective_samples": 0,
                    "class_counts": {},
                    "class_count": 0,
                    "balanced_accuracy_chance": None,
                    "reason": "singleton task has no distinct change-location comparison",
                }
                continue
            signatures: set[tuple[tuple[int, int], ...]] = set()
            for feedback in FEEDBACKS:
                for width in WIDTHS:
                    labels = np.asarray(shards[(task, feedback, width)][key], dtype=np.int64)
                    if estimand != "change":
                        labels = labels[labels >= 0]
                    values, counts = np.unique(labels, return_counts=True)
                    signatures.add(tuple(zip(values.tolist(), counts.tolist(), strict=True)))
            if len(signatures) != 1:
                raise ValueError(f"decoder class support differs within {task}/{estimand}: {signatures}")
            signature = signatures.pop()
            task_contract[estimand] = {
                "defined": bool(signature),
                "effective_samples": int(sum(count for _, count in signature)),
                "class_counts": {str(value): int(count) for value, count in signature},
                "class_count": len(signature),
                "balanced_accuracy_chance": (1.0 / len(signature)) if signature else None,
            }
        contracts[task] = task_contract
    return contracts


def trainable_parameter_count(state_dict: dict[str, torch.Tensor]) -> int:
    """Count checkpoint parameters while excluding registered non-parameter buffers."""
    buffer_keys = {"jepa_center", "critic_head.taus"}
    return int(sum(value.numel() for key, value in state_dict.items() if key not in buffer_keys))


def checkpoint_inventory(upstream: dict[str, Any]) -> list[dict[str, Any]]:
    """Reconstruct model identity and trainable size from hash-verified checkpoints."""
    records: list[dict[str, Any]] = []
    for checkpoint in upstream["checkpoints"]:
        path = Path(str(checkpoint["path"]))
        if path.stat().st_size != int(checkpoint["bytes"]) or sha256(path) != checkpoint["sha256"]:
            raise RuntimeError(f"checkpoint identity mismatch: {path}")
        payload = torch.load(path, map_location="cpu", weights_only=True)
        state_dict = payload["model_state_dict"]
        grid = [int(value) for value in checkpoint["grid"]]
        records.append(
            {
                "task": checkpoint["task"],
                "feedback": checkpoint["feedback"],
                "recurrent_width": int(checkpoint["width"]),
                "iteration": int(checkpoint["iteration"]),
                "checkpoint_path": str(path),
                "checkpoint_sha256": checkpoint["sha256"],
                "checkpoint_bytes": int(checkpoint["bytes"]),
                "token_geometry": {"grid": grid, "token_count": int(np.prod(grid))},
                "active_locations": [int(value) for value in checkpoint["active_locations"]],
                "trainable_parameter_count": trainable_parameter_count(state_dict),
            }
        )
    return sorted(records, key=lambda record: (record["task"], record["feedback"], record["recurrent_width"]))
...
def build_figure(summary: dict[str, Any], output_stem: Path) -> None:
    ...
    ax_b.text(
        0.02,
        0.035,
        "VDA1 location probes undefined:\nsingleton task · chance N/A",
        transform=ax_b.transAxes,
        ha="left",
        va="bottom",
        fontsize=8,
        color=COLORS["vda1"],
        bbox={"boxstyle": "round,pad=0.3", "facecolor": "white", "edgecolor": COLORS["vda1"], "alpha": 0.92},
    )
    handles, labels = ax_a.get_legend_handles_labels()
    fig.legend(handles, labels, loc="outside lower center", ncol=3, frameon=False)
    footer_lines = (
        "Iteration-19999 separately trained checkpoint contrasts · point estimates · not causal width effects · no seed replication or paired intervals",
        "N: behavior 300/point; decoder 900 (change) or 450 (location probes); clamp 250/cell · affine target: image/self key · cross-attention: image + memory key",
        "Equal biases are not equal cross-routing doses or achieved attention · VDA2 blocked (4-token d128 vs 2-token d256) · VDA9 cross-attention d256 competence-gated",
    )
...
def build(input_root: Path, output_root: Path) -> Path:
    ...
    decoder_support = decoder_support_contract(shards)
    models = checkpoint_inventory(upstream)
    ...
    sidecar = {
        ...
        "matched_width_definition": "Comparison of recurrent-state width within a fixed task geometry and routing family; total trainable parameter count is not matched.",
        "blocked_cells": upstream["blocked_cells"],
        "model_inventory": models,
        "dependency_closure": {
            "self_contained": False,
            "classification": "validated projection of the immutable upstream battery",
            "external_dependencies": "Read-only re-audit requires the absolute upstream root, its 12 immutable shards, hash-bound source graph, and checkpoint files.",
        },
        ...
        "decoder": {
            ...
            "target_support_by_task": decoder_support,
            "estimands": {
                "change": {
        ...
        "limits": [
            ...
            "Warning localization came from a reviewed but non-hash-bound process log. Native cued-change trajectories for VDA9 affine d256 and VDA9 cross-attention d128 are omitted conservatively; finite matched PCA-128 scores are reported without claiming a hash-bound no-warning ledger.",
    ...
    validation_path = provenance / "VALIDATION_RESULT.json"
    validation = {
        "schema_version": 1,
        "recorded_after_artifact_build": True,
        "result": "pass",
        "checks": {
            "upstream_read_only_audit": "pass",
            "summary_reconstruction": "pass",
            "trial_contract": "pass",
            "decoder_support_contract": "pass",
            "checkpoint_inventory": "pass",
        },
        "upstream_manifest_sha256": sha256(input_root / "MANIFEST.json"),
        "summary_json_sha256": sha256(sidecar_path),
        "figure_sha256": {
            suffix: sha256(stem.with_suffix(f".{suffix}")) for suffix in ("pdf", "svg", "png")
        },
    }
    ...
    manifest = {
        ...
        "validation_gate": {"result": "pass", "record": "provenance/VALIDATION_RESULT.json"},
...
def audit(output_root: Path) -> Path:
    ...
    expected_decoder_support = decoder_support_contract(shards)
    expected_models = checkpoint_inventory(upstream)
    ...
    if sidecar.get("decoder", {}).get("target_support_by_task") != expected_decoder_support:
        raise RuntimeError("decoder support and chance contract does not reconstruct from labels")
    if sidecar.get("model_inventory") != expected_models:
        raise RuntimeError("checkpoint model inventory does not reconstruct")
    if sidecar.get("blocked_cells") != upstream.get("blocked_cells"):
        raise RuntimeError("blocked-cell scope does not match upstream")
    validation = json.loads((output_root / "provenance" / "VALIDATION_RESULT.json").read_text(encoding="utf-8"))
    if validation.get("result") != "pass" or not validation.get("recorded_after_artifact_build"):
        raise RuntimeError("post-build validation result is absent or failed")
    if validation.get("summary_json_sha256") != sha256(output_root / "figures" / "matched_width_summary.json"):
        raise RuntimeError("post-build validation is not bound to the summary JSON")
