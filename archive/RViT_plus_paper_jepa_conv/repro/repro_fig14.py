...
A: δ@change vs Δ, cued (S1) vs uncued (S4) change.  B: V@change vs Δ.  C: δ@change vs enhancing
...
dS1,vS1=vd_curve(0); dS4,vS4=vd_curve(3)
# C,D: sweep α4 (enhance uncued change at S4), fixed Δ
A4=[0.0,0.25,0.5,0.75,1.0]; MAG=18.0; dA=[];hA=[]
for a in A4:
    cl=S.clamp_alpha(K,3,a); V=C._tens([C.make_video(0,1.0,"red",1,3,MAG) for _ in range(N)])
...
ax[0,0].plot(DE,dS1,"o-",label="cued change (S1)"); ax[0,0].plot(DE,dS4,"s-",label="uncued change (S4)")
...
ax[0,1].plot(DE,vS1,"o-",label="cued (S1)"); ax[0,1].plot(DE,vS4,"s-",label="uncued (S4)")
