...
from RViT_plus.encoder import RViTPlusEncoder
from RViT_plus.latent import VAELatentSampler
from RViT_plus.decoder import IterativeReconstructionDecoder
from RViT_plus.losses import kl_to_unit_gaussian, pixel_recon_loss, slowness_loss
from RViT_plus.model import RViTPlusModel
from RViT_plus.stem import V1Stem
...
def test_encoder():
    ...
    # Channel attention: per-layer attn is (B, C, 1, 1) — one weight per channel.
    _check("attn_per_iter[0][0] shape (B, 64, 1, 1)",
           tuple(out["attn_per_iter"][0][0].shape) == (B, 64, 1, 1))
    _check("attn_per_iter[0][1] shape (B, 96, 1, 1)",
           tuple(out["attn_per_iter"][0][1].shape) == (B, 96, 1, 1))
    _check("attn_per_iter[0][2] shape (B, 128, 1, 1)",
           tuple(out["attn_per_iter"][0][2].shape) == (B, 128, 1, 1))
...
def test_full_model():
    ...
    # Channel attention: per-layer attn shape (B, C, 1, 1).
    _check("encoder_attn_per_frame[0][0][0] shape (B, 64, 1, 1)",
           tuple(out.encoder_attn_per_frame[0][0][0].shape) == (B, 64, 1, 1))
...
def test_rl_heads_gradient_flow():
    ...
    from RViT_plus.rl_heads import quantile_huber_loss
    B, T = 2, 3
