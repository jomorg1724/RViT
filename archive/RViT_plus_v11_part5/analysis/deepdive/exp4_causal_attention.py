...
Usage:
  .venv/bin/python -m RViT_plus_v11_part2.analysis.deepdive.exp4_causal_attention \
...
from RViT_plus_v11_part2.analysis.deepdive import dd_core as dd  # noqa: E402
...
def main(argv=None):
    ...
    # v11_part2 split readout: salience (→ ACTOR) reads [H1‖H2]; top-down (→ CRITIC)
    # reads H2. Hypothesis: salience levers move the DECISION, top-down levers move
    # value/uncertainty. Salience spatial levers target its H2-key block (h2quad) or
    # H1-key block (h1quad) at the changed quadrant.
    region_specs = [
        ("sal_all",      "sal", "all",    None, "salience keys [H1‖H2] (→ actor), all"),
        ("td_all",       "td",  "all",    None, "top-down keys H2 (→ critic), all"),
        ("sal_h2_cued",  "sal", "h2quad", CUED, "salience H2-keys @ changed quad (S1)"),
        ("sal_h1_cued",  "sal", "h1quad", CUED, "salience H1-keys @ changed quad (S1)"),
        ("td_cued",      "td",  "quad",   CUED, "top-down H2-keys @ changed quad (S1)"),
    ]
    ...
    for name, stream, region, quad in [("sal_h2_changeS1_invalid", "sal", "h2quad", 0),
    ...
    cued_regions = ["sal_all", "td_all", "sal_h2_cued", "sal_h1_cued", "td_cued"]
