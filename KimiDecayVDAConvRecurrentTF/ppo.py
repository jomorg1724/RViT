...
try:
    ...
    try:
        from model import RViTPaperModel  # type: ignore[no-redef]
    except ImportError:  # standalone use (e.g. pretrain_conv_memory only needs the loss utils)
        from typing import Any as RViTPaperModel  # type: ignore[no-redef]
...
def collect_episodes(
    ...
    representation_teacher: Optional[RViTPaperModel] = None,
    ...
    """
    model.eval()
    if representation_teacher is not None:
        representation_teacher.eval()
    N = model.n_quantiles
    ...
    with torch.no_grad():
        for _ in range(n_episodes):
            ...
            teacher_states = (
                ...
                if representation_teacher is not None else None
            )
            ...
            while not done:
                ...
                step = model.rl_step(x_t, states, inject_memory_noise=True)

                if representation_teacher is not None:
                    teacher_step = representation_teacher.rl_step(
                        x_t, teacher_states, inject_memory_noise=False
                    )
                    actor_logits, q_dist, V_dist, V_scalar = model._run_heads(
                        teacher_step["rec"]
                    )
                    step["actor_logits"] = actor_logits
                    step["critic_q_dist"] = q_dist
                    step["V_dist"] = V_dist
                    step["V_scalar"] = V_scalar
                    teacher_states = teacher_step["new_states"]
    ...
    def _pad_obs(seq, T):
        hwc = np.asarray(seq[0]).shape           # infer (H,W,C) from the actual frame (50 or 75 etc.)
        out = np.zeros((T, *hwc), dtype=np.float32)
...
class PPOConfig:
    ...
    bc_alpha:        float = 0.0      # α blend: 0 = pure MPO/no BC, 1 = pure BC
    ...
    # EMA target (user-requested for the paper reproduction): if > 0, θ' is updated
    # every iteration as θ' ← decay·θ' + (1−decay)·θ (a Polyak/EMA target on BOTH the
    # actor and the critic), instead of the periodic hard-copy. 0 = use hard-copy.
    ema_decay: float = 0.0
    ...
    # ── JEPA / DINO temporal self-distillation on the cell output (OFF by default) ──
    jepa_coef: float = 0.0                  # weight on the JEPA cross-entropy (set >0 to enable)
    jepa_n_heads: int = 4                   # structured softmax heads per token (each softmaxed independently)
    jepa_proto_dim: int = 256               # softmax dim per head (prototypes within each embedding)
    jepa_temporal: bool = True              # True: student@t predicts teacher@t+1 (V-JEPA); False: same-time (ablation)
    jepa_tau_student: float = 0.1           # student softmax temperature
    jepa_tau_teacher_start: float = 0.04    # teacher temp (warmup start)
    jepa_tau_teacher_end: float = 0.07      # teacher temp (warmup end)
    jepa_tau_teacher_warmup_iters: int = 300
    jepa_center_momentum: float = 0.9       # DINO teacher-centering EMA
    jepa_ema_decay: float = 0.996           # JEPA teacher EMA (DECOUPLED from the RL target)
    jepa_sinkhorn_iters: int = 0            # >0: balance teacher prototypes per layer/token/head
    jepa_var_coef: float = 0.0              # variance-floor penalty on student projection features
    jepa_cov_coef: float = 0.0              # covariance-decorrelation penalty on those features
    teacher_actor_critic_ste: bool = False  # teacher forward representation; student backward via STE
...
def compose_total_loss(
    *,
    ...
    cfg: PPOConfig,
    jepa_multiplier: float = 1.0,
) -> torch.Tensor:
    """Compose the configured objective while keeping each developmental weight explicit."""
    return (
        cfg.actor_coef * loss_policy
        + cfg.value_coef * loss_value
        + cfg.entropy_coef * loss_entropy
        + cfg.contrastive_coef * loss_contrastive
        + cfg.jepa_coef * jepa_multiplier * loss_jepa
    )


def structured_jepa_loss(
    ...
    *,
    ...
    """DINO cross-entropy for one memory or an explicit H1/H2 layer axis.

    Single-memory logits are ``(B,T,P,heads,proto)``. Stacked-memory logits are
    ``(B,T,L,P,heads,proto)``. The returned vector has one independently
    normalized loss per memory level; the scalar is their mean.
    """
    if teacher_logits.shape != student_logits.shape:
        raise ValueError(
            "teacher and student JEPA logits must have the same aligned shape; "
            f"got {tuple(teacher_logits.shape)} and {tuple(student_logits.shape)}"
        )
    if teacher_logits.dim() not in (5, 6):
        raise ValueError("JEPA logits must be single-layer 5D or stacked-layer 6D tensors")
    if valid_mask.shape != teacher_logits.shape[:2]:
        raise ValueError("valid_mask must match the JEPA batch/time dimensions")

    teacher_scores = (teacher_logits - center) / tau_teacher
    if sinkhorn_iters > 0:
        p_t = sinkhorn_teacher_assignments(
            teacher_scores, valid_mask, n_iters=sinkhorn_iters
        )
    else:
        p_t = torch.softmax(teacher_scores, dim=-1)
    logp_s = torch.log_softmax(student_logits / tau_student, dim=-1)
    ce = -(p_t * logp_s).sum(dim=-1)
    vm = valid_mask.reshape(*valid_mask.shape, *([1] * (ce.dim() - 2))).expand_as(ce)

    if teacher_logits.dim() == 6:
        layer_axis = 2
        reduce_dims = tuple(dim for dim in range(ce.dim()) if dim != layer_axis)
        layer_losses = (ce * vm).sum(dim=reduce_dims) / vm.sum(dim=reduce_dims).clamp(min=1.0)
    else:
        layer_losses = ((ce * vm).sum() / vm.sum().clamp(min=1.0)).unsqueeze(0)
    return layer_losses.mean(), layer_losses


def masked_jepa_center(teacher_logits: torch.Tensor, valid_mask: torch.Tensor) -> torch.Tensor:
    """Average teacher logits over valid batch/time entries, preserving structured axes."""
    if valid_mask.shape != teacher_logits.shape[:2]:
        raise ValueError("valid_mask must match the JEPA teacher batch/time dimensions")
    weights = valid_mask.reshape(
        *valid_mask.shape, *([1] * (teacher_logits.dim() - valid_mask.dim()))
    )
    return (teacher_logits * weights).sum(dim=(0, 1)) / valid_mask.sum().clamp(min=1.0)


@torch.no_grad()
def sinkhorn_teacher_assignments(
    teacher_scores: torch.Tensor,
    valid_mask: torch.Tensor,
    *,
    n_iters: int = 3,
) -> torch.Tensor:
    """Balanced assignments for each structured layer/token/head group.

    Batch and time are the assignment-sample axes. All remaining axes except the
    final prototype axis define independent groups. Invalid padded timesteps are
    returned as zeros and never participate in balancing.
    """
    if teacher_scores.dim() not in (5, 6):
        raise ValueError("teacher scores must be 5D or 6D structured JEPA tensors")
    if valid_mask.shape != teacher_scores.shape[:2]:
        raise ValueError("valid_mask must match teacher batch/time dimensions")
    if n_iters <= 0:
        raise ValueError("n_iters must be positive")

    valid = valid_mask.bool()
    output = torch.zeros_like(teacher_scores)
    if not valid.any():
        return output

    selected = teacher_scores[valid]                       # (N,*groups,K)
    n_samples, n_proto = selected.shape[0], selected.shape[-1]
    group_shape = selected.shape[1:-1]
    n_groups = math.prod(group_shape)
    scores = selected.reshape(n_samples, n_groups, n_proto).permute(1, 2, 0)
    scores = scores - scores.amax(dim=1, keepdim=True)
    q = scores.exp().clamp_min(torch.finfo(scores.dtype).tiny)  # (G,K,N)
    q /= q.sum(dim=(1, 2), keepdim=True).clamp_min(1e-12)
    for _ in range(n_iters):
        q /= q.sum(dim=2, keepdim=True).clamp_min(1e-12)
        q /= float(n_proto)
        q /= q.sum(dim=1, keepdim=True).clamp_min(1e-12)
        q /= float(n_samples)
    q *= float(n_samples)                                   # columns sum to one
    balanced = q.permute(2, 0, 1).reshape(n_samples, *group_shape, n_proto)
    output[valid] = balanced
    return output


def jepa_variance_covariance_loss(
    ...
    *,
    ...
    """VICReg-style variance floor and covariance penalty on student features.

    Features are ``(B,T,P,F)`` or ``(B,T,L,P,F)``. Each memory level is
    regularized independently after pooling valid batch/time/token samples.
    """
    if features.dim() not in (4, 5):
        raise ValueError("JEPA projection features must be 4D or 5D")
    if valid_mask.shape != features.shape[:2]:
        raise ValueError("valid_mask must match feature batch/time dimensions")
    if features.dim() == 4:
        features = features.unsqueeze(2)
    selected = features[valid_mask.bool()]                  # (N,L,P,F)
    ...
    for layer in range(selected.shape[1]):
        x = selected[:, layer].reshape(-1, selected.shape[-1])
        x = x - x.mean(dim=0, keepdim=True)
        std = torch.sqrt(x.var(dim=0, unbiased=False) + eps)
        variance_losses.append(torch.relu(float(variance_target) - std).mean())
        denom = max(x.shape[0] - 1, 1)
        cov = x.T @ x / float(denom)
        off_diagonal = cov - torch.diag_embed(torch.diagonal(cov))
        covariance_losses.append(off_diagonal.square().sum() / cov.shape[0])
    return torch.stack(variance_losses).mean(), torch.stack(covariance_losses).mean()


def straight_through_teacher_representation(
    student: torch.Tensor,
    teacher: torch.Tensor,
) -> torch.Tensor:
    """Return teacher values in forward while routing backward only to student."""
    if student.shape != teacher.shape:
        raise ValueError(
            f"student/teacher representation shapes differ: {tuple(student.shape)} vs "
            f"{tuple(teacher.shape)}"
        )
    return student + (teacher.detach() - student).detach()


def actor_critic_from_teacher_memory_ste(
    model: RViTPaperModel,
    student_cells: torch.Tensor,
    teacher_cells: torch.Tensor,
) -> dict[str, torch.Tensor]:
    """Decode teacher H2 in forward while actor/critic representation gradients reach student H2."""
    if student_cells.dim() != 5 or student_cells.shape[2] != 2:
        raise ValueError(
            "teacher actor/critic STE requires (B,T,2,tokens,d_mem) student memory; "
            f"got {tuple(student_cells.shape)}"
        )
    routed_h2 = straight_through_teacher_representation(
        student_cells[:, :, 1], teacher_cells[:, :, 1]
    )
    return model.heads_from_memory_sequence(routed_h2)
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPaperModel] = None,
    train_actor: bool = True,
    jepa_teacher: Optional[RViTPaperModel] = None,
    it: int = 0,
) -> dict:
    ...
    accumulated = {
        ...
        "loss_policy_weighted": 0.0, "loss_value_weighted": 0.0,
        "loss_jepa_weighted": 0.0,
        ...
        "student_encoder_grad_max": 0.0, "actor_head_grad_max": 0.0,
        "critic_head_grad_max": 0.0, "jepa_teacher_grad_max": 0.0,
    ...
    use_jepa = jepa_teacher is not None and cfg.jepa_coef > 0.0
    use_teacher_actor_critic_ste = bool(cfg.teacher_actor_critic_ste)
    if use_teacher_actor_critic_ste and jepa_teacher is None:
        raise ValueError("teacher_actor_critic_ste requires an initialized JEPA teacher")

    # Evaluate the detached JEPA teacher representation once. It serves both the
    # unchanged temporal JEPA targets and, in STE mode, actor/critic forward values.
    teacher_cells = z_teacher = tau_t = None
    if use_jepa or use_teacher_actor_critic_ste:
        jepa_teacher.eval()
        with torch.no_grad():
            jout = jepa_teacher.forward_rl_sequence(batch.observations, return_cell=True)
            teacher_cells = jout["cell_seq"]
            if use_jepa:
                z_teacher = jepa_teacher.jepa_logits(teacher_cells)
        if use_jepa:
            frac = min(float(it) / max(1, cfg.jepa_tau_teacher_warmup_iters), 1.0)
            tau_t = cfg.jepa_tau_teacher_start + frac * (
                cfg.jepa_tau_teacher_end - cfg.jepa_tau_teacher_start
            )
    ...
    if use_target:
        target_model.eval()
        with torch.no_grad():
            if use_teacher_actor_critic_ste:
                tout = target_model.heads_from_memory_sequence(teacher_cells[:, :, 1])
            else:
    ...
    for _epoch in range(cfg.n_epochs):
        out = model.forward_rl_sequence(
            batch.observations,
            return_cell=(use_jepa or use_teacher_actor_critic_ste),
            inject_memory_noise=True,
        )
        if use_teacher_actor_critic_ste:
            student_h2 = out["cell_seq"][:, :, 1]
            out.update(actor_critic_from_teacher_memory_ste(model, out["cell_seq"], teacher_cells))
            with torch.no_grad():
                accumulated["teacher_student_h2_mse"] += float(
                    F.mse_loss(student_h2, teacher_cells[:, :, 1]).item()
                )
                accumulated["teacher_student_h2_cosine"] += float(
                    F.cosine_similarity(
                        student_h2.flatten(2), teacher_cells[:, :, 1].flatten(2), dim=-1
                    ).mean().item()
                )
        ...
        # ── PAC actor loss (MPO; optional historical BC), reference = target π_θ' ──
        ...
        if train_actor:
            ...
            # M-step. The default α=0 path is pure MPO and does not even construct
            # a behavior-cloning loss. Historical BC is opt-in only.
        ...
        # ── JEPA / DINO temporal self-distillation on the cell output ──
        loss_jepa = logits_t.new_zeros(())
        if use_jepa:
            z_s = model.jepa_logits(out["cell_seq"])
            if cfg.jepa_temporal:                                # student@t predicts teacher@t+1
                zt, zs, vmask = z_teacher[:, 1:], z_s[:, :-1], (m[:, 1:] * m[:, :-1])
            else:                                                # same-time ablation
                zt, zs, vmask = z_teacher, z_s, m
            loss_jepa_ce, layer_jepa_losses = structured_jepa_loss(
                zt,
                zs,
                model.jepa_center,
                vmask,
                tau_teacher=tau_t,
                tau_student=cfg.jepa_tau_student,
                sinkhorn_iters=cfg.jepa_sinkhorn_iters,
            )
            student_features = model.jepa_features(out["cell_seq"])
            if cfg.jepa_temporal:
                student_features = student_features[:, :-1]
            loss_jepa_var, loss_jepa_cov = jepa_variance_covariance_loss(
                student_features, vmask
            )
            loss_jepa = (
                loss_jepa_ce
                + cfg.jepa_var_coef * loss_jepa_var
                + cfg.jepa_cov_coef * loss_jepa_cov
            )
            accumulated["loss_jepa"] += float(loss_jepa.detach().item())
            accumulated["loss_jepa_ce"] += float(loss_jepa_ce.detach().item())
            accumulated["loss_jepa_var"] += float(loss_jepa_var.detach().item())
            accumulated["loss_jepa_cov"] += float(loss_jepa_cov.detach().item())
            if getattr(model, "dual_actor_critic_streams", False):
                accumulated["loss_jepa_actor"] += float(layer_jepa_losses[0].detach().item())
                accumulated["loss_jepa_critic"] += float(layer_jepa_losses[1].detach().item())
            else:
                accumulated["loss_jepa_h1"] += float(layer_jepa_losses[0].detach().item())
        ...
        accumulated["loss_policy_weighted"] += float((cfg.actor_coef * loss_policy).detach().item())
        accumulated["loss_value_weighted"] += float((cfg.value_coef * loss_value).detach().item())
        accumulated["loss_jepa_weighted"] += float(
            (cfg.jepa_coef * jepa_multiplier * loss_jepa).detach().item()
        )
        ...
        def _module_grad_max(module: nn.Module) -> float:
            return max(
                (float(p.grad.detach().abs().max().item()) for p in module.parameters()
                 if p.grad is not None),
                default=0.0,
            )

        accumulated["student_encoder_grad_max"] = max(
            accumulated["student_encoder_grad_max"], _module_grad_max(model.encoder)
        )
        accumulated["actor_head_grad_max"] = max(
            accumulated["actor_head_grad_max"], _module_grad_max(model.actor_head)
        )
        accumulated["critic_head_grad_max"] = max(
            accumulated["critic_head_grad_max"], _module_grad_max(model.critic_head)
        )
        if jepa_teacher is not None:
            accumulated["jepa_teacher_grad_max"] = max(
                accumulated["jepa_teacher_grad_max"], _module_grad_max(jepa_teacher)
            )
    ...
    # DINO teacher-centering: EMA the center (on the student) from this update's teacher logits.
    if use_jepa:
        with torch.no_grad():
            # Preserve layer/token/head/prototype axes and exclude padded episode tails.
            batch_center = masked_jepa_center(z_teacher, m)
            model.jepa_center.mul_(cfg.jepa_center_momentum).add_(
                batch_center, alpha=1.0 - cfg.jepa_center_momentum)
...
def training_iteration_numbers(*, start_iteration: int, n_iterations: int) -> range:
    """Return global iteration numbers for a fresh or resumed training phase."""
    if start_iteration < 0:
        raise ValueError("start_iteration must be non-negative")
    if n_iterations < 0:
        raise ValueError("n_iterations must be non-negative")
    return range(start_iteration, start_iteration + n_iterations)


def validate_training_schedule_bounds(
    *,
    phase_final_iteration: int,
    schedule_final_iteration: int,
    allow_schedule_overrun: bool = False,
) -> None:
    """Reject phase horizons beyond the saved schedule unless explicitly acknowledged."""
    if schedule_final_iteration < phase_final_iteration and not allow_schedule_overrun:
        raise ValueError("schedule_final_iteration cannot precede this phase's final iteration")


def _capture_rng_state(*, required_backend: Optional[str] = None) -> dict:
    state = {
    ...
    if torch.backends.mps.is_available() and hasattr(torch.mps, "get_rng_state"):
        try:
            ...
            pass
    backend_key = {"cpu": "torch_cpu", "cuda": "torch_cuda", "mps": "torch_mps"}.get(required_backend)
    if backend_key is not None and backend_key not in state:
        raise RuntimeError(f"could not capture required {required_backend} RNG state")
    return state


def _restore_rng_state(state: dict, *, required_backend: Optional[str] = None) -> None:
    backend_key = {"cpu": "torch_cpu", "cuda": "torch_cuda", "mps": "torch_mps"}.get(required_backend)
    if backend_key is not None and backend_key not in state:
        raise ValueError(f"checkpoint is missing required {required_backend} RNG state")
    random.setstate(state["python"])
    np.random.set_state(state["numpy"])
    torch.set_rng_state(state["torch_cpu"].cpu())
    if "torch_cuda" in state and torch.cuda.is_available():
        torch.cuda.set_rng_state_all([value.cpu() for value in state["torch_cuda"]])
    if "torch_mps" in state and torch.backends.mps.is_available() and hasattr(torch.mps, "set_rng_state"):
...
def build_training_checkpoint(
    *,
    ...
    target_step: int,
    jepa_teacher: Optional[nn.Module],
    env,
    rolling_correct,
    rolling_return,
    metadata: dict,
    ...
    """Capture resumable trainer state, deliberately excluding episode replay."""
    payload = {
        ...
        "target_model_state_dict": (
            copy.deepcopy(target_model.state_dict()) if target_model is not None else None
        ),
        "target_step": int(target_step),
        "jepa_teacher_state_dict": (
            copy.deepcopy(jepa_teacher.state_dict()) if jepa_teacher is not None else None
        ),
        "environment_state": copy.deepcopy(env.training_state_dict()),
        "rolling_correct": [float(value) for value in rolling_correct],
        "rolling_return": [float(value) for value in rolling_return],
        "rng_state": _capture_rng_state(required_backend=required_backend),
    }
...
def validate_resume_contract(
    ...
    *,
    ...
    active_backend: str,
    allowed_producer_changes: Optional[set[str]] = None,
) -> None:
    saved = checkpoint.get("resume_contract")
    if not isinstance(saved, dict):
        raise ValueError("checkpoint has no replay-excluded resume contract")
    saved_backend = saved.get("training_backend")
    if saved_backend != active_backend:
        raise ValueError(f"checkpoint backend {saved_backend!r} does not match active backend {active_backend!r}")
    for key in ("task", "model_kwargs", "ppo_config", "episodes_per_iter", "schedule_final_iteration",
                "runtime_versions"):
        if saved.get(key) != current_contract.get(key):
            raise ValueError(f"resume contract mismatch for {key}")

    saved_producers = saved.get("producer_sha256")
    current_producers = current_contract.get("producer_sha256")
    if not isinstance(saved_producers, dict) or not isinstance(current_producers, dict):
        raise ValueError("resume contract mismatch for producer_sha256")
    allowed = set(allowed_producer_changes or ())
    if saved_producers.keys() != current_producers.keys() or any(
        saved_producers[path] != current_producers[path]
        for path in saved_producers.keys() - allowed
    ):
        raise ValueError("resume contract mismatch for producer_sha256")
...
def restore_training_checkpoint(
    ...
    *,
    ...
    """Restore all persisted trainer state and return loop-owned scalar history."""
    if int(checkpoint.get("checkpoint_schema_version", 0)) < 3:
        raise ValueError("checkpoint is not a schema-v3 replay-excluded trainer-state checkpoint")
    model.load_state_dict(checkpoint["model_state_dict"], strict=True)
    optimizer.load_state_dict(checkpoint["optimizer_state_dict"])

    target_state = checkpoint.get("target_model_state_dict")
    if (target_model is None) != (target_state is None):
        raise ValueError("checkpoint PAC target-network configuration mismatch")
    if target_model is not None:
        target_model.load_state_dict(target_state, strict=True)

    teacher_state = checkpoint.get("jepa_teacher_state_dict")
    if (jepa_teacher is None) != (teacher_state is None):
        raise ValueError("checkpoint JEPA-teacher configuration mismatch")
    if jepa_teacher is not None:
        jepa_teacher.load_state_dict(teacher_state, strict=True)

    env.load_training_state_dict(checkpoint["environment_state"])
    try:
        _restore_rng_state(checkpoint["rng_state"], required_backend=required_backend)
    except (ValueError, RuntimeError, TypeError) as exc:
        print(f"[resume] RNG restore skipped ({exc}); continuing with freshly seeded RNGs",
              flush=True)
    return {
        "target_step": int(checkpoint.get("target_step", 0)),
        "rolling_correct": [float(value) for value in checkpoint.get("rolling_correct", [])],
        "rolling_return": [float(value) for value in checkpoint.get("rolling_return", [])],
    }


def reconcile_metrics_for_resume(path, *, checkpoint_iteration: int):
    """Truncate crash-surviving metric rows newer than the checkpoint being resumed."""
    import csv
    ...
    with open(path, newline="") as stream:
        ...
        if not fieldnames or "iter" not in fieldnames:
            raise ValueError(f"metrics file has no iter column: {path}")
        rows = list(reader)
    iterations = [int(row["iter"]) for row in rows]
    if any(current <= previous for previous, current in zip(iterations, iterations[1:])):
        raise ValueError(f"metrics iterations must be strictly increasing: {path}")
    kept = [row for row in rows if int(row["iter"]) <= checkpoint_iteration]
    if not kept or int(kept[-1]["iter"]) != checkpoint_iteration:
        raise ValueError(
            f"metrics file does not contain checkpoint iteration {checkpoint_iteration}: {path}"
        )
...
def save_training_checkpoint(path, checkpoint: dict) -> None:
...
def train(
    ...
    *,
    ...
    schedule_final_iteration: Optional[int] = None,
    resume_checkpoint: Optional[dict] = None,
    checkpoint_metadata: Optional[dict] = None,
    allow_schedule_overrun: bool = False,
    allowed_resume_producer_changes: Optional[set[str]] = None,
) -> list[dict]:
    """PAC + QR-DQN + PER training loop (RL only — no recon, no PC, no pretrain)."""
    if device is None:
        device = next(model.parameters()).device
    phase_final_iteration = start_iteration + n_iterations - 1
    if schedule_final_iteration is None:
        schedule_final_iteration = phase_final_iteration
    validate_training_schedule_bounds(
        phase_final_iteration=phase_final_iteration,
    ...
    # Writes EVERY iteration's losses + behaviour + current orientation-change level
    # (env.theta) to metrics.csv, flushed each row so it survives a crash. Pure
    # instrumentation — does not touch the loss/algorithm.
    import csv  # local import; keeps the top-level harness imports byte-identical
    _metrics_path = os.path.join(checkpoint_dir, "metrics.csv") if checkpoint_dir else None
    _metrics_f = None
    _metrics_w = None
    _resume_metrics_fieldnames = None
    ...
    print(f"[setup] PAC: η={cfg.mpo_temperature}, bc_α={cfg.bc_alpha}; "
          f"actor={cfg.actor_coef}, critic={cfg.value_coef}, JEPA={cfg.jepa_coef}, "
          f"entropy={cfg.entropy_coef}, γ={cfg.gamma}")
    if cfg.teacher_actor_critic_ste:
        print(
            "[setup] TEACHER→A/C STE: forward=detached EMA-teacher H2; "
            "backward=online student H2; online actor/critic heads remain trainable"
        )
    ...
    if use_ema or (cfg.target_update_period and cfg.target_update_period > 0):
        ...
        target_model.eval()
        if use_ema:
            print(f"[setup] PAC target network: EMA θ'←{cfg.ema_decay}·θ'+{1-cfg.ema_decay:.4g}·θ "
                  f"every iteration (actor + critic; critic TD target + E-step reference π_θ')")
        else:
        ...
        print("[setup] PAC target network: DISABLED — online bootstrap")

    # ── JEPA / DINO teacher: a SEPARATE EMA copy (decoupled from the PAC target) ──
    jepa_teacher = None
    if cfg.jepa_coef > 0.0:
        ...
        for p in jepa_teacher.parameters():
            p.requires_grad_(False)
        jepa_teacher.eval()
        nt=int(getattr(model,"n_tokens",4))
        print(f"[setup] JEPA self-distillation: {'temporal t→t+1' if cfg.jepa_temporal else 'same-time'} "
              f"on the cell output, {cfg.jepa_n_heads} heads × {cfg.jepa_proto_dim}-way softmax per token "
              f"({nt} tokens → {nt*cfg.jepa_n_heads} independent softmaxes), coef={cfg.jepa_coef}, "
              f"teacher EMA={cfg.jepa_ema_decay} (fixed), τs={cfg.jepa_tau_student} "
              f"τt={cfg.jepa_tau_teacher_start}→{cfg.jepa_tau_teacher_end}; "
              f"sinkhorn_iters={cfg.jepa_sinkhorn_iters}, var_coef={cfg.jepa_var_coef}, "
              f"cov_coef={cfg.jepa_cov_coef}")
        if getattr(model, "dual_actor_critic_streams", False):
            print("[setup] JEPA branches: independent actor + critic students/EMA targets/centers; "
                  f"per-branch coef={cfg.jepa_coef}")
    ...
    if resume_checkpoint is not None:
        ...
        current_contract = checkpoint_metadata.get("resume_contract")
        if isinstance(saved_contract, dict) and isinstance(current_contract, dict):
            validate_resume_contract(
                resume_checkpoint,
                current_contract=current_contract,
                active_backend=device.type,
                allowed_producer_changes=allowed_resume_producer_changes,
            )
        else:
            print("[resume] checkpoint has no resume_contract; restoring "
                  "weights/opt/target/teacher/env/RNG only (replay empty)", flush=True)
        restored = restore_training_checkpoint(
            resume_checkpoint,
            model=model,
            optimizer=optimizer,
            target_model=target_model,
            jepa_teacher=jepa_teacher,
            env=env,
            required_backend=device.type,
        )
        target_step = restored["target_step"]
        _correct_buf.extend(restored["rolling_correct"])
        _return_buf.extend(restored["rolling_return"])
        if _metrics_path is not None:
            _resume_metrics_fieldnames = reconcile_metrics_for_resume(
                _metrics_path,
                checkpoint_iteration=int(resume_checkpoint["iter"]),
            )
        print(
            f"[resume] restored optimizer, PAC target, JEPA teacher, theta={env.theta:.1f}, "
            f"curriculum_window={len(env._recent_correct)}, RNG, and rolling metrics; replay starts empty"
        )

    def _checkpoint_payload(iteration: int) -> dict:
        return build_training_checkpoint(
            ...
            jepa_teacher=jepa_teacher,
            env=env,
            rolling_correct=_correct_buf,
            rolling_return=_return_buf,
            metadata=checkpoint_metadata,
            required_backend=device.type,
        )

    t_start = time.time()
    _is_mps = "mps" in str(device).lower()      # release the MPS cache each iter (see loop end)
    iteration_numbers = training_iteration_numbers(
    ...
    for it in iteration_numbers:
        ...
        fresh_batch, rollout_stats = collect_episodes(
            ...
            representation_teacher=(jepa_teacher if cfg.teacher_actor_critic_ste else None),
        )
        n_fresh = fresh_batch.observations.shape[0]

        # PER: sample replay episodes (skipped during burn-in — actor not training yet).
        beta_frac = min(it / max(schedule_final_iteration, 1), 1.0)
        ...
        update_stats = ppo_update(model, optimizer, combined_batch, cfg,
                                  ...
                                  jepa_teacher=jepa_teacher, it=it)
        update_stats["in_burn_in"] = 1.0 if in_burn_in else 0.0

        # PAC target update: EMA every iteration (user spec) or periodic hard-copy.
        if target_model is not None:
            if use_ema:
        ...
        # JEPA teacher EMA — PARAMETERS ONLY. No buffer copy: taus is a constant (correct from
        # deepcopy) and the jepa_center buffer must stay solely on the student (the teacher softmax
        # in ppo_update reads model.jepa_center directly).
        if jepa_teacher is not None:
            dj = float(cfg.jepa_ema_decay)
            with torch.no_grad():
                for tp, p in zip(jepa_teacher.parameters(), model.parameters()):
                    tp.mul_(dj).add_(p.detach(), alpha=1.0 - dj)
        ...
        # Release the MPS caching allocator each iter. On Apple-Silicon unified memory the MPS
        # allocator otherwise accumulates/fragments RAM unboundedly over a long run (worst on the
        # larger 3×3/75px tensors) → the 100GB spikes. No-op on CPU/CUDA.
        if _is_mps:
            torch.mps.empty_cache()

        # append this iteration's row to metrics.csv (flushed)
        if _metrics_path is not None:
            if _metrics_w is None:
                os.makedirs(checkpoint_dir, exist_ok=True)
                append = _resume_metrics_fieldnames is not None
                fieldnames = _resume_metrics_fieldnames or list(log.keys())
                if list(log.keys()) != fieldnames:
                    raise ValueError("resumed metrics schema does not match current trainer schema")
                _metrics_f = open(_metrics_path, "a" if append else "w", newline="")
                _metrics_w = csv.DictWriter(_metrics_f, fieldnames=fieldnames,
                                            extrasaction="ignore")
                if not append:
                    _metrics_w.writeheader()
            _metrics_w.writerow(log)
            try:
                _metrics_f.flush()
            except OSError as exc:
                print(f"[metrics] flush I/O error ({exc}); retrying once", flush=True)
                time.sleep(0.5)
                try:
                    _metrics_f.flush()
                except OSError as exc2:
                    print(f"[metrics] flush failed again ({exc2}); continuing without crash",
                          flush=True)
        ...
        if (it + 1) % log_every == 0:
            ...
            print(
                ...
                f"L_jepa={log.get('loss_jepa', 0.0):.3f}  "
                f"H={-log['loss_entropy']:.3f}  KL={log['approx_kl']:.4f}  "
                f"θ={log['env/theta']:.1f}  "
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            save_training_checkpoint(ckpt_path, _checkpoint_payload(it))
            print(f"[checkpoint] saved to {ckpt_path}")

    if _metrics_f is not None:
        _metrics_f.close()
        print(f"[metrics] per-iteration losses + orientation change → {_metrics_path}")

    if checkpoint_dir is not None and n_iterations > 0:
        os.makedirs(checkpoint_dir, exist_ok=True)
        final_payload = _checkpoint_payload(phase_final_iteration)
        latest_path = os.path.join(checkpoint_dir, "rvit_plus_rl_latest.pt")
        save_training_checkpoint(latest_path, final_payload)
        task = checkpoint_metadata.get("task")
        if task:
            final_path = os.path.join(checkpoint_dir, f"rvit_paper_{task}_final.pt")
            save_training_checkpoint(final_path, final_payload)
            print(f"[checkpoint] saved replay-excluded trainer state to {final_path}")
