...
EXPERIMENT = (
    ...
    / "grid2x2_crossattn1_2layer_softmax_modern_gamma095_seed0_v1"
)


def test_corrected_theta_curriculum_uses_nonoverlapping_subtractive_windows() -> None:
    env = make_env("vda4", curriculum=True, theta=65.0, curr_window=4,
                   curr_threshold=0.75, curr_step=3.0, theta_floor=8.0)
...
def test_registered_experiment_is_gamma_095_modern_softmax_jepa_contract() -> None:
    ...
    required = (
        ...
        "--d-mem 128", "--mem-heads 4", "--gamma 0.95", "--jepa-coef 0.5",
        "--jepa-ema-decay 0.996", "--jepa-sinkhorn-iters 3", "--jepa-var-coef 1.0",
        "--jepa-cov-coef 0.01", "--ema-decay 0.995", "--curriculum",
        "--theta-start 65.0", "--curr-window 1000", "--curr-threshold 0.85",
        "--curr-step 3.0", "--curr-floor 8.0", "--init-mode fresh",
    )
    ...
    assert manifest["training"]["discounting"] == "one-step TD targets use gamma 0.95"
