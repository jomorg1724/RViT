...
DE=[0,3,6,9,12,15,18,22,26,30]; DEf=np.linspace(0,30,120); UNC=C.UNC; VAL=[0.25,0.5,0.75,1.0]
def cued(prop): return np.array([np.mean([C.behavior(m,c,prop,c,float(d),B=N)[0] for c in (0,8)]) for d in DE])
def side(prop,s): return np.array([np.mean([C.behavior(m,c,prop,c if s=="cued" else UNC[c],float(d),B=N)[0] for c in (0,8)]) for d in DE])
...
def a1_vs_delta(chg):
    ...
    return np.array(out)
a1c=a1_vs_delta(0); a1u=a1_vs_delta(8)
