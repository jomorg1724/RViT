...
def build_spatial_figure(
    ...
    figure, axes = plt.subplots(2, 2, figsize=(9.2, 7.6), constrained_layout=True)
    image = None
    for condition in range(2):
        ...
        for column, timestep in enumerate((4, 5)):
            ...
            if column == 0:
                axis.set_ylabel(
                    "cued S1 change" if condition == 0 else "forced S16 change",
                    fontweight="bold",
                )
...
def rerender_from_cache(output_root: Path) -> dict[str, Any]:
    """Regenerate figures from the frozen cache without loading the model."""
    output_root = output_root.resolve()
    cache_path = output_root / "data" / "vda16_cued_vs_opposite_attention.npz"
    summary_path = output_root / "data" / "summary.json"
    with np.load(cache_path, allow_pickle=False) as payload:
        press = np.asarray(payload["press_times"], dtype=np.int64)
        spatial_mass = np.asarray(payload["spatial_attention_mass_trials"], dtype=np.float32)
        magnitudes = np.asarray(payload["change_magnitudes"], dtype=np.float64)
    focal_matches = np.flatnonzero(np.isclose(magnitudes, FOCAL_MAGNITUDE))
    if focal_matches.size != 1:
        raise RuntimeError(f"focal magnitude {FOCAL_MAGNITUDE} is not uniquely present")
    focal_index = int(focal_matches[0])
    figures_dir = output_root / "figures"
    ...
    producer_snapshot = (
        ...
        / "provenance"
        / "source_snapshot"
        / "vda16_cued_vs_opposite_attention.py"
    )
    metadata["data_producer_snapshot_path"] = str(producer_snapshot.resolve())
    metadata["data_producer_snapshot_sha256"] = sha256_file(producer_snapshot)
    metadata["figure_renderer_path"] = str(Path(__file__).resolve())
    metadata["figure_renderer_sha256"] = sha256_file(Path(__file__).resolve())
    metadata["figure_outputs"] = {
        "summary": {suffix: str(path.resolve()) for suffix, path in summary_outputs.items()},
        "spatial_maps": {suffix: str(path.resolve()) for suffix, path in spatial_outputs.items()},
    }
    ...
    manifest = {
        "schema_version": 1,
        "data_producer": str(producer_snapshot.resolve()),
        "figure_renderer": str(Path(__file__).resolve()),
        "artifacts": [
            {
                "path": str(path.resolve()),
                "sha256": sha256_file(path),
    ...
    manifest_path = output_root / "MANIFEST.json"
    manifest_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"rerendered {output_root}")
    return metadata
...
if __name__ == "__main__":
    ...
    if arguments.render_only:
        rerender_from_cache(arguments.output_root)
    else:
        if arguments.checkpoint is None:
            raise SystemExit("--checkpoint is required unless --render-only is used")
