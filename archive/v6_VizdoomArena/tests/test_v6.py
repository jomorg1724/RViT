"""
Unit + smoke tests for V6 (shapes, identities, trainer math, end-to-end).

    .venv/bin/python -m v6_VizdoomArena.tests.test_v6
...
from v6_VizdoomArena.env import FEAT_DIM, FEAT_GROUPS, N_ACTIONS
from v6_VizdoomArena.model import V6ArenaModel
from v6_VizdoomArena.patch_embed import PatchEmbed
from v6_VizdoomArena.trainer import (
...
def _tiny_model(**over) -> V6ArenaModel:
    kw = dict(
...
def test_encoder_step():
    ...
    assert len(Hs) == 2 and Hs[0].shape == (B, 48, 32)
    assert len(attn) == 2
    n_keys = (1 + 2) * 48 + 3
    assert enc.n_keys == n_keys
    assert attn[0].shape == (B, 4, 48, n_keys)
...
def test_attn_bias():
    ...
    with torch.no_grad():
        ...
        zeros = [torch.zeros(48, enc.n_keys), None]
...
def test_decoders():
    m = _tiny_model().eval()
    B = 3
    rec = [torch.randn(B, 48, 32) for _ in range(2)]
    with torch.no_grad():
        ...
        assert attns[0].shape == (B, 4, 1 + 96, 1 + 96)
        assert m.actor_head.token_layout()["H2"] == (49, 97)
    _ok("decoder shapes / derive_V / decoder attention")
...
# ── 8. PER buffer ────────────────────────────────────────────────────────────
def _fake_batch(model: V6ArenaModel, B: int, T: int) -> SegmentBatch:
    iH, iC = V6ArenaModel.states_to_tensors(model.init_states(B))
...
def test_env_and_train_smoke():
    try:
        ...
        return
    from v6_VizdoomArena.env import VizdoomArenaEnv
    from v6_VizdoomArena.trainer import train
...
if __name__ == "__main__":
    for fn in (test_patch_embed, test_encoder_step, test_attn_bias, test_decoders,
               test_step_vs_segment, test_done_reset, test_nstep_targets,
