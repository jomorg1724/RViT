...
class RViTPlusModel(nn.Module):
    ...
    def __init__(
        ...
        # Predictive-coding auxiliary head kind (run-19):
        #   "cosine"        → legacy: predict C₂ₜ, loss = 1 − cos.
        #   "discrete_jepa" → discrete categorical latent JEPA (DreamerV3-style):
        #                     predict a per-position categorical CODE of C₂ₜ
        #                     (pc_groups × pc_classes) via cross-entropy. No
        #                     pixel target. See ppo._discrete_jepa_loss.
        pc_kind: str = "cosine",
        pc_groups: int = 8,                  # K categorical groups (discrete_jepa)
        pc_classes: int = 16,                # V classes per group (discrete_jepa)
    ) -> None:
        super().__init__()
        ...
        self.pc_kind = str(pc_kind)
        self.pc_groups = int(pc_groups)
        self.pc_classes = int(pc_classes)
        ...
        c3 = state_channels[2]
        # ── Predictive-coding / discrete-JEPA heads ─────────────────────
        # For each branch X ∈ {actor, critic}: a small upsample+refine
        # predictor P_X mapping C_{3, X} (B, c3, h3, w3) at t-1 to a
        # prediction of the next-step shared middle state C_2.
        #   • pc_kind="cosine": P_X outputs a C₂-shaped tensor (c_out = c2);
        #     the PC loss is 1 − cos(P_X, C₂ₜ.detach()).
        #   • pc_kind="discrete_jepa": P_X outputs per-position logits over a
        #     categorical code (c_out = pc_groups·pc_classes), and a tokenizer
        #     (Conv1×1) quantizes C₂ₜ into the target code; the loss is
        #     cross-entropy / KL in code space (see ppo._discrete_jepa_loss).
        # PC's signal is a temporal-consistency constraint on encoder dynamics
        # — independent of (action, reward) labels — so it stays useful even
        # when the actor collapses.
        c2 = state_channels[1]
        h2, w2 = self.encoder.GRID_HW[1]
        h3, w3 = self.encoder.GRID_HW[2]
        # Three modes:
        #   "cosine"         → predictor c_out = c2 (continuous C₂ prediction).
        #   "discrete_jepa"  → predictor c_out = K·V; per-branch tokenizer
        #                      quantizes C₂ₜ as a stop-grad CODE TARGET (the code
        #                      is NOT consumed by the agent).
        #   "discrete_latent"→ predictor c_out = K·V; ONE SHARED tokenizer +
        #                      codebook turn C₂ₜ into a hard one-hot latent z
        #                      (straight-through) that the actor/critic READ in
        #                      place of C₂ (see _route_states_to_heads). Policy +
        #                      value gradients flow through z (STE) → the latent
        #                      is anchored by reward, à la DreamerV3. The per-
        #                      branch predictors P_X(C₃ₜ₋₁) become the dynamics
        #                      model that predicts the next z (the JEPA loss).
        _discrete_target = (self.pc_kind == "discrete_jepa")
        _discrete_latent = (self.pc_kind == "discrete_latent")
        _discrete = _discrete_target or _discrete_latent
        KV = self.pc_groups * self.pc_classes
        pred_c_out = KV if _discrete else c2
        self.pc_actor_head: Optional[PredictiveCodingHead] = None
        self.pc_critic_head: Optional[PredictiveCodingHead] = None
        # discrete_jepa: per-branch tokenizers (target-only, isolated branches).
        self.pc_actor_tokenizer: Optional[nn.Conv2d] = None
        self.pc_critic_tokenizer: Optional[nn.Conv2d] = None
        # discrete_latent: ONE shared tokenizer (C₂ → code logits) + codebook
        # (one-hot code → c2-shaped embedding the heads read in the C₂ slot).
        self.pc_tokenizer: Optional[nn.Conv2d] = None
        self.pc_embed: Optional[nn.Conv2d] = None
        if _discrete_latent:
            self.pc_tokenizer = nn.Conv2d(c2, KV, kernel_size=1)
            self.pc_embed = nn.Conv2d(KV, c2, kernel_size=1)   # codebook: one-hot → c2
        if self.enable_actor:
            self.pc_actor_head = PredictiveCodingHead(
                c_in=c3, c_out=pred_c_out, spatial_in=(h3, w3), spatial_out=(h2, w2),
            )
            if _discrete_target:
                self.pc_actor_tokenizer = nn.Conv2d(c2, pred_c_out, kernel_size=1)
        if self.enable_critic:
            self.pc_critic_head = PredictiveCodingHead(
                c_in=c3, c_out=pred_c_out, spatial_in=(h3, w3), spatial_out=(h2, w2),
            )
            if _discrete_target:
                self.pc_critic_tokenizer = nn.Conv2d(c2, pred_c_out, kernel_size=1)
    ...
    def _discrete_latent_embed(self, C2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """discrete_latent: turn the shared middle state C₂ into a hard one-hot
        categorical latent z (DreamerV3-style) and embed it back to C₂'s shape.

        Returns (z_embed, post_logits):
            z_embed     : (B, c2, h2, w2) — codebook embedding of the one-hot z,
                          what the actor/critic read in place of C₂.
            post_logits : (B, K·V, h2, w2) — pre-softmax code logits (returned
                          for callers that want to cache the posterior).

        Straight-through: forward uses the argmax one-hot (a true binary code
        matrix), backward uses the softmax gradient, so policy/value gradients
        flow through z into the tokenizer + encoder (the latent is anchored by
        reward). argmax (not sampling) keeps z a deterministic function of the
        observation, so the PAC `old_log_probs` stay consistent on re-encode.
        """
        K, V = self.pc_groups, self.pc_classes
        logits = self.pc_tokenizer(C2)                          # (B, K·V, h, w)
        B, _, Hh, Ww = logits.shape
        logits_g = logits.view(B, K, V, Hh, Ww)
        probs = torch.softmax(logits_g, dim=2)
        idx = probs.argmax(dim=2, keepdim=True)                 # (B, K, 1, h, w)
        onehot = torch.zeros_like(probs).scatter_(2, idx, 1.0)
        z = (onehot + probs - probs.detach()).view(B, K * V, Hh, Ww)   # STE
        z_embed = self.pc_embed(z)                              # (B, c2, h, w)
        return z_embed, logits
    ...
    def _route_states_to_heads(
        ...
        discrete_latent: the C₂ the heads read is REPLACED by the embedded
        discrete latent embed(z) (shared across both heads). Injecting it here
        means every head call site — rollout `rl_step`, `forward_rl_sequence`,
        and analysis `forward_rl` — sees the same latent, so the acting policy
        and the trained policy never diverge.
        """
        C1, C2, C3 = final_states[0], final_states[1], final_states[2]
        head_C2 = C2
        if self.pc_kind == "discrete_latent" and self.pc_tokenizer is not None:
            head_C2, _ = self._discrete_latent_embed(C2)
        if not self.split_c3:
            s = (C1, head_C2, C3)
            return s, s
        return (
            (C1, head_C2, c3_specialists["actor"]),
            (C1, head_C2, c3_specialists["critic"]),
        )
