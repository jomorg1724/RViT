...
def _common_training_args(
    *,
    ...
    return [
        ...
        "--T", "7",
        "--patch-grid-rows", str(int(patch_grid_rows)),
        "--patch-grid-cols", str(int(patch_grid_cols)),
        "--cell", "xlstm",
        "--feedback", feedback,
        "--memory-decay", str(float(memory_decay)),
        "--memory-noise-std", str(float(memory_noise_std)),
...
def build_cells(
    *,
    ...
    d_mem: int = 128,
    noise: float = 5.0,
    patch_grid_rows: int = 2,
    patch_grid_cols: int = 2,
) -> list[ExperimentCell]:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--memory-noise-std", type=float, default=0.0)
    parser.add_argument("--d-mem", type=int, default=128)
    parser.add_argument("--patch-grid-rows", type=int, default=2)
    parser.add_argument("--patch-grid-cols", type=int, default=2)
    parser.add_argument(
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if not math.isfinite(args.memory_noise_std) or args.memory_noise_std < 0.0:
        raise ValueError("--memory-noise-std must be finite and nonnegative")
    if args.d_mem <= 0:
        raise ValueError("--d-mem must be positive")
    if args.patch_grid_rows <= 0 or args.patch_grid_cols <= 0:
        raise ValueError("--patch-grid-rows and --patch-grid-cols must be positive")
...
def evaluate_parent_gate_metrics(
    ...
    *,
    ...
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    environment_config = checkpoint.get("environment_state", {}).get(
        "environment_config", {}
    )
    task = str(checkpoint.get("task", ""))
    orientation_sampling = environment_config.get("orientation_sampling")
    noise_multiplier = environment_config.get("noise_multiplier")
    spatial_grid_size = environment_config.get("spatial_grid_size")
    if task not in ("luo2015_criterion", "luo2015_sensitivity"):
        raise RuntimeError("neutral parent checkpoint has no Luo task contract")
    if orientation_sampling != "independent_uniform_axial_0_180":
        raise RuntimeError("neutral parent checkpoint uses an invalid initial-orientation contract")
    if noise_multiplier is None or spatial_grid_size is None:
        raise RuntimeError("neutral parent checkpoint has incomplete sensory configuration")
...
def _new_manifest(args: argparse.Namespace, cells: list[ExperimentCell]) -> dict:
    return {
        ...
        "protocol": {
            ...
            "memory_noise_std": float(args.memory_noise_std),
            "d_mem": int(args.d_mem),
            "model_patch_grid": [int(args.patch_grid_rows), int(args.patch_grid_cols)],
            "task_image_geometry": "unchanged Luo 50x50 scene",
...
def main(argv: list[str] | None = None, *, runner=subprocess.run) -> int:
    ...
    cells = build_cells(
        ...
        d_mem=args.d_mem,
        noise=args.noise,
        patch_grid_rows=args.patch_grid_rows,
        patch_grid_cols=args.patch_grid_cols,
    )
