...
and the forced-trial mechanism are IDENTICAL; the v5-specific pieces are:
  * build_model     → constructs RViTPlusV5Part2Model from the v5 config
...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_v8_part2", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusV5Part2Model:
    """Construct the v5_part2 model exactly as train_rl.py does, from the config."""
    m = cfg["model"]
    rl = m["rl"]
...
def load_checkpoint(model: RViTPlusV5Part2Model, ckpt_path: str, device: torch.device) -> int:
...
def batched_attention_rollout(
    ...
    """Run B trials forced to wait; per layer, take the cross-attention
    (B, heads, N, 2N), sum attention to H1+H2 keys at each patch-aligned
    memory position, and reduce to a PER-HEAD per-patch saliency — mean over
    QUERIES only (heads can attend to opposite locations, so head-averaging
    cancels real structure), reshaped to (grid_h, grid_w) and averaged over trials.
    ...
    while t <= T and not done.all():
        ...
        for L in range(n_layers):
            aw = attn[L]                                  # (B, heads, N, 2N)
            # H1 keys at 0:N, H2 at N:2N — both patch-position-aligned via mem_pos_emb
            mem_keys = aw[:, :, :, :2 * N]                # all keys are memory
            sal = mem_keys.mean(dim=2)                    # mean over QUERIES → (B, heads, 2N)
            # sum H1+H2 attention at each patch position
            sal_pos = sal[:, :, :N] + sal[:, :, N:2 * N]  # (B, heads, N)
            grid = sal_pos.view(B, sal_pos.shape[1], gh, gw)
