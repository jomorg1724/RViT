"""
RViTPlusV5Part2Model (v8 build) — conv-free, RL-only recurrent attention model with the H1-RESIDUAL encoder: the cross-attention skip path carries the previous frame's H1 memory instead of the visual input X, so current-frame visual content moves downstream ONLY via attended values (see tx_lstm_encoder.py).
