...
DE=[0,3,6,9,12,15,18,22,26,30]; DEf=np.linspace(0,30,120); UNC={0:3,3:0}
def curve_cued(prop):
    R=[]
    for d in DE:
        rr=[C.behavior(m,cue,prop,cue,float(d),B=N)[0] for cue in (0,3)]; R.append(np.mean(rr))
    return np.array(R)
...
axD=fig.add_subplot(gs[0,4]); axD.plot(DE,aC,"o-",label=r"$\alpha$ on $S_1$(cued)",color="tab:green"); axD.plot(DE,aU,"s-",label=r"$\alpha$ on $S_4$(change)",color="tab:purple")
