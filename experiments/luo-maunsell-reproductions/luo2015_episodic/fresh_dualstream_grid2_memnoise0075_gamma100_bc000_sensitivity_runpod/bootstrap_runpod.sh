...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON_BIN" -m pytest -q \
 ...
 tests/test_luo2015_dualstream_sensitivity_runpod.py tests/test_luo2015_protocol.py \
 tests/test_luo2015_scientific_assay.py tests/test_luo2015_attention_allocation.py \
 tests/test_training_checkpoint.py
CANARY="/workspace/canary_luo_dualstream_${CELL}"; rm -rf "$CANARY"
env CELL="$CELL" RUN_ROOT="$CANARY" PYTHON_BIN="$PYTHON_BIN" ITERS=1 START_ITERATION=20 SAVE_EVERY=1 DEVICE=cuda bash "$EXPERIMENT/launch_cell.sh" > "/workspace/canary_dualstream_${CELL}.log" 2>&1
...
assert math.isclose(float(a['noise']),5.0); assert a['curriculum'] is False
...
with open(os.path.join(os.environ['CANARY'],'metrics.csv'),newline='') as f:
...
printf '{"source_archive_sha256":"%s","cell":"%s","grid":[2,2],"dual_actor_critic_streams":true,"independent_jepa_branches":["actor","critic"],"jepa_coef_per_branch":0.5,"bc_alpha":0.0,"memory_noise_std":0.075,"sensory_noise_std":5.0,"gamma":1.0,"initialization":"fresh","canary_checkpoint_sha256":"%s"}\n' "$ACTUAL_SHA" "$CELL" "$CANARY_SHA" > "$RUN_ROOT/deployment_contract.json"
(
 set +e
...
printf 'BOOTSTRAP_OK cell=%s dual_streams=true bc_alpha=0.0 gamma=1.0 memory_noise=0.075 source_sha256=%s canary_sha256=%s pid=%s root=%s\n' "$CELL" "$ACTUAL_SHA" "$CANARY_SHA" "$pid" "$RUN_ROOT"
