...
def regular_run_files(root: Path) -> set[Path]:
    """Inventory lexical regular files and reject symlinks or special entries."""
...
def tree_identity(root: Path) -> tuple[tuple[str, int, str], ...]:
    """Return a framed identity for every regular file, including the manifest."""
    root = Path(os.path.abspath(root))
    return tuple(
        (str(path.relative_to(root)), path.stat().st_size, sha256(path))
        for path in sorted(regular_run_files(root))
    )


def require_exact_file_inventory(root: Path, expected_files: set[Path]) -> None:
    """Reject every missing or unmanifested file in a prospective run tree."""
    root = Path(os.path.abspath(root))
    expected = {Path(os.path.abspath(path)) for path in expected_files}
    for path in expected:
        if root not in path.parents:
            raise RuntimeError(f"prospective file escapes the run root: {path}")
    actual = regular_run_files(root)
    if actual != expected:
        missing = sorted(str(path) for path in expected - actual)
        unmanifested = sorted(str(path) for path in actual - expected)
        raise RuntimeError(
...
def publish_manifest_with_inventory(
    ...
    """Atomically publish only an exact regular-file tree; roll back on post-check failure."""
    require_exact_file_inventory(root, expected_without_manifest | {manifest_temporary})
    manifest_temporary.replace(manifest_path)
    try:
        require_exact_file_inventory(root, expected_without_manifest | {manifest_path})
    except Exception:
...
def lexical_output_root(path: str | Path) -> Path:
    """Normalize syntax without dereferencing the output-root boundary."""
...
def audit_existing_build(root: Path, args: argparse.Namespace) -> Path:
    ...
    root = root.resolve()
    actual_files = regular_run_files(root)
    ...
    if manifest.get("schema_version") != 4:
        raise ValueError("existing build does not use manifest schema version 4")
    ...
    expected_artifact_keys.update({
        ("attention", task, "crossattn1", f"{source_slug}_{kind}")
        for task in args.tasks
        if "crossattn1" in args.feedbacks
        for source_slug in ("image_keys", "recurrent_memory_keys")
    ...
    expected_files = artifact_paths | provenance_snapshot_paths | {manifest_path}
...
def main() -> int:
    ...
    root = lexical_output_root(args.output_root)
    ...
    for task in args.tasks:
        ...
        for feedback in args.feedbacks:
            ...
            if feedback == "crossattn1":
                attention_metadata = json.loads(attention.metadata.read_text(encoding="utf-8"))
                source_panel_outputs = attention_metadata.get("source_panel_outputs")
                if not isinstance(source_panel_outputs, dict):
                    raise RuntimeError("cross-attention metadata lacks dedicated source-panel outputs")
                for source_slug, formats in source_panel_outputs.items():
                    if source_slug not in {"image_keys", "recurrent_memory_keys"}:
                        raise RuntimeError(f"unexpected cross-attention source panel {source_slug!r}")
                    if not isinstance(formats, dict) or set(formats) != {"pdf", "svg", "png"}:
                        raise RuntimeError(f"incomplete cross-attention formats for {source_slug}")
                    for kind, filename in formats.items():
                        path = attention_figures / str(filename)
                        artifacts.append({
                            "stage": "attention",
                            "task": task,
                            "feedback": feedback,
                            "kind": f"{source_slug}_{kind}",
                            "path": str(path),
                            "sha256": sha256(path),
                        })
    ...
    manifest = {
        ...
        "attention_condition": (
            "red cue at fixed S1; no-change trials only; rows=displayed cue proportions; "
            "columns=logical timesteps; query patches averaged"
        ),
        "cross_attention_arrays": ["image keys", "recurrent-memory keys"],
        "psychometric_panels": [
    ...
    frozen_snapshots = {
        producer_snapshot,
        build_script_snapshot,
        *(Path(record["snapshot"]) for record in dependency_snapshots),
    }
    manifested_artifacts = {Path(str(record["path"])) for record in artifacts}
    prospective_files = manifested_artifacts | frozen_snapshots | {manifest_temporary}
    require_exact_file_inventory(root, prospective_files)
    ...
    publish_manifest_with_inventory(
        root,
        manifest_temporary,
        manifest_path,
        manifested_artifacts | frozen_snapshots,
    )
