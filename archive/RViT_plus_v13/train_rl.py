...
Usage:
    .venv/bin/python RViT_plus_v13/train_rl.py
    .venv/bin/python RViT_plus_v13/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v13.config.loader import (
...
from RViT_plus_v13.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--pc-checkpoint", default=cfg_get(cfg, "run.pc_checkpoint", None),
                   help="PHASE 1 predictive-coding pretrained model (from pretrain_pc.py). "
                        "Loaded then FROZEN; only the energy stream + heads train. THE experiment.")
    ...
    model_kwargs = dict(
        ...
        d_latent=cfg_get(cfg, "model.d_latent", args.d_model),
    )
    model = RViTPlusV13Model(**model_kwargs).to(device)
    ...
    # ── PHASE 2: load the predictive-coding pretrained encoder, then FREEZE it ──
    # This is the v13 experiment: a frozen self-supervised encoder repurposed for the
    # RL task purely by the energy stream. Only PART B (LSTM3 + T2) + the heads train.
    if args.pc_checkpoint:
        if not os.path.exists(args.pc_checkpoint):
            raise FileNotFoundError(f"--pc-checkpoint '{args.pc_checkpoint}' not found. "
                                    f"Run pretrain_pc.py first (Phase 1).")
        ck = torch.load(args.pc_checkpoint, map_location=device, weights_only=False)
        info = model.load_state_dict(ck["model_state_dict"], strict=False)
        n_frozen = model.freeze_pc()
        n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"[phase2] loaded PC-pretrained encoder from {args.pc_checkpoint} "
              f"(iter={ck.get('iter')}); FROZE PART A ({n_frozen:,} params); "
              f"use_energy=ON; trainable (energy + heads) = {n_train:,}")
        if info.missing_keys or info.unexpected_keys:
            print(f"[phase2] load: missing={list(info.missing_keys)[:3]} unexpected={list(info.unexpected_keys)[:3]}")
    else:
        print("[phase2] WARNING: no --pc-checkpoint → training the WHOLE model from scratch "
              "(NOT the frozen-encoder experiment). Run pretrain_pc.py and pass --pc-checkpoint.")
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v13_rl_final.pt")
