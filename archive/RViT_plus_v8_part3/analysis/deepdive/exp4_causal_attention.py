...
Usage:
  .venv/bin/python -m RViT_plus_v8.analysis.deepdive.exp4_causal_attention \
...
def _region_bias(model, device, *, region, value, layer=0, head=None, quad=None):
    """Build an attention-bias tensor (tx_layers, n_heads, 3N).
    ...
    bias = torch.zeros(nL, nH, (1 + enc.n_lstm) * model.n_tokens, device=device)
    for h in range(nH):
