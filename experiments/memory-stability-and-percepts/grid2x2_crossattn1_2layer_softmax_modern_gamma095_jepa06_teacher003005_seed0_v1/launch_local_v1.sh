...
cp "$PROJECT_ROOT/tests/test_modern_transformer_memory.py" "$RUN_ROOT/provenance/source_snapshot/tests/test_modern_transformer_memory.py"
cp "$PROJECT_ROOT/tests/test_modern_jepa_sharpening_experiment_contract.py" "$RUN_ROOT/provenance/source_snapshot/tests/test_modern_jepa_sharpening_experiment_contract.py"
cp "$PROJECT_ROOT/tests/test_loss_weight_hierarchy.py" "$RUN_ROOT/provenance/source_snapshot/tests/test_loss_weight_hierarchy.py"
sha256sum \
  ...
  "$PROJECT_ROOT/tests/test_modern_jepa_sharpening_experiment_contract.py" \
  "$PROJECT_ROOT/tests/test_loss_weight_hierarchy.py" \
...
{
  printf 'experiment=vda4_modern_gamma095_jepa06_teacher003005_seed0_v1\n'
  printf 'hypothesis=slightly_stronger_JEPA_plus_sharper_teacher_targets_improves_task_relevant_H1_H2_structure\n'
  printf 'controlled_delta=jepa_coef_0.5_to_0.6__teacher_tau_0.04-0.07_to_0.03-0.05\n'
  printf 'objective_hierarchy=actor_0.01_lt_critic_0.5_lt_jepa_0.6__bc_alpha_0.0\n'
  printf 'run_root=%s\n' "$RUN_ROOT"
  printf 'iterations=%s\n' "$ITERS"
