...
for i,nm in enumerate(["B  $S_1$ changes","C  $S_2$ changes","D  $S_3$ changes","E  $S_4$ changes"]):
    mags=np.random.default_rng(20+i).uniform(1,60,N)            # all change trials at this location, uniform Δ
    Vi=C._tens([C.make_video(0,1.0,"red",1,i,float(mg)) for mg in mags])
