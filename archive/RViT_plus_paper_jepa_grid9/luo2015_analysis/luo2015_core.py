...
Protocol (from the vetted design review):
  * curriculum OFF; env.theta is set directly to the fixed near-threshold |Δ| magnitude.
...
def _env(task, mag):
    e = make_env(task, T=T, frame_repeat=1, curriculum=False, theta=float(mag))
    return e
...
def gen_trials(task, mag, change_true, B, test_loc=None):
    e = _env(task, mag); vids, tlocs = [], []
    for _ in range(B):
...
def press_times(m, V):
    with torch.no_grad():
        act = m.forward_rl_sequence(V)["actor_logits_seq"].argmax(-1).cpu().numpy()   # (B,T)
...
def sdt(m, task, mag, B=400):
    ...
    Vc, tlc = gen_trials(task, mag, 1, B)          # change trials
    Vn, tln = gen_trials(task, mag, 0, B)          # no-change trials
    pc, pn = press_times(m, Vc), press_times(m, Vn)
    out = summarize_sdt(pc, pn, tlc, tln)
    out.update({"mag": mag, "B": B})
    return out
