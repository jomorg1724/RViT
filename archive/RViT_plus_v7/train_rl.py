...
Usage:
    .venv/bin/python RViT_plus_v7/train_rl.py
    .venv/bin/python RViT_plus_v7/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v7.config.loader import (
...
from RViT_plus_v7.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    # ── Environment ──
    p.add_argument("--distractor-prob", type=float,
                   default=cfg_get(cfg, "environment.distractor_prob", 0.5),
                   help="Probability that the UNCUED side also changes (a distractor "
                        "the agent must ignore). 0 disables distractors.")
    ...
    print_resolved_config(cfg, used_keys=[
        ...
        "environment.min_change_time", "environment.max_change_time", "environment.distractor_prob",
    ...
    model = RViTPlusV7Model(**model_kwargs).to(device)
    ...
    env = ChangeDetectionEnv(
        min_change_time=int(cfg_get(cfg, "environment.min_change_time", 11)),
        max_change_time=int(cfg_get(cfg, "environment.max_change_time", 25)),
        distractor_prob=args.distractor_prob,
    )
    print(f"[setup] env: change window [{env.min_change_time},{env.max_change_time}], "
          f"distractor_prob={env.distractor_prob} (uncued-side distractor changes)")
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v7_rl_final.pt")
