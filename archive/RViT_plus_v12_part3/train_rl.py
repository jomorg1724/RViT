...
Usage:
    .venv/bin/python RViT_plus_v12_part3/train_rl.py
    .venv/bin/python RViT_plus_v12_part3/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v12_part3.config.loader import (
...
from RViT_plus_v12_part3.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--n-lstm", type=int, default=cfg_get(cfg, "model.n_lstm", 1),
                   help="Config parity — the architecture is fixed: LSTM2 (readout) + LSTM3 (write stream).")
    p.add_argument("--temperature", type=float, default=cfg_get(cfg, "model.temperature", 1.0),
                   help="Softmax temperature τ for the readout attention over the codebook (lower=sharper).")
    ...
    p.add_argument("--gate-coef", type=float, default=cfg_get(cfg, "ppo.gate_coef", 1.0e-3),
                   help="Weight on the write-gate openness penalty mean(gate) (warmed in) — the 'energy cost' "
                        "of writing. Higher → gate closes harder once content has consolidated. 0 disables.")
    p.add_argument("--gate-warmup-iters", type=int, default=cfg_get(cfg, "ppo.gate_warmup_iters", 2000),
                   help="Gate penalty is silent for this many iters (lets the write flow to ground the readout).")
    p.add_argument("--gate-ramp-end-iter", type=int, default=cfg_get(cfg, "ppo.gate_ramp_end_iter", 4000),
                   help="Gate penalty ramps from 0 (at warmup) to gate_coef by this global iter.")
    p.add_argument("--codebook-ema-rate", type=float, default=cfg_get(cfg, "ppo.codebook_ema_rate", 2.0e-3),
                   help="VQ-VAE EMA rate r for the codebook (a buffer, NOT gradient-learned): "
                        "C_base ← (1−r)·C_base + r·mean(T_t) toward attended content, once per update. 0 freezes it.")
    ...
    print_resolved_config(cfg, used_keys=[
        ...
        "model.tx_heads", "model.tx_layers", "model.n_lstm", "model.codebook_decay", "model.temperature",
        ...
        "ppo.value_coef", "ppo.entropy_coef", "ppo.gate_coef", "ppo.gate_warmup_iters",
        "ppo.gate_ramp_end_iter", "ppo.codebook_ema_rate", "ppo.qr_kappa",
    ...
    model = RViTPlusV12Part3Model(**model_kwargs).to(device)
    ...
    print(f"[diag] d_model={args.d_model} d_mem={args.d_mem} tx[{args.tx_heads}h×{args.tx_layers}L] "
          f"+ LSTM2(readout) + LSTM3(write stream, V=image) + gated write[gate_coef={args.gate_coef:g}"
          f"(warm {args.gate_warmup_iters}→{args.gate_ramp_end_iter})] "
          f"+ EMA codebook[r={args.codebook_ema_rate:g} toward C_t (buffer, no grad)] "
          f"→ Conv1d head[{args.n_conv_layers}×{args.conv_channels}ch k{args.conv_kernel}]")
    print(f"[diag] actor init bias: {list(args.init_action_bias)}")
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[diag] total params: {n_params:,}")

    # ── Checkpoint load ──
    resume_iter = 0   # global iteration to continue FROM (0 unless strict-resuming)
    if args.init_mode == "fresh":
        ...
        if strict:
            resume_iter = int(info.get("ckpt_iter", 0) or 0) + 1   # continue AFTER the saved iter (skips re-burn-in)
            print(f"[init] RESUME (strict) from {args.checkpoint_path} "
                  f"(iter={info['ckpt_iter']}, {info['loaded']} tensors) → continuing at global iter {resume_iter}")
        else:
    ...
    ppo_cfg = PPOConfig(
        ...
        gate_coef=args.gate_coef, gate_warmup_iters=args.gate_warmup_iters,
        gate_ramp_end_iter=args.gate_ramp_end_iter,
        codebook_ema_rate=args.codebook_ema_rate,
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v12_part3_rl_final.pt")
    torch.save({"iter": resume_iter + args.iters - 1, "model_state_dict": model.state_dict(),
