...
\textbf{Primary result} & Suppressing the future-tested sample region produced 0 discordant correctness outcomes among n=704 paired trials (observed Δ accuracy +0.000). Because the ordinary paired bootstrap is degenerate when every observed pair agrees, we report the conservative two-sided 95\% no-discordance bound |Δ accuracy| < 0.0052 under IID trial-pair sampling.\\
\textbf{Run} & \texttt{luo2015\_scientific\_assay\_20260803\_production}.\\\bottomrule
\end{tabularx}
\vfill{\color{Blue}\rule{\textwidth}{2.2pt}}
\end{titlepage}

\begin{abstract}
We evaluated one frozen recurrent vision-transformer policy in a seven-frame Luo--Maunsell-inspired change-detection task. Attention maps average 64 repeats of each exactly fixed latent condition while sensory, mnemonic, and policy-sampling noise vary. A causal routing assay suppressed the future-tested sample quadrant only during sample presentation, with equal-area suppression of the other active sample and a blank quadrant as spatial controls. Suppressing the future-tested sample region produced 0 discordant correctness outcomes among n=704 paired trials (observed Δ accuracy +0.000). Because the ordinary paired bootstrap is degenerate when every observed pair agrees, we report the conservative two-sided 95\% no-discordance bound |Δ accuracy| < 0.0052 under IID trial-pair sampling. The clamp changed mean sample-frame mass from 0.2342 to 0.0011. Fixed-magnitude psychometric and chronometric curves characterize the behavioral regime. Results concern one mid-training checkpoint and do not estimate training-seed uncertainty.
\end{abstract}
...
Natural & 476/704 & 0.676 & 0.2342 & reference & 0.984 & 3.443 \\
Inhibit future-tested sample & 476/704 & 0.676 & 0.0011 & +0.000; $|\Delta|<0.0052$ & 0.984 & 3.443 \\
Inhibit other active sample & 476/704 & 0.676 & 0.0012 & +0.000; $|\Delta|<0.0052$ & 0.984 & 3.443 \\
Inhibit blank control & 476/704 & 0.676 & 0.0009 & +0.000; $|\Delta|<0.0052$ & 0.984 & 3.443 \\
\bottomrule\end{tabular}
\end{center}

Suppressing the future-tested sample region produced 0 discordant correctness outcomes among n=704 paired trials (observed Δ accuracy +0.000). Because the ordinary paired bootstrap is degenerate when every observed pair agrees, we report the conservative two-sided 95\% no-discordance bound |Δ accuracy| < 0.0052 under IID trial-pair sampling. The other-active-sample and blank-quadrant interventions are equal-area controls under the same logit bias and common trial/noise schedule. Confidence intervals are paired over evaluation trials.
