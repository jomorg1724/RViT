...
class ContrastiveProjectionHead(nn.Module):
    """Projection head for contrastive losses on a single specialist C₃ state.

    Takes ONE spatial tensor `C ∈ (B, c, H, W)` — the actor's or critic's own
    `cell3_X` specialist state — and produces an L2-normalised embedding
    `z ∈ (B, projection_dim)` for cosine-similarity contrastive losses.

    Restricting input to the unique specialist (rather than the full
    `(C₁, C₂, C₃)` triplet) keeps the actor and critic contrastive losses
    from putting conflicting pressure on the shared C₁ / C₂ layers — those
    are shaped only by PPO + QH + recon.

    Architecture:
        Conv3x3 (c → c) → GroupNorm → GELU →
        Conv3x3 (c → c // 2) → GroupNorm → GELU →
        GAP → flatten → Linear → GELU → Linear(→ projection_dim) →
        L2-normalise.

    Args
    ----
    in_channels      : c — channel count of the specialist C₃ state.
    projection_dim   : output embedding size (default 128, SimCLR-style).
    hidden           : MLP hidden dim (default 256).
    """
    ...
    def __init__(
        ...
        self.reducer = nn.Sequential(
            nn.Conv2d(c_in, c_in, kernel_size=3, padding=1, bias=True),
            nn.GroupNorm(_gn_groups(c_in), c_in),
            nn.GELU(),
            nn.Conv2d(c_in, c_mid, kernel_size=3, padding=1, bias=True),
            nn.GroupNorm(_gn_groups(c_mid), c_mid),
            nn.GELU(),
            nn.AdaptiveAvgPool2d(1),       # → (B, c_mid, 1, 1)
            nn.Flatten(),                   # → (B, c_mid)
        )
        self.mlp = nn.Sequential(
            nn.Linear(c_mid, hidden),
            nn.GELU(),
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        h = self.reducer(state)                         # (B, c_mid)
        z = self.mlp(h)                                  # (B, projection_dim)
        z = F.normalize(z, dim=-1)                       # L2-normalise for cosine
        return z
