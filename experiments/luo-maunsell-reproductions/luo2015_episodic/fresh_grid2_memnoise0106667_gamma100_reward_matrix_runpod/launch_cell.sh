...
cmd=("$PYTHON_BIN" -u train_rl.py --T 7
 ...
 --memory-decay 1.0 --memory-noise-std 0.10666666666666666 --noise 5.0 --conv-frontend
 --jepa-coef 0.5 --d-mem 32 --episodes-per-iter 8 --gamma 1.0
 ...
 --experiment-launcher experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma100_reward_matrix_runpod/launch_cell.sh
...
mkdir -p "$RUN_ROOT"
cp experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma100_reward_matrix_runpod/experiment_manifest.json "$RUN_ROOT/experiment_manifest.json"
printf 'cell=%s\ngamma=1.0\ninitialization=fresh\ntask=%s\ncondition_loc=%s\nreward_scale=%s\nmemory_noise_std=0.10666666666666666\nsensory_noise_std=5.0\ntheta=65.0\ncurriculum=false\nrun_root=%s\n' "$CELL" "$TASK" "$HIGH_LOC" "$REWARD_SCALE" "$RUN_ROOT" > "$RUN_ROOT/launch_contract.txt"
exec "${cmd[@]}"
