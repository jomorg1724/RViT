...
NEW = ROOT / "experiments" / "luo2015_episodic" / "fresh_grid2_memnoise016_gamma080_reward_matrix_runpod"
PARENT = ROOT / "experiments" / "luo2015_episodic" / "fresh_grid2_memnoise032_gamma_ladder_runpod"
CELLS = {
...
def command(script: Path, cell: str, root: str, parent: bool = False) -> list[str]:
    env = os.environ.copy()
...
def test_manifest_declares_single_variable_followup() -> None:
    m = json.loads((NEW / "experiment_manifest.json").read_text(encoding="utf-8"))
    assert m["gamma"] == 0.8
    assert m["memory_noise_sd"] == 0.16
    assert m["parent_memory_noise_sd"] == 0.32
...
def test_all_cells_have_exact_new_contract() -> None:
    for cell, (task, location, scale) in CELLS.items():
        ...
        assert float(value(cmd, "--memory-noise-std")) == 0.16
...
def test_parent_and_followup_differ_only_in_noise_and_provenance_paths() -> None:
    for cell in CELLS:
        old = command(PARENT / "launch_cell.sh", cell, f"/tmp/old/{cell}", parent=True)
        new = command(NEW / "launch_cell.sh", cell, f"/tmp/new/{cell}")
        assert float(value(old, "--memory-noise-std")) == 0.32
        assert float(value(new, "--memory-noise-std")) == 0.16
