"""
Train the triplet-codebook recurrent ViT.

    python RViT_plus_triplet_codebook/train_rl.py --device mps
    python RViT_plus_triplet_codebook/train_rl.py --iters 2 --device cpu
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train triplet-codebook recurrent ViT")
    ...
    p.add_argument("--codebook-inter-coef", type=float, default=None)
    p.add_argument("--codebook-intra-coef", type=float, default=None)
    p.add_argument("--codebook-norm-coef", type=float, default=None)
    p.add_argument("--codebook-temperature", type=float, default=None)
...
def main() -> None:
    ...
    model_kwargs = dict(
        ...
        init_action_bias=init_action_bias, seq_len=seq_len,
    )
    model = TripletCodebookModel(**model_kwargs).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[triplet_codebook] task={args.task} tokens={model.n_tokens} "
          f"codebooks=3 params={n_params:,} device={device}")

    ckpt_dir = (args.checkpoint_dir or cfg_get(cfg, "run.checkpoint_dir", "")
                or os.path.expanduser("~/rvit_plus_checkpoints/triplet_codebook"))
    ...
    if init_mode in ("warm_start", "resume"):
        ...
        print(f"[triplet_codebook] {init_mode} LOADED {ckpt_path}\n[triplet_codebook] -> {info}")
    else:
        print("[triplet_codebook] init_mode=fresh")
    ...
    ppo_cfg = PPOConfig(
        ...
        codebook_inter_coef=args.codebook_inter_coef if args.codebook_inter_coef is not None
        else float(g("codebook_inter_coef", 0.05)),
        codebook_intra_coef=args.codebook_intra_coef if args.codebook_intra_coef is not None
        else float(g("codebook_intra_coef", 0.005)),
        codebook_norm_coef=args.codebook_norm_coef if args.codebook_norm_coef is not None
        else float(g("codebook_norm_coef", 0.0001)),
        codebook_temperature=args.codebook_temperature if args.codebook_temperature is not None
        else float(g("codebook_temperature", 0.1)),
    )
    ...
    print(f"[triplet_codebook] {iters} iters x {eps} episodes/iter = {iters * eps:,} episodes")
    ...
    final = os.path.join(ckpt_dir, f"rvit_triplet_codebook_{args.task}_final.pt")
    torch.save({"iter": iters, "model_state_dict": model.state_dict(),
                "model_kwargs": model_kwargs, "task": args.task}, final)
    print(f"[triplet_codebook] saved {final}; iters logged={len(history)}")
