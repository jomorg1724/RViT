...
_STREAM_FACTORIAL_LEVELS = frozenset((4, 100))


def _stream_factorial_contract(checkpoint, task):
    """Return a validated stream-factorial factory contract, or ``None``.

    Ordinary checkpoints have no ``model_factory`` entry and retain the
    historical reconstruction path.  A checkpoint that does declare a custom
    factory is fail-closed: only the registered v1 factorial contract is
    accepted, with an exact 10x10 carrier and one of the two preregistered
    effective stream levels on each axis.
    """
    factory = checkpoint.get("model_factory")
    if factory is None:
        return None
    if not isinstance(factory, dict):
        raise ValueError("checkpoint model_factory must be a mapping")
    if set(factory) != _STREAM_FACTORIAL_FACTORY_KEYS:
        raise ValueError(
            "checkpoint model_factory inventory mismatch: "
            f"got {sorted(factory)}, expected {sorted(_STREAM_FACTORIAL_FACTORY_KEYS)}"
        )
    if factory.get("kind") != "stream_factorial_v1":
        raise ValueError(f"unsupported checkpoint model_factory kind {factory.get('kind')!r}")
    if str(task) != "vda4":
        raise ValueError("stream_factorial_v1 checkpoints are registered only for task 'vda4'")
    carrier = factory.get("carrier_grid")
    if not isinstance(carrier, list) or carrier != [10, 10]:
        raise ValueError(
            "stream_factorial_v1 model_factory carrier_grid must be the exact list [10, 10]"
        )
    for field in ("effective_visual_streams", "effective_memory_streams"):
        value = factory.get(field)
        if isinstance(value, (bool, np.bool_)) or not isinstance(value, (int, np.integer)):
            raise ValueError(f"stream_factorial_v1 model_factory {field} must be an integer")
        if int(value) not in _STREAM_FACTORIAL_LEVELS:
            raise ValueError(f"stream_factorial_v1 model_factory {field} must be 4 or 100")
    return {
        "kind": "stream_factorial_v1",
        "effective_visual_streams": int(factory["effective_visual_streams"]),
        "effective_memory_streams": int(factory["effective_memory_streams"]),
        "carrier_grid": [10, 10],
    }
...
GRID = {"vda1": (2, 2, 50), "vda2": (2, 2, 50), "vda4": (2, 2, 50),
        "vda9": (3, 3, 75), "vda16": (4, 4, 100),
        "vda_fixed9": (4, 4, 100)}   # model grid per set-size env
...
VDA_TASK_METADATA = {
    "vda1": {"cue_index": TL, "active_locations": (TL,), "active_count": 1},
    "vda2": {"cue_index": TL, "active_locations": (TL, BR), "active_count": 2},
    "vda4": {"cue_index": TL, "active_locations": tuple(range(4)), "active_count": 4},
    "vda9": {"cue_index": TL, "active_locations": tuple(range(9)), "active_count": 9},
    "vda16": {"cue_index": TL, "active_locations": tuple(range(16)), "active_count": 16},
    # The controlled midpoint uses the same 4x4 carrier as VDA16 but samples
    # nine active items per trial. ``active_locations`` is therefore the set of
    # eligible locations; ``active_count`` is the realized set size.
    "vda_fixed9": {
        "cue_index": TL,
...
def vda_task_metadata(task):
    ...
    return {"cue_index": metadata["cue_index"],
            ...
            "active_count": int(metadata.get("active_count", len(metadata["active_locations"])))}
...
def load(
    ...
    *,
    ...
    factory = _stream_factorial_contract(ck, task)
    gr, gc, isz = ((10, 10, 50) if factory is not None else GRID.get(task, (2, 2, 50)))
    embedded = ck.get("model_kwargs")
    if factory is not None and not isinstance(embedded, dict):
        raise ValueError("stream-factorial checkpoint lacks embedded model_kwargs")
    if isinstance(embedded, dict):
        model_kwargs = dict(embedded)
        if factory is not None:
            # These fields define both the serialized base architecture and the
            # custom factory invocation, so they are required even when a
            # caller disables requested-identity checks for legacy analysis.
            required_factory_model = {
            ...
            for field, value in required_factory_model.items():
                if field not in model_kwargs:
                    raise ValueError(
                        f"stream-factorial checkpoint is missing model_kwargs[{field!r}]"
                    )
                actual = model_kwargs[field]
                ...
                if actual != value:
                    raise ValueError(
                        f"stream-factorial checkpoint {field}={actual!r} "
        ...
        if factory is None:
            ...
            factory_kwargs = dict(model_kwargs)
            for field in (
                "feedback",
                "d_mem",
                "grid_rows",
                ...
                factory_kwargs.pop(field, None)
            m = build_stream_factorial_model(
    ...
    model_front = m.front.base if hasattr(m.front, "base") else m.front
    out_norm_key = "front.base.out_norm.bias" if factory is not None else "front.out_norm.bias"
    if out_norm_key in sd and not isinstance(model_front.out_norm, nn.LayerNorm):
        model_front.out_norm = nn.LayerNorm(128)
...
def _active_for(task, invalid=False):
    ...
    metadata = vda_task_metadata(task)
    if metadata["active_count"] != len(metadata["active_locations"]):
        raise ValueError(f"{task} requires a conditioned active-set draw")
    return list(metadata["active_locations"])


def conditioned_active_locations(
    task,
    cue,
    change_true,
    change_index,
    *,
    required_active_locations=(),
    seed=0,
):
    """Draw a reproducible active set while keeping forced evaluation targets visible.

    Full-set tasks retain their historical fixed active locations.  Reduced
    fixed-grid tasks sample the remaining items without replacement after
    conditioning on the cue, any forced change, and caller-declared controls.
    This keeps a held-out forced-location contrast valid without replacing the
    random active-set nuisance distribution with one hand-picked layout.
    """
    metadata = vda_task_metadata(task)
    eligible = tuple(int(value) for value in metadata["active_locations"])
    active_count = int(metadata["active_count"])
    if active_count == len(eligible):
        return eligible
    required = {int(cue), *(int(value) for value in required_active_locations)}
    if int(change_true):
        required.add(int(change_index))
    if not required.issubset(eligible):
        raise ValueError(f"required active locations {sorted(required)} are invalid for {task}")
    if len(required) > active_count:
        raise ValueError(
            f"{task} has set size {active_count}, smaller than {len(required)} required locations"
        )
    remaining = np.asarray([value for value in eligible if value not in required], dtype=np.int64)
    rng = np.random.default_rng(seed)
    draw_count = active_count - len(required)
    drawn = rng.choice(remaining, size=draw_count, replace=False).astype(int).tolist()
    return tuple(sorted((*required, *drawn)))


def make_video(
    task, cue, prop, color, ct, ci, mag=56.0, seed=None, *,
    required_active_locations=(),
):
    ...
    try:
        ...
        if hasattr(e, "active"):
            ...
            if metadata["active_count"] == len(metadata["active_locations"]):
                ...
                active_seed = (int(seed) ^ 0x9E3779B9) % (2**32)
                e.active = list(conditioned_active_locations(
                    task,
                    cue,
                    ct,
                    ci,
                    required_active_locations=required_active_locations,
                    seed=active_seed,
                ))
...
def make_video_batch(
    task, cue, prop, color, ct, ci, mag=56.0, B=300, seed=0, *,
    required_active_locations=(),
):
