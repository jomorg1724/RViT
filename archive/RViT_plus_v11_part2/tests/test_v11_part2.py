"""
Smoke tests for RViT+ v11_part2 — single-head dual-stream with a SPLIT readout.

  Tμ SALIENCE: Q=X, K=V=[H1‖H2], residual=X  → ACTOR
  TQ TOP-DOWN: Q=X, K=V=H2,      residual=H2 → CRITIC
  H1=LSTM1(X), H2=LSTM2(Z_sal).

Run:  .venv/bin/python -m RViT_plus_v11_part2.tests.test_v11_part2
"""
...
from RViT_plus_v11_part2.env import ChangeDetectionEnv
from RViT_plus_v11_part2.tx_lstm_encoder import DualStreamEncoder
from RViT_plus_v11_part2.model import RViTPlusV11Part2Model
from RViT_plus_v11_part2.ppo import (
...
def _model(**over):
    ...
    kw.update(over)
    return RViTPlusV11Part2Model(**kw).to(DEVICE)
...
def test_structure_single_head_and_tags():
    ...
    assert enc.sal_tag.shape == (2, 1, 64), "salience K/V needs 2 source tags (H1, H2)"
    assert enc.stream_dim == 64, "each head reads ONE stream (d_model)"
    assert len(enc.cells) == 2
    try:
        DualStreamEncoder(n_tokens=100, d_model=64, d_mem=64, n_heads=1, n_lstm=3)
        raise AssertionError("n_lstm=3 should raise")
    except ValueError:
        pass
    print("✓ structure: single head each, salience source-tags (2,1,64), stream_dim=64")


def test_stream_shapes_and_routing():
    """Tμ attends over [H1‖H2] (2N keys), TQ over H2 (N keys). Salience weights move
    when EITHER H1 or H2 changes; top-down weights move only when H2 changes; the
    image moves both."""
    m = _model().eval()
    ...
    with torch.no_grad():
        ...
        # change H1 only → salience moves, top-down does NOT (TQ reads H2 only)
        ...
        assert not torch.allclose(attn[0], attn_h1[0]), "H1 change did not move salience weights (H1 is in its K/V)"
        assert torch.allclose(attn[1], attn_h1[1]), "H1 change leaked into top-down weights (TQ reads H2 only)"
        # change H2 only → BOTH move (H2 is in both K/V banks)
        ...
        assert not torch.allclose(attn[0], attn_h2[0]), "H2 change did not move salience weights (H2 now in its K/V)"
        assert not torch.allclose(attn[1], attn_h2[1]), "H2 change did not move top-down weights"
        # change image → both move
    ...
    print("✓ routing: Tμ reads [H1‖H2] (2N keys), TQ reads H2 (N keys); H1→sal only, H2→both, X→both")
...
def test_salience_grounded_topdown_gating_only():
    ...
    with torch.no_grad():
        ...
        assert not torch.allclose(rec_a[0], rec_b[0]), "salience Z_sal image-invariant with attention off — residual=X missing"
...
def test_split_readout():
    """The decisive v11_part2 proof: the ACTOR reads ONLY Z_sal, the CRITIC reads
    ONLY Z_td. Changing Z_sal moves the actor logits but NOT the critic Q; changing
    Z_td moves the critic Q but NOT the actor logits."""
    m = _model().eval()
    B = 4
    ...
    with torch.no_grad():
        ...
        # perturb salience → actor changes, critic unchanged
        al2, q2, _, _ = m._run_heads([torch.randn(B, 100, 64), zt])
        assert not torch.allclose(al, al2), "actor ignores Z_sal — should read it"
        assert torch.allclose(q, q2), "critic moved when only Z_sal changed — it must read Z_td only"
        # perturb top-down → critic changes, actor unchanged
        al3, q3, _, _ = m._run_heads([zs, torch.randn(B, 100, 64)])
        assert torch.allclose(al, al3), "actor moved when only Z_td changed — it must read Z_sal only"
        assert not torch.allclose(q, q3), "critic ignores Z_td — should read it"
    print("✓ split readout: actor←Z_sal (Tμ), critic←Z_td (TQ); each ignores the other stream")
...
def test_memory_update_sources():
    ...
    with torch.no_grad():
        ...
        assert not torch.allclose(Hs[0], Hsb[0]) and not torch.allclose(Hs[1], Hsb[1])
...
def test_conv_heads_single_stream():
    ...
    for name, dec in (("actor", m.actor_head), ("critic", m.critic_head)):
        ...
        assert len(convs) == 3, f"{name} trunk has {len(convs)} Conv1d; expected 3"
        assert convs[0].in_channels == 64, f"{name} first conv must ingest d_model=64 (single stream), got {convs[0].in_channels}"
...
def test_grad_flows_through_stack():
    ...
    print("✓ gradient reaches patch_embed + BOTH streams + salience tags + both LSTMs + both heads")
...
if __name__ == "__main__":
    tests = [
        ...
        test_stream_shapes_and_routing,
    ...
    for t in tests:
        t()
    print(f"\nAll {len(tests)} v11_part2 smoke tests passed (incl. split-readout + Tμ[H1‖H2] proofs).")
