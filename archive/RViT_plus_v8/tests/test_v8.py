"""
Smoke tests for RViT+ v8 — v5_part2 with the H1-RESIDUAL: the cross-attention
block's skip path carries the previous frame's H1 memory instead of the visual
input X, so current-frame visual content can only move downstream through the
attention values (visual attention is load-bearing by construction).

Run:  .venv/bin/python -m RViT_plus_v8.tests.test_v8
"""
...
from RViT_plus_v8.tx_lstm_encoder import TxLSTMEncoder
from RViT_plus_v8.model import RViTPlusV5Part2Model
from RViT_plus_v8.ppo import (
...
def test_h1_residual_no_visual_bypass():
    """THE v8 property: with the attention output silenced (out_proj zeroed),
...
if __name__ == "__main__":
    ...
    for t in tests:
        t()
    print(f"\nAll {len(tests)} v8 smoke tests passed (incl. the H1-residual no-bypass proof).")
