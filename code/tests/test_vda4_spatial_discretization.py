...
def test_uneven_grid_batches_exact_shapes_and_matches_patch_loop():
    torch.manual_seed(0)
    front = ConvPatchFrontEnd(grid_rows=4, grid_cols=4, image_size=50)
    ...
    patches = [x[:, :, r0:r1, c0:c1] for (r0, r1, c0, c1) in front._bounds]
    loop_embeddings = torch.stack([front._encode_patch(p) for p in patches], dim=1)
    torch.testing.assert_close(tokens[:, :, :128], loop_embeddings, rtol=1e-5, atol=1e-6)
    assert tokens.shape == (2, 16, 128 + 16 + 8)

    tokens.square().mean().backward()
    stem_grad = front.stem[0].weight.grad
