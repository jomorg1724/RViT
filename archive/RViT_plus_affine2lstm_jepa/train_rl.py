"""
Train affine two-xLSTM + sequential JEPA variant.

    python RViT_plus_affine2lstm_jepa/train_rl.py --device mps
    python RViT_plus_affine2lstm_jepa/train_rl.py --iters 2 --device cpu
"""
...
from model import Affine2LSTMJEPAModel                                   # noqa: E402
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train affine2lstm + sequential JEPA")
    ...
    p.add_argument("--jepa-coef", type=float, default=None)
    p.add_argument("--jepa-ema-decay", type=float, default=None)
...
def main() -> None:
    ...
    jepa_latent_dim = int(cfg_get(cfg, "model.jepa_latent_dim", 256))
    model_kwargs = dict(
        n_actions=2, n_quantiles=n_quantiles, init_action_bias=init_action_bias,
        seq_len=seq_len, jepa_latent_dim=jepa_latent_dim,
    )
    model = Affine2LSTMJEPAModel(**model_kwargs).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[affine2lstm_jepa] task={args.task} no_vae conv4x4 tokens={model.n_tokens} "
          f"params={n_params:,} device={device}")

    ckpt_dir = (args.checkpoint_dir or cfg_get(cfg, "run.checkpoint_dir", "")
                or os.path.expanduser("~/rvit_plus_checkpoints/affine2lstm_jepa"))
    ...
    if init_mode in ("warm_start", "resume"):
        ...
        print(f"[affine2lstm_jepa] {init_mode} LOADED {ckpt_path}\n[affine2lstm_jepa] -> {info}")
    else:
        print("[affine2lstm_jepa] init_mode=fresh (no VAE, no pretraining)")
    ...
    ppo_cfg = PPOConfig(
        ...
        jepa_coef=args.jepa_coef if args.jepa_coef is not None else float(g("jepa_coef", 0.25)),
        jepa_ema_decay=args.jepa_ema_decay if args.jepa_ema_decay is not None else float(g("jepa_ema_decay", 0.995)),
    )
    ...
    print(f"[affine2lstm_jepa] {iters} iters x {eps} episodes/iter = {iters * eps:,} episodes")
    ...
    final = os.path.join(ckpt_dir, f"rvit_affine2lstm_jepa_{args.task}_final.pt")
    torch.save({"iter": iters, "model_state_dict": model.state_dict(),
                "model_kwargs": model_kwargs, "task": args.task}, final)
    print(f"[affine2lstm_jepa] saved {final}; iters logged={len(history)}")
