"""
Actor and critic decoders for V10.

Each head reads the encoder's READOUT memory [CLS ++ H₃] (1 + N tokens; v10
decoders see only the readout LSTM's state, not H₁/H₂) through a small
pre-norm transformer whose attention is exposed and bias-injectable (same
mechanism as the encoder), then decodes the CLS embedding with a ≥2-layer MLP
head (small-init final Linear — v5's proven readout).
...
class _CLSTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        if state_names is not None and len(state_names) != n_states:
            raise ValueError(f"state_names must have {n_states} entries; got {state_names}")
        self.state_names = state_names or [f"H{k + 1}" for k in range(n_states)]
    ...
    def token_layout(self) -> Dict[str, Tuple[int, int]]:
        """Named spans of the decoder sequence: [CLS | state_names...]."""
        layout = {"cls": (0, 1)}
        for k, name in enumerate(self.state_names):
            layout[name] = (1 + k * self.n_tokens, 1 + (k + 1) * self.n_tokens)
        return layout
