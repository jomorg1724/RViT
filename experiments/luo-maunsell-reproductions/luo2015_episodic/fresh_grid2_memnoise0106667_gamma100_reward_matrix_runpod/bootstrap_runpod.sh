...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON_BIN" -m pytest -q \
 ...
 tests/test_luo2015_fresh_memnoise0106667_gamma080_runpod.py tests/test_luo2015_fresh_memnoise0106667_gamma100_runpod.py
CANARY="/workspace/canary_luo_memnoise0106667_gamma100_${CELL}"; rm -rf "$CANARY"
env CELL="$CELL" RUN_ROOT="$CANARY" PYTHON_BIN="$PYTHON_BIN" ITERS=1 SAVE_EVERY=1 DEVICE=cuda bash experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma100_reward_matrix_runpod/launch_cell.sh > "/workspace/canary_memnoise0106667_${CELL}.log" 2>&1
...
a=c['training_args']; assert math.isclose(float(a['memory_noise_std']),0.10666666666666666); assert math.isclose(float(a['noise']),5.0); assert a['curriculum'] is False
assert math.isclose(float(c['environment_state']['theta']),65.0)
PY
CANARY_SHA="$(sha256sum "$FINAL" | cut -d' ' -f1)"; mkdir -p "$RUN_ROOT"
printf '{"source_archive_sha256":"%s","cell":"%s","grid":[2,2],"memory_noise_std":0.10666666666666666,"sensory_noise_std":5.0,"gamma":1.0,"initialization":"fresh","canary_checkpoint_sha256":"%s"}\n' "$ACTUAL_SHA" "$CELL" "$CANARY_SHA" > "$RUN_ROOT/deployment_contract.json"
(
 set +e
 env CELL="$CELL" RUN_ROOT="$RUN_ROOT" PYTHON_BIN="$PYTHON_BIN" ITERS=20000 SAVE_EVERY=50 DEVICE=cuda bash experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma100_reward_matrix_runpod/launch_cell.sh
...
printf 'BOOTSTRAP_OK cell=%s gamma=1.0 memory_noise=0.10666666666666666 source_sha256=%s canary_sha256=%s pid=%s root=%s\n' "$CELL" "$ACTUAL_SHA" "$CANARY_SHA" "$pid" "$RUN_ROOT"
