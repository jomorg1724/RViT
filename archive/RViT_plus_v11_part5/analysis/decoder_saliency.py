...
Usage:
    .venv/bin/python RViT_plus_v11_part2/analysis/decoder_saliency.py \
...
from RViT_plus_v11_part2.analysis.avg_attention_maps import (
...
def saliency_rollout(model, envs, obs0, device):
    ...
    readout token position. v11_part2 has a SPLIT readout: the actor reads ONLY the
    salience output Z_sal, the critic reads ONLY the top-down output Z_td, so each
    head's saliency is over its OWN stream:
        actor:  s_i = ||∂press/∂Z_sal[:,i,:]||_1     (over salience-stream positions)
        critic: s_i = ||∂Q(press)/∂Z_td[:,i,:]||_1   (over top-down-stream positions)
    Returns (T, 2, gh, gw) [row0=actor over Z_sal, row1=critic over Z_td], an obs
    example, and a placeholder split (always 1.0 — actor reads salience by design)."""
    B = len(envs)
    model.eval()
    ...
    while t <= T and not done.all():
        ...
        logits = model.actor_head([Z_sal])                     # actor reads Z_sal only
        q = model.critic_head([Z_td])                          # critic reads Z_td only
        press = logits[:, 1].sum()
        q_press = q[:, 1, :].mean(dim=1).sum()
        g_a = torch.autograd.grad(press, Z_sal, retain_graph=True)[0]   # ∂press/∂Z_sal
        g_c = torch.autograd.grad(q_press, Z_td, retain_graph=True)[0]  # ∂Q/∂Z_td
        sal_a = g_a.abs().sum(-1).mean(0)                       # (N,)
        sal_c = g_c.abs().sum(-1).mean(0)
        frac_sal.append(1.0)                                   # actor reads salience by construction
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out-dir", default="RViT_plus_v11_part2/analysis/figures_decoder")
    ...
    for cond in args.conditions:
        ...
        from RViT_plus_v11_part2.analysis.avg_attention_maps import quad_aggregate
