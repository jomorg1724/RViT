...
import os, sys, glob, numbers, numpy as np, torch, torch.nn as nn
torch.set_num_threads(3)
_ROOT = "/Users/jonathanmorgan/AttentionManuscript/RViT_plus_paper_jepa_grid9"
...
CKROOT = "/Users/jonathanmorgan/AttentionManuscript/battery_sweep_results/pod2/ckpt2"
...
def load(task, feedback, dm=128, checkpoint_path=None):
    snap = checkpoint_path or ckpt(task, feedback, dm)
    assert snap, f"no ckpt for {task} {feedback} d{dm}"
    ...
    m = RViTPaperModel(cell="xlstm", feedback=feedback, n_quantiles=5, seq_len=T, jepa_n_heads=4,
                       ...
                       grid_rows=gr, grid_cols=gc, image_size=isz)
