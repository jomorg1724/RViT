...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_recur", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusRecurModel:
...
def load_checkpoint(model: RViTPlusRecurModel, ckpt_path: str, device: torch.device) -> int:
