...
class SpatialXLSTM(nn.Module):
    def __init__(self, input_dim: int = D_TOKEN, d_mem: int = D_MEM) -> None:
        ...
        # WORKING-MEMORY NOISE lever: relative Gaussian noise added to the PERSISTENT cell state C each
        # step (NOT the readout). Because C_prev carries it forward, the maintained memory DRIFTS, so the
        # recurrence must learn ATTRACTORS that pull drift back onto meaningful representations, and the
        # model must PRIORITISE what it holds (e.g. by value) — the WM-capacity / attentional-blink
        # bottleneck. 0 = off. Set externally on both cells via the encoder; applied in train AND eval.
        self.mem_noise = 0.0
    ...
    def forward(self, Z, H_prev, C_prev, N_prev, M_prev):
        ...
        N = F * N_prev + I
        C = C_prev * F + U * I
        if self.mem_noise > 0.0:                                 # noise on the PERSISTENT cell state C:
            C = C + self.mem_noise * C.detach().std() * torch.randn_like(C)   # C_prev carries it forward, so
        H = O * (C / (N + 1e-8))                                 # the memory DRIFTS each step -> the recurrence
        return H, C, N, M                                        # must learn ATTRACTORS to pull it back
...
class RecurrentViTxLSTM(nn.Module):
    ...
    def __init__(self, d_token: int = D_TOKEN, d_mem: int = D_MEM, n_patch: int = N_PATCH,
                 ...
                 cell: str = "xlstm", mem_heads: int = 4) -> None:
        super().__init__()
        ...
        if feedback in ("crossattn", "crossattn1"):
            ...
            self.attn = CrossAttentionXH(d_token, d_mem)         # Q(X), K=K(X,H), V=V(X,H)
