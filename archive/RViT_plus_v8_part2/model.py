"""
RViTPlusV5Part2Model (v8_part2 build) — conv-free, RL-only recurrent attention
model with the H1-RESIDUAL encoder and MEMORY-ONLY K/V: patch tokens are queries
only; keys/values are [H1 ++ H2] with no visual content in the value stream.
Visual information reaches the recurrent state ONLY through query-side gating
over the mnemonic codebook (see tx_lstm_encoder.py).
...
PAC + QR-DQN + PER trainer, all hyperparameters — is unchanged):
  ...
  1. ENCODER: a CROSS-ATTENTION-over-memory block (tx_heads=8) followed by TWO
     ...
     QUERIES ONLY; the keys/values are [H1 ++ H2] (carried LSTM memories, AS
     TOKENS — no patch in K/V). Memory is the sole mnemonic codebook; with only
     N queries the output Z is (B,N,d_model).
...
Pipeline (no convolution stem, no PC/JEPA, no decoder/VAE):
    ...
    frame x_t (B,3,50,50)
       ...
       │     cross-attn: Q=X, K=V=[H1 ++ H2] (memory-only) → Z (B,N,d)
