...
try:
    ...
    from .rl_heads import (
        ActorHead, ContrastiveProjectionHead, CriticHead, PredictiveCodingHead,
    )
    ...
    from rl_heads import (  # type: ignore[no-redef]
        ActorHead, ContrastiveProjectionHead, CriticHead, PredictiveCodingHead,
    )
...
class RViTPlusModel(nn.Module):
    ...
    def __init__(
        ...
        # Two SimCLR-style heads project EACH BRANCH'S UNIQUE specialist C₃
        # state (cell3_actor / cell3_critic) to an L2-normalised vector for
        # cosine-similarity contrastive losses. The shared C₁/C₂ states are
        # deliberately NOT fed into these heads — they're shaped by PPO + QH +
        # recon only, and folding them into both contrastive losses would put
        # conflicting pressure on the shared encoder layers.
        # When `split_c3=False`, both heads receive the SAME shared C₃; the
        # design assumes `split_c3=True` to give each branch a unique signal.
        c3 = state_channels[2]
        self.contrastive_actor_head: Optional[ContrastiveProjectionHead] = None
        self.contrastive_critic_head: Optional[ContrastiveProjectionHead] = None
        if self.enable_actor:
            self.contrastive_actor_head = ContrastiveProjectionHead(
                in_channels=c3,
                projection_dim=contrastive_projection_dim,
                hidden=contrastive_hidden,
            )
        if self.enable_critic:
            self.contrastive_critic_head = ContrastiveProjectionHead(
                in_channels=c3,
                projection_dim=contrastive_projection_dim,
                hidden=contrastive_hidden,
            )
