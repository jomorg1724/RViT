...
STREAM_FACTORIAL_KIND = "stream_factorial_v1"
STREAM_FACTORIAL_LEVELS = frozenset((4, 100))
STREAM_FACTORIAL_CARRIER_GRID = (10, 10)
_STREAM_FACTORIAL_FACTORY_KEYS = frozenset(
    (
        "kind",
...
def _validated_stream_factorial_factory(model_factory: dict) -> dict:
    """Return the exact versioned factory record or reject malformed metadata."""
    if not isinstance(model_factory, dict) or set(model_factory) != _STREAM_FACTORIAL_FACTORY_KEYS:
        raise ValueError("stream-factorial model_factory must contain exactly the registered fields")
    visual = model_factory.get("effective_visual_streams")
    memory = model_factory.get("effective_memory_streams")
    if visual not in STREAM_FACTORIAL_LEVELS or memory not in STREAM_FACTORIAL_LEVELS:
        raise ValueError("stream-factorial effective stream counts must each be 4 or 100")
    if model_factory.get("kind") != STREAM_FACTORIAL_KIND:
        raise ValueError(f"stream-factorial model_factory kind must be {STREAM_FACTORIAL_KIND!r}")
    if model_factory.get("carrier_grid") != list(STREAM_FACTORIAL_CARRIER_GRID):
        raise ValueError("stream-factorial carrier_grid must be [10, 10]")
    return {
        "kind": STREAM_FACTORIAL_KIND,
        "effective_visual_streams": int(visual),
        "effective_memory_streams": int(memory),
        "carrier_grid": list(STREAM_FACTORIAL_CARRIER_GRID),
    }


def resolve_stream_factorial_model_factory(
    args: argparse.Namespace, *, grid_rows: int, grid_cols: int
) -> dict | None:
    """Resolve and fail-closed validate the optional stream-factorial contract."""
    visual = args.effective_visual_streams
    memory = args.effective_memory_streams
    if (visual is None) != (memory is None):
        raise ValueError(
            "--effective-visual-streams and --effective-memory-streams must be provided together"
        )
    if visual is None:
        return None

    required = (
        (args.task == "vda4", "--task vda4"),
        ((int(grid_rows), int(grid_cols)) == STREAM_FACTORIAL_CARRIER_GRID,
         "--patch-grid-rows 10 --patch-grid-cols 10"),
        (args.conv_frontend is True, "--conv-frontend"),
        (args.cell == "xlstm", "--cell xlstm"),
        (args.two_lstm is False, "one xLSTM (omit --two-lstm)"),
        (int(args.d_mem) == 128, "--d-mem 128"),
        (args.feedback in ("crossattn1", "affine_ew"),
         "--feedback crossattn1 or --feedback affine_ew"),
        (float(args.memory_decay) == 1.0, "--memory-decay 1.0"),
        (float(args.memory_noise_std) == 0.0, "--memory-noise-std 0.0"),
    )
    violations = [description for satisfied, description in required if not satisfied]
    if violations:
        raise ValueError(
            "stream-factorial contract violation; required: " + ", ".join(violations)
        )
    return _validated_stream_factorial_factory({
        "kind": STREAM_FACTORIAL_KIND,
        "effective_visual_streams": int(visual),
        "effective_memory_streams": int(memory),
        "carrier_grid": list(STREAM_FACTORIAL_CARRIER_GRID),
...
def build_training_model(model_kwargs: dict, model_factory: dict | None = None):
    """Construct the registered model while leaving the historical path unchanged."""
    if model_factory is None:
        return RViTPaperModel(**model_kwargs)
    factory = _validated_stream_factorial_factory(model_factory)
    from experiments.vda_stream_factorial.stream_model import (  # noqa: E402
        build_stream_factorial_model,
    )

    kwargs = dict(model_kwargs)
    feedback = kwargs.pop("feedback")
    return build_stream_factorial_model(
        factory["effective_visual_streams"],
        factory["effective_memory_streams"],
        feedback,
        **kwargs,
    )


def _persist_model_factory_metadata(
    resume_contract: dict, checkpoint_metadata: dict, model_factory: dict | None
) -> None:
    """Bind a custom factory into both checkpoint identity records."""
    if model_factory is None:
        return
    factory = _validated_stream_factorial_factory(model_factory)
    resume_contract["model_factory"] = dict(factory)
    checkpoint_metadata["model_factory"] = dict(factory)


def _validate_checkpoint_model_factory(
    checkpoint: dict, expected_model_factory: dict | None
) -> None:
    """Prevent same-shape projector buffers from crossing factorial conditions."""
    saved = checkpoint.get("model_factory")
    if saved is None and expected_model_factory is None:
        return
    if saved is None or expected_model_factory is None:
        raise ValueError("checkpoint model_factory does not match the requested model path")
    if _validated_stream_factorial_factory(saved) != _validated_stream_factorial_factory(
        expected_model_factory
    ):
        raise ValueError("checkpoint stream-factorial model_factory mismatch")
...
def load_warm_start_parent(model, checkpoint_path: str, *, expected_sha256: str, device,
                           expected_model_factory: dict | None = None):
    ...
    _validate_checkpoint_model_factory(checkpoint, expected_model_factory)
...
def load_frozen_trunk_probe_parent(
    model,
    checkpoint_path: str,
    *,
    expected_sha256: str,
    ...
    """Load only ``front`` + ``encoder`` from a parent and train fresh RL heads.

    The parent actor, critic, JEPA heads, centers, optimizer, teachers, and environment
    state are intentionally excluded.  The copied trunk is frozen so this is a probe
    of information already present in its representation rather than further trunk
    learning.
    """
    resolved_path = os.path.abspath(checkpoint_path)
    actual_sha256 = _sha256(resolved_path)
    if not expected_sha256:
        raise ValueError("frozen_trunk_probe requires --expected-parent-sha256")
    if actual_sha256 != expected_sha256:
        raise ValueError(
            f"frozen-trunk parent hash mismatch: expected {expected_sha256}, got {actual_sha256}"
        )

    checkpoint = torch.load(resolved_path, map_location="cpu", weights_only=False)
    parent_state = checkpoint.get("model_state_dict")
    if not isinstance(parent_state, dict):
        raise ValueError("frozen-trunk parent has no model_state_dict")

    current_state = model.state_dict()
    trunk_prefixes = ("front.", "encoder.")
    trunk_names = [name for name in current_state if name.startswith(trunk_prefixes)]
    missing = [name for name in trunk_names if name not in parent_state]
    if missing:
        raise ValueError(f"frozen-trunk parent is missing trunk tensors: {missing[:8]}")
    mismatched = [
        name
        for name in trunk_names
        if tuple(parent_state[name].shape) != tuple(current_state[name].shape)
    ]
    if mismatched:
        raise ValueError(f"frozen-trunk parent has incompatible tensor shapes: {mismatched[:8]}")

    merged_state = dict(current_state)
    for name in trunk_names:
        merged_state[name] = parent_state[name]
    model.load_state_dict(merged_state, strict=True)

    trainable_prefixes = ("actor_head.", "critic_head.")
    ...
    info = {
        ...
        "checkpoint_task": checkpoint.get("task"),
        "checkpoint_sha256": actual_sha256,
        "loaded_trunk_tensors": len(trunk_names),
        "fresh_head_tensors": sum(
            name.startswith(trainable_prefixes) for name in current_state
        ),
    }
    contract = {
        "mode": "frozen_trunk_probe",
        "checkpoint_path": resolved_path,
        "checkpoint_sha256": actual_sha256,
        "checkpoint_iteration": checkpoint.get("iter"),
        "checkpoint_task": checkpoint.get("task"),
        "loaded_prefixes": ["front.", "encoder."],
        "frozen_prefixes": ["front.", "encoder."],
        "fresh_trainable_prefixes": ["actor_head.", "critic_head."],
        "excluded_parent_state": [
            "actor_head",
            "critic_head",
            "jepa_heads_and_centers",
            "optimizer",
            "PAC_target",
            "JEPA_teacher",
            "environment_and_curriculum",
        ],
    }
    return info, contract
...
def _producer_hashes(config_path: str | None = None,
                     experiment_launcher: str | None = None,
                     model_factory: dict | None = None) -> dict[str, str]:
    relative_paths = (
        "train_rl.py",
        "ppo.py",
        "model.py",
    ...
    if model_factory is not None:
        _validated_stream_factorial_factory(model_factory)
        relative_paths += (
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--memory-noise-std", type=float, default=0.0,
                   help="standard deviation of zero-mean Gaussian noise added to the xLSTM "
                        "cell state C after every update (default 0.0, disabled).")
    p.add_argument("--memory-output-noise-std", type=float, default=0.0,
                   help="standard deviation of independent Gaussian noise added to the H1/H2 "
                        "memory OUTPUTS (post-nonlinearity) when inject_memory_noise=True; used "
                        "for the JEPA-pretrain denoising phase (default 0.0, disabled).")
    ...
    p.add_argument(
        "--effective-visual-streams", type=int, choices=(4, 100), default=None,
        help="activate the registered fixed-100-carrier stream factorial at visual rank 4 or 100; "
             "requires --effective-memory-streams and the exact factorial contract",
    )
    p.add_argument(
        "--effective-memory-streams", type=int, choices=(4, 100), default=None,
        help="activate the registered fixed-100-carrier stream factorial at memory rank 4 or 100; "
             "requires --effective-visual-streams and the exact factorial contract",
    )
    p.add_argument("--luo-spatial-grid-size", type=int, choices=(2, 4), default=2,
                   help="Luo-only spatial layout. 4 center-pads the unchanged two-stimulus "
                        "50px scene in a 100px 4x4 grid (stimuli in patches 5 and 10).")
    ...
    p.add_argument("--bc-alpha", type=float, default=None,
                   help="historical PAC behavioral-cloning blend α; 0 disables BC (default 0.0)")
    p.add_argument("--actor-coef", type=float, default=None,
                   help="weight on the MPO actor objective (default from config, 1.0)")
    p.add_argument("--value-coef", type=float, default=None,
                   help="weight on the distributional critic objective (default from config, 0.5)")
    ...
    p.add_argument(
        "--init-mode",
        choices=["fresh", "warm_start", "frozen_trunk_probe", "resume"],
        default=None,
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=("use independently initialized front-end→Transformer→memory streams; "
              "actor reads one stream and critic reads the other"),
    )
    ...
    p.add_argument(
        ...
        help=(
            "recurrent cell: xlstm (paper H,C,N,M) | layernorm_lstm "
            "(H,C only with LayerNorm(C)) | softmax_head (C-only, mem_heads "
            "heads per patch, each softmax-normalised — a categorical memory) | "
            "transformer_memory (H-only cross-attention memory: Q=H_prev, "
            "K/V=[H_prev,Z]) | transformer_memory_2layer (stacked H1/H2 memories: "
            "visual feedback from H1_prev; H1 reads Z; H2 reads H1; actor/critic read H2) | "
            "transformer_memory_2layer_softmax (same routing, but each H1/H2 token is "
            "softmax-normalised over d_mem before any downstream use)."
        ),
    )
    ...
    p.add_argument("--trunk-lr-ratio", type=float, default=1.0,
                   help="trunk (front+encoder+representation) learning rate as a fraction of "
                        "--lr; use e.g. 0.001 to keep a pretrained trunk nearly frozen while "
                        "the actor/critic heads train at full rate.")
    p.add_argument("--pretrained-trunk", default=None,
                   help="checkpoint (e.g. jepa_pretrain_latest.pt) whose front.* and encoder.* "
                        "weights are loaded into a fresh model before RL head training.")
    p.add_argument("--fsq-levels", type=int, default=1,
                   help="FSQ quantization levels per coordinate on memory tokens "
                        "(1=softmax, 2=binary, 5=5-level FSQ, etc.)")
    ...
    p.add_argument("--jepa-sinkhorn-iters", type=int, default=0,
                   help="positive iterations enable per-layer/token/head balanced teacher assignments")
    p.add_argument("--jepa-var-coef", type=float, default=0.0,
                   help="variance-floor coefficient on pre-prototype student features")
    p.add_argument("--jepa-cov-coef", type=float, default=0.0,
                   help="covariance-decorrelation coefficient on those features")
    p.add_argument(
        "--teacher-actor-critic-ste",
        action="store_true",
        help=(
            "feed detached JEPA-teacher H2 to actor/critic in forward while routing "
            "their representation gradients to student H2 with a straight-through estimator"
        ),
    )
...
def main() -> None:
    ...
    is_luo2015 = args.task.startswith("luo2015_")
    if args.luo_spatial_grid_size != 2 and not is_luo2015:
        raise ValueError("--luo-spatial-grid-size is only valid for luo2015_* tasks")
    if is_luo2015 and args.luo_spatial_grid_size == 4:
        requested_patch_grid = (args.patch_grid_rows, args.patch_grid_cols)
        if requested_patch_grid == (None, None):
            args.patch_grid_rows = args.patch_grid_cols = 4
        elif requested_patch_grid != (4, 4):
            raise ValueError(
                "--luo-spatial-grid-size 4 requires a 4x4 sensory patch grid"
            )

    # Pass Luo reward/layout kwargs through ONLY for Luo2015 tasks, so other envs
    ...
    if is_luo2015 and args.luo_spatial_grid_size == 4:
        print("[luo] centered 4x4 layout: exactly two unchanged 25x25 stimuli in "
              "patches 5 and 10; 12 peripheral patches are blank")
    if args.curriculum:
    ...
    model_factory = resolve_stream_factorial_model_factory(
        args, grid_rows=grid_rows, grid_cols=grid_cols
    )
    ...
    model_kwargs = dict(
        ...
        dual_actor_critic_streams=args.dual_actor_critic_streams,
        conv_frontend=args.conv_frontend,
        grid_rows=grid_rows, grid_cols=grid_cols, image_size=image_size,
    )
    model = build_training_model(model_kwargs, model_factory).to(device)
    if args.fsq_levels >= 2:
        model.encoder.fsq_levels = args.fsq_levels
        print(f"[paper] FSQ quantization: {args.fsq_levels} levels per coordinate "
              f"(replaces softmax on H1/H2 memory tokens)")
    if args.pretrained_trunk:
        blob = torch.load(args.pretrained_trunk, map_location=device, weights_only=False)
        trunk_state = {k: v for k, v in blob["model_state_dict"].items()
                       if k.startswith("front.") or k.startswith("encoder.")}
        res = model.load_state_dict(trunk_state, strict=False)
        print(f"[paper] loaded pretrained trunk from {args.pretrained_trunk}: "
              f"{len(trunk_state)} params (missing={len(res.missing_keys)}, "
    ...
    print(f"[paper] task={args.task} tokens={model.n_tokens} particles={n_quantiles} "
          f"feedback={args.feedback} cell={args.cell} memory_decay={args.memory_decay:.3f} "
          f"memory_noise_std={args.memory_noise_std:.4f} "
          f"dual_actor_critic_streams={args.dual_actor_critic_streams} "
    ...
    if init_mode in ("warm_start", "frozen_trunk_probe", "resume"):
        import glob
        ...
        if init_mode == "frozen_trunk_probe":
            if args.jepa_coef != 0.0:
                raise ValueError(
                    "frozen_trunk_probe trains only fresh actor/critic heads; use --jepa-coef 0"
                )
            if not args.expected_parent_sha256:
                raise ValueError(
                    "frozen_trunk_probe requires --expected-parent-sha256 to bind the trunk"
                )
            checkpoint_info, initialization_contract = load_frozen_trunk_probe_parent(
                model,
                ckpt_path,
                expected_sha256=args.expected_parent_sha256,
                device=device,
            )
            if checkpoint_info["checkpoint_task"] != args.task:
                raise ValueError(
                    "frozen-trunk parent task mismatch: "
                    f"expected {args.task}, got {checkpoint_info['checkpoint_task']}"
                )
            ...
            _validate_checkpoint_model_factory(checkpoint_blob, model_factory)
            ...
            if args.expected_parent_sha256 is not None:
                checkpoint_info, initialization_contract = load_warm_start_parent(
                    ...
                    expected_sha256=args.expected_parent_sha256,
                    device=device,
                    expected_model_factory=model_factory,
                )
            else:
        ...
        if init_mode == "frozen_trunk_probe":
            ...
            print(
                "[paper] FROZEN TRUNK PROBE: front+encoder frozen; fresh actor+critic only; "
                f"trainable tensors={len(trainable_names)} params="
    ...
    ppo_cfg = PPOConfig(
        lr=args.lr if args.lr is not None else float(g("lr", 2e-4)),
        ...
        bc_alpha=args.bc_alpha if args.bc_alpha is not None else float(g("bc_alpha", 0.0)),
        actor_coef=args.actor_coef if args.actor_coef is not None else float(g("actor_coef", 1.0)),
        value_coef=args.value_coef if args.value_coef is not None else float(g("value_coef", 0.5)),
        ...
        teacher_actor_critic_ste=args.teacher_actor_critic_ste,
        trunk_lr_ratio=args.trunk_lr_ratio,
    )
    if ppo_cfg.jepa_sinkhorn_iters < 0:
        raise ValueError("--jepa-sinkhorn-iters must be non-negative")
    if ppo_cfg.teacher_actor_critic_ste:
        if ppo_cfg.jepa_coef <= 0.0:
            raise ValueError("--teacher-actor-critic-ste requires --jepa-coef > 0")
        if args.cell not in (
            "transformer_memory_2layer", "transformer_memory_2layer_softmax",
            "transformer_memory_2layer_softmax_modern",
        ):
            raise ValueError("--teacher-actor-critic-ste requires a two-layer H1/H2 memory cell")
    if not 0.0 <= ppo_cfg.bc_alpha <= 1.0:
        raise ValueError("--bc-alpha must be in [0,1]")
    if ppo_cfg.actor_coef < 0.0 or ppo_cfg.value_coef < 0.0:
        raise ValueError("actor and critic loss coefficients must be non-negative")
    if ppo_cfg.jepa_var_coef < 0.0 or ppo_cfg.jepa_cov_coef < 0.0:
        raise ValueError("JEPA variance/covariance coefficients must be non-negative")
    print(f"[paper] ppo: reward_scale={1.0 if args.reward_scale is None else args.reward_scale} "
          ...
          f"actor_coef={ppo_cfg.actor_coef} value_coef={ppo_cfg.value_coef} "
          f"jepa_coef={ppo_cfg.jepa_coef} bc_alpha={ppo_cfg.bc_alpha} "
          f"entropy_coef={ppo_cfg.entropy_coef} gamma={ppo_cfg.gamma}")
    ...
    producer_sha256 = _producer_hashes(
        args.config, args.experiment_launcher, model_factory=model_factory
    )
    runtime_versions = {
    ...
    _persist_model_factory_metadata(resume_contract, checkpoint_metadata, model_factory)
