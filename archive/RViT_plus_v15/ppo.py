...
try:
    ...
    from model import RViTPlusV15Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # v15 ENERGY term (VQ-VAE commitment): penalize mean‖energy‖² = mean‖Z − C‖²
    # (energy_cost_seq) so the energized codebook Z stays near the codebook C — the
    # "energy cost". With it high (or 0 EMA) the model reduces to v12. 0 disables.
    energy_coef: float = 0.01
    # v15 EMA (VQ-VAE codebook update): C ← C + codebook_ema_rate·mean(energy), i.e.
    # EMA the codebook toward the energized Z. SMALL; the slow consolidation on top of
    # the gradient-learned codebook. 0 = pure v12 + energy (no consolidation).
    codebook_ema_rate: float = 1.0e-3
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV15Model] = None,
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_energy": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0, "energy_dev": 0.0, "codebook_drift": 0.0,
        "n_updates": 0.0, "n_loss_evals": 0.0, "n_skipped": 0.0, "max_grad_elem": 0.0,
        "per_episode_priority": None,
    }
    energy_for_ema = None   # last epoch's detached energy_seq (B,T,N,d) — the EMA signal
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        energy_cost_seq = out.get("energy_cost_seq", None)  # (B,T) mean‖energy‖² or None
        energy_for_ema = out.get("energy_seq", None)        # (B,T,N,d) detached — keep last epoch's
        ...
        # ── v15 ENERGY / commitment cost: mean‖energy‖² = mean‖Z − C‖² ──────
        # Keeps the energized codebook Z near the codebook C (the "energy cost"), so
        # the model defaults to v12 unless adapting earns more reward than it costs.
        # Plain valid mask m (encoder-intrinsic regularizer, not a TD error).
        loss_energy = logits_t.new_zeros(())
        if energy_cost_seq is not None and cfg.energy_coef > 0.0:
            energy_cost_seq = torch.where(m.bool(), energy_cost_seq, torch.zeros_like(energy_cost_seq))
            denom_m = m.sum().clamp(min=1.0)
            loss_energy = (energy_cost_seq * m).sum() / denom_m
            accumulated["loss_energy"] += float(loss_energy.detach().item())
            accumulated["energy_dev"] = max(accumulated["energy_dev"],
                                            float((energy_cost_seq * m).max().detach().item()))

        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive + cfg.energy_coef * loss_energy)
    ...
    # ── v15 codebook EMA: C ← C + r·mean(energy) (EMA toward Z), no_grad ────
    # The slow consolidation ON TOP of the gradient-learned codebook. Uses the last
    # epoch's detached energy, masked to valid steps.
    ema_fn = getattr(getattr(model, "encoder", None), "ema_update_codebook", None)
    if energy_for_ema is not None and cfg.codebook_ema_rate > 0.0 and ema_fn is not None:
        with torch.no_grad():
            vmask = batch.valid_mask.unsqueeze(-1).unsqueeze(-1)          # (B,T,1,1)
            denom_steps = batch.valid_mask.sum().clamp(min=1.0)
            mean_energy = (energy_for_ema * vmask).sum(dim=(0, 1)) / denom_steps   # (N,d)
            if torch.isfinite(mean_energy).all():
                increment = ema_fn(mean_energy, cfg.codebook_ema_rate)    # C += r·mean(energy)
    ...
    LOSS_KEYS = {"loss_policy", "loss_value", "loss_entropy", "loss_contrastive",
                 "loss_energy", "loss_total", "approx_kl"}
    result = {}
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if (it + 1) % log_every == 0:
            ...
            en_str = (f"  L_eng={log.get('loss_energy', 0.0):.4f} edev={log.get('energy_dev', 0.0):.3f}"
                      if cfg.energy_coef > 0 else "")
            if cfg.codebook_ema_rate > 0:
                en_str += f" cbΔ={log.get('codebook_drift', 0.0):.2e}"
            print(
                ...
                f"H={-log['loss_entropy']:.3f}  KL={log['approx_kl']:.4f}{en_str}  "
                f"upd={int(n_upd)}  gmax={max_g:.2e}{buf_str}{skip_flag}"
            )

        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            os.makedirs(checkpoint_dir, exist_ok=True)
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v15_rl_latest.pt")
