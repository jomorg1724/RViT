...
yloc=S.loc10(loc); yocc=S.occ(loc)
...
decH_loc,nH=S.train_decoder(Xtr_H,yloc2,10)
dech1_occ,nh1=S.train_decoder(Xtr_h1,yocc2,2)
decAct_loc,nAl=S.train_decoder(Xtr_act,yloc2,10)
decAct_occ,nAo=S.train_decoder(Xtr_act,yocc2,2)

# ── shared test set (mixed) ─────────────────────────────────────────────────────────────────────────
print("[gen] test videos...",flush=True)
Vte,tloc,tdl=S.gen_trials(0,1.0,NTE,p_change=0.5,seed=2); tyloc=S.loc10(tloc); tyocc=S.occ(tloc)
...
for c,(ck,cn) in enumerate(order):
    for ri,t in enumerate((5,6)):
        H,_,_=feats(cache[ck],t); yhat=S.dec_predict(decH_loc,nH,H); M=S.confmat(tyloc,yhat,10)
...
for c,(ck,cn) in enumerate([("natural","no mod."),("enhance",r"$\alpha_1{=}1$")]):
    for ri,t in enumerate((5,6)):
        ...
        M=S.confmat(tyloc,S.dec_predict(decAct_loc,nAl,act),10); cm_plot(ax[ri,c],M,S.LOCNAMES,f"loc: {cn} t={t}")
