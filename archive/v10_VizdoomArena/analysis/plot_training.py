"""
Plot V10 training curves from the trainer's history.jsonl.

    .venv/bin/python v10_VizdoomArena/analysis/plot_training.py \
        [--history ~/rvit_plus_checkpoints/v10_vizdoom_arena/history.jsonl]
"""
...
def main(argv=None) -> int:
    ...
    p.add_argument("--history", default=os.path.expanduser(
        "~/rvit_plus_checkpoints/v10_vizdoom_arena/history.jsonl"))
    ...
    for ax, (key, title) in zip(axes.flat, panels):
        ...
        ax.grid(alpha=0.3)
    fig.suptitle(f"V10 arena training — {len(rows)} iterations, "
