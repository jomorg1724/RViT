...
def _to_conv_input(recurrent_states: List[torch.Tensor]) -> torch.Tensor:
    """Concatenate the recurrent states on the FEATURE axis and lay them out as
    (B, Σd_mem, N): features = conv channels, tokens = the conv axis.

    v8_part3 passes [H1, H2] → 2·d_mem channels, so at every token position the
    conv sees both the fresh sensory memory (H1) and the deep/codebook memory (H2).
    A single-element list recovers v8's H2-only readout."""
    x = torch.cat(recurrent_states, dim=-1)                 # (B, N, Σd_mem)
    return x.transpose(1, 2).contiguous()                   # (B, Σd_mem, N)
