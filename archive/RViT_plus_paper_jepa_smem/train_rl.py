...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--d-mem", type=int, default=1024,
                   help="xLSTM internal cell width per patch (default 1024; try 512/256/128).")
    p.add_argument("--softmax-memory", action="store_true",
                   help="externalise the JEPA student's per-head softmax (jepa_heads×jepa_proto_dim) "
                        "as the recurrent memory: it feeds the cross-attention K/V AND the readout "
                        "(crossattn1 only). The xLSTM keeps its continuous cell internally.")
...
def main() -> None:
    ...
    # softmax-memory IS the JEPA student's distribution, so the distillation must be on (jepa_coef>0):
    # it's what shapes the memory, and it keeps model/PPOConfig head-counts consistent.
    if args.softmax_memory and args.jepa_coef <= 0:
        raise SystemExit("--softmax-memory requires --jepa-coef > 0 (the JEPA loss shapes the memory)")
    ...
    model_kwargs = dict(
        ...
        softmax_memory=args.softmax_memory,
    )
