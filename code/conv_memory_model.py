...
class ConvStem(nn.Module):
    """Full-image conv processing: 50x50x3 -> [C, 16, 16] (downsample + interpolate)."""

    def __init__(self, channels: int = 128, map_size: int = 16):
        super().__init__()
        self.map_size = map_size
        c1 = max(channels // 4, 8)
        c2 = max(channels // 2, 8)
        self.net = nn.Sequential(
            nn.Conv2d(3, c1, 7, stride=2, padding=3, bias=False), nn.GELU(),  # 25x25
...
class ConvAttentionBlock(nn.Module):
    ...
    def __init__(self, c: int = 128):
        ...
        self.W_kx = nn.Conv2d(c, c, 1, bias=False)
        self.W_kh = nn.Conv2d(c, c, 1, bias=False)
        self.W_vx = nn.Conv2d(c, c, 1, bias=False)
...
class ConvMemoryModel(nn.Module):
    ...
    def step(self, X_t: torch.Tensor, state):
        H1, H2 = state
        Z, att_vis = self.vision(X_t, H1, H2)
        H1_new, H2_new = self.memory(Z, H1)
        R = torch.cat([H1_new, H2_new, Z, att_vis], dim=1)      # (B, 4C, 16, 16)
        return R, (H1_new, H2_new)

    def forward_seq(self, obs: torch.Tensor) -> torch.Tensor:
        """obs: (B, T, 50, 50, 3) -> R_seq (B, T, 4C, 16, 16)."""
        B, T = obs.shape[:2]
        H1, H2 = self.init_state(B, obs.device, obs.dtype)
        Rs = []
        for t in range(T):
            frame = obs[:, t].permute(0, 3, 1, 2).contiguous()  # (B,3,50,50)
            X_t = self.stem(frame)
            R, (H1, H2) = self.step(X_t, (H1, H2))
            Rs.append(R)
