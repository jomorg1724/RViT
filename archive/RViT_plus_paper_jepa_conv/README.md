# RViT+ — CONV front-end + feedback-variant JEPA (`_conv`)

**This directory (built 2026-06-29):** the no-VAE / cross-attn / JEPA architecture with a **capable
small SE-ResNet conv front-end** (`conv_frontend.py`, `--conv-frontend`) replacing the from-scratch
VAE-shaped encoder: per 25×25×3 patch → stem + 3 SE-residual stages (GroupNorm, SiLU, squeeze-excite,
progressive downsample) → global-avg-pool → RMSNorm → 128-d; token stays `[emb128‖pos4‖temporal8]=140`
(drop-in, shared across patches). ~594K front-end params. Trained END-TO-END (no VAE checkpoint).

Feedback options (`--feedback`) include, beyond the paper's `multiplicative`/`film`/`crossattn1`:
- **`affine`** — matrix-valued FiLM: `X' = (I+ΔΓ(H))·X + β(H)`, ΔΓ a full d×d scale matrix.
- **`affine_ew`** — ELEMENT-WISE affine: `X' = γ(H)⊙X + β(H)`, γ and β length-d VECTORS from feedback,
  **no `1+` identity term** (scale init=1 / shift init=0 for a live start); then self-attention.

Canonical runs (plain 7-step vda4 = colour value + proportion validity, d_mem=128, JEPA):
```
# cross-attention feedback
python RViT_plus_paper_jepa_conv/train_rl.py --device mps --init-mode fresh --T 7 \
  --min-change-time 5 --max-change-time 5 --cell xlstm --feedback crossattn1 \
  --conv-frontend --jepa-coef 0.5 --d-mem 128 --checkpoint-dir ~/rvit_plus_checkpoints/paper_jepa_conv_d128
# element-wise affine feedback
python RViT_plus_paper_jepa_conv/train_rl.py --device mps --init-mode fresh --T 7 \
  --min-change-time 5 --max-change-time 5 --cell xlstm --feedback affine_ew \
  --conv-frontend --jepa-coef 0.5 --d-mem 128 --checkpoint-dir ~/rvit_plus_checkpoints/paper_jepa_conv_affine_ew
```

---

## (Inherited) RViT+ FiLM — battery reproduction scaffold
