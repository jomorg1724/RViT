...
Usage:
    .venv/bin/python RViT_plus_recur/train_rl.py
    .venv/bin/python RViT_plus_recur/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_recur.config.loader import (
    DEFAULT_CONFIG_PATH, cfg_get, load_checkpoint_weights, load_config,
    print_resolved_config,
)
from RViT_plus_recur.env import ChangeDetectionEnv
from RViT_plus_recur.model import RViTPlusRecurModel
from RViT_plus_recur.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--conv-embed", action="store_true",
                   help="Use a per-patch CONV stem (Conv2d x3 + Linear) to embed each patch to "
                        "d_model, instead of the conv-free reshape+MLP. Pairs with --patch-size 25 "
                        "(2x2 grid) and --d-model 256.")
    ...
    p.add_argument("--kl-beta", type=float, default=cfg_get(cfg, "ppo.kl_beta", 1.0e-4),
                   help="Weight on the VAE KL penalty for the recurrent latent (small by design).")
    ...
    model = RViTPlusRecurModel(**model_kwargs).to(device)

    print(f"[diag] RECUR model: single recurrent cross-attn transformer ({args.tx_heads}h) | "
          f"Q=Q(H), K=V=[X‖H], residual H + FFN + OUTPUT LN | H_new → next state AND actor+critic | "
          f"4-patch MLP, no VAE/conv/lstm, d_model=d_mem={args.d_model}")
    print(f"[diag] patch_size={args.patch_size} → n_tokens={model.n_tokens} (4 patches, no CLS)")
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_recur_rl_final.pt")
