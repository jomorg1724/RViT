...
[[ "$SEED" == "0" ]] || { printf 'ERROR: paired-pilot-v1 production is frozen to seed 0\n' >&2; exit 2; }
: "${VDA_PAIR_ID:?VDA_PAIR_ID must be set by queue_seed0_pair_v1.sh}"
: "${VDA_PAIR_RUNTIME_SHA256:?VDA_PAIR_RUNTIME_SHA256 must be set by queue_seed0_pair_v1.sh}"
...
[[ "$CURRENT_RUNTIME_SHA256" == "$VDA_PAIR_RUNTIME_SHA256" ]] || {
  printf 'ERROR: runtime/GPU fingerprint changed before production launch: %s != %s\n' \
...
CHECKPOINT_DIR="/workspace/vda4_memory_noise_grid2x2_crossattn1_${CONDITION_ID}_seed${SEED}_production_v1_${RUN_STAMP}"
