"""
RViTPlusV12Part3Model — conv-free, RL-only recurrent attention model with an EMA
codebook written by a GATED IMAGE-CONTENT STREAM. The codebook C_base is a buffer
(no gradient) consolidated by a slow VQ-VAE-style EMA; the content written into it
comes from a dedicated transformer + LSTM ("stream 3") whose VALUES are the image.

Per frame the encoder runs (see tx_lstm_encoder.py):

  (1) WRITE STREAM — image content → gated codebook write:
        Q3 = W_q3·C_base ;  K3 = W_k3·H3 ;  V3 = W_v3·X
        Z3 = W_o3·softmax(Q3 K3ᵀ/√d) V3 + FFN3
        H3 ← LSTM3(Z3) ;  gate = σ(W_g·H3) ;  write = gate ⊙ H3
  ...
  (2) READOUT — soft attention over the written codebook C_t (the v12 mechanism):
        ...
        Z = W_o·(softmax(QKᵀ/√d) @ C_t) + FFN ;  H2 ← LSTM2(X+Z) ;  Z → actor AND critic

  (3) CONSOLIDATION — VQ-VAE EMA toward the WRITTEN codebook C_t (trainer, no_grad):
        C_base ← (1−r)·C_base + r·mean(C_t)   = C_base + r·mean(gate⊙H3)
      Target is C_t (not the write alone) so C_base keeps its scale and ACCUMULATES
      the gated write. The gate is the throttle (closing it returns C_t→C_base); a
      small warmed-in penalty on mean(gate) is the energy cost of writing.

C_base is a BUFFER (optimizer never touches it; inits at ZERO, built only by the EMA).
Perception is gradient-grounded through the write (Z→C_t→write→H3→LSTM3→W_v3·X), so it
is reliable from the start — what avoids the always-WAIT collapse. forward_rl_sequence
exposes Ct_seq (B,T,N,d, detached, the EMA target) + gate_seq (B,T, differentiable, the
energy cost). The recurrent State is (Hs,Cs) with Hs=[H2, H3] (two LSTM states).
"""
...
class RViTPlusV12Part3Model(nn.Module):
    ...
    tx_heads         : heads in BOTH the write and readout attention blocks (default 1).
    tx_layers        : number of attention blocks (config parity; fixed at 1 each).
    n_lstm           : config parity (the architecture is fixed: LSTM2 + LSTM3).
    ...
    def forward_rl_sequence(
        ...
        for t in range(T):
            ...
            gate_seq.append(self.encoder.last_gate_cost)       # (B,) differentiable mean(gate) — energy cost
            Ct_seq.append(self.encoder.last_Ct)                # (B,N,d) DETACHED written codebook C_t (EMA target)
            actor_logits, q_dist, V_dist, V_scalar = self._run_heads(rec_states)
        ...
        return {
            ...
            "gate_seq":         torch.stack(gate_seq, dim=1),           # (B, T) mean(gate) — energy cost
            "Ct_seq":           torch.stack(Ct_seq, dim=1),            # (B, T, N, d) DETACHED — EMA target C_t
