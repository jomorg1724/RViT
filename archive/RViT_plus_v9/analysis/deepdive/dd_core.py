...
DEFAULT_CKPT = "/Users/jonathanmorgan/rvit_plus_checkpoints/v9/v9_analysis_snapshot.pt"
...
def cross_attn_forward_step(
    ...
    """Bias-injectable replica of TxLSTMEncoder.forward_step (v9 memory-query).

    Block 0 (the v9 design — MUST mirror tx_lstm_encoder.py exactly):
        kv_mems = [ Hs[k] + mem_pos_emb + mem_tag[k-1] ]  for k = 1..n_lstm-1
        kv      = norm_kv( cat([X] + kv_mems) )           # (B, 2N, d)
        q       = norm_q( Hs[0] + mem_pos_emb )           # N MEMORY queries
        a, aw   = attn(q, kv, kv)
        Z       = Hs[0] + drop(a)                         # H1 residual
        Z       = Z + ffn(norm_ff(Z))
    Later blocks (tx_layers > 1): stream queries over [stream ++ kv_mems],
    residual on the stream. Then the stacked LSTMCells advance as in the stock
    forward_step.

    attn_bias : optional (tx_layers, n_heads, 2N) additive per-key bias added to
                the pre-softmax logits of that block (broadcast over the N
                queries). Keys are ordered [patch(0:N) ++ H2(N:2N)].
    Returns (new_state, rec[, attn_per_block]); attn_per_block[i]: (B, heads, N, 2N).
    """
    Hs, Cs = list(prev_state[0]), list(prev_state[1])
    B, N = tokens.shape[0], encoder.n_tokens

    # Key/value memories = the LATER carried states (H2, ...), tagged.
    kv_mems = [Hs[k] + encoder.mem_pos_emb + encoder.mem_tag[k - 1]
               for k in range(1, encoder.n_lstm)]

    attn_per_block: List[Optional[torch.Tensor]] = []
    stream = None
    for li, blk in enumerate(encoder.blocks):
        if li == 0:
            kv = blk.norm_kv(torch.cat([tokens] + kv_mems, dim=1))   # (B, 2N, d)
            q_in = blk.norm_q(Hs[0] + encoder.mem_pos_emb)           # memory queries
            res = Hs[0]                                              # H1 residual
        else:
            kv = blk.norm_kv(torch.cat([stream] + kv_mems, dim=1))
            q_in = blk.norm_q(stream)
            res = stream
...
def make_attn_bias(model, device, *, layer: int, head: int, region: str,
    ...
    """Build a (tx_layers, n_heads, 2N) additive attention-logit bias targeting
    ONE (layer, head). v9 keys are ordered [patch(0:N) ++ H2(N:2N)] — H1 is the
    query/residual, not a key. region ∈ {'patch','quad','memory','memquad'}:
      'patch'   → bias all N patch keys by `value`
      'quad'    → bias the patch keys of `quad` by `value`
      'memory'  → bias the N H2 memory keys by `value`
      'memquad' → bias the H2 keys of `quad` by `value`
    """
    enc = model.encoder
    nL, H, N = enc.tx_layers, enc.n_heads, model.n_tokens
    n_kv = (1 + (enc.n_lstm - 1)) * N                       # 2N for n_lstm=2
    bias = torch.zeros(nL, H, n_kv, device=device)
    if region == "patch":
        bias[layer, head, :N] = value
    elif region == "memory":
        bias[layer, head, N:n_kv] = value
    elif region == "quad":
        ...
        assert quad is not None
        qi = quadrant_token_indices(model.patch_embed.grid_h, model.patch_embed.grid_w)[quad]
        for k in range(1, enc.n_lstm):                      # H2 block, …
