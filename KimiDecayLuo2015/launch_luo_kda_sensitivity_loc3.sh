#!/usr/bin/env bash
# Luo–Maunsell 2015 sensitivity, high_loc=3, KDA conv-memory agent. NOT launched yet.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_ROOT="${RUN_ROOT:-/workspace/runs/luo2015_kda_c64_loc3_seed0}"
DEVICE="${DEVICE:-cuda}"
mkdir -p "$RUN_ROOT/checkpoints"

exec "$HERE/.venv/bin/python" -u "$HERE/train_rl.py" \
  --task luo2015_sensitivity --curriculum --high-loc 3 \
