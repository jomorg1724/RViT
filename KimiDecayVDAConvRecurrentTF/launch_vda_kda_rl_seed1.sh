#!/usr/bin/env bash
# VDA16 KDA + PAC/QR RL, seed 1, 250k episodes. Fresh (not the supervised ckpt).
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_ROOT="${RUN_ROOT:-/workspace/runs/vda16_kda_rl_c128_seed1}"
DEVICE="${DEVICE:-cuda}"
...
exec "$HERE/.venv/bin/python" -u "$HERE/train_rl_vda.py" \
  ...
  --iters 31250 --episodes-per-iter 8 --seed 1 \
