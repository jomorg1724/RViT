...
class BaseChangeDetectionEnv(gym.Env):
    def __init__(
        ...
        color_values=None,
        image_size: int = 50,
        # When True, an uncued change on a change trial is drawn uniformly over the
        # NON-cued cells only, so the displayed ring proportion equals the true
        # P(change at cued) (a clean Posner validity). Default False preserves the
        # original behaviour (uniform over ALL cells incl. the cued one).
        exclude_cued_in_uniform: bool = False,
        ...
        self.exclude_cued_in_uniform = bool(exclude_cued_in_uniform)
    ...
    def reset(self):
        ...
        # Per-location signal gain on the orientation change (1.0 = no manipulation).
        # A subclass may concentrate discriminability at a location (e.g. the Luo &
        # Maunsell sensitivity session raises d' at the attended cell) by scaling this;
        # it is applied to Δ in _next_observation. Set AFTER super().reset() in overrides.
        self.loc_signal = np.ones(self.n_stim, dtype=np.float32)
    ...
    def _draw_change_index(self) -> int:
        ...
        if self.exclude_cued_in_uniform and self.n_stim > 1:
    ...
    def _current_feature(self, i: int, L: int) -> float:
        """Per-cell feature value (Gabor orientation OR motion direction), with the change and
        per-location signal gain applied from change_time on. Shared by every stimulus renderer so
        the change/curriculum/loc_signal logic lives in ONE place regardless of what pixels draw."""
        f = self.orientations[i]
        if L >= self.change_time and self.change_true == 1 and self.change_index == i:
            f += self.orientation_change * float(self.loc_signal[i])
        return f

    def _render_stimuli(self, obs, L):
        """Draw the per-cell stimulus for a stimulus frame. DEFAULT: a static Gabor at the cell's
        orientation + per-frame orientation noise (the wobble). Subclasses override ONLY this to
        render a different stimulus (e.g. a random-dot motion field) without touching the
        timeline / cue / change / reward / curriculum logic."""
    ...
    def _is_correct_rejection(self) -> bool:
        """Whether waiting to the trial end should be rewarded as a correct rejection.
        Base: only true no-change trials. Subclasses widen this — e.g. Krauzlis rewards
        correctly WITHHOLDING on an unreportable (uncued/distractor) change, which is a
        correct rejection of the to-be-ignored event, not a miss."""
        return self.change_true == 0
    ...
    def step(self, action):
        ...
        if self.t >= self.T and not done:
            ...
            if self._is_correct_rejection():                # correct rejection / correct withhold
                reward = self._reward_value()
