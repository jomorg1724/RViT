...
from RViT_plus.attention import FeedbackTransformer
from RViT_plus.memory import GridCellRNN_LSTM
...
def test_feedback_transformer():
    print("\n[2] FeedbackTransformer (conv spatial / channel attention) ---")
    B, D, H, W = 2, 64, 12, 12
    ...
    _check("FT.attn shape (B, C, 1, 1) — channel attention",
           tuple(out["attn"].shape) == (B, D, 1, 1))
    ...
    # Channel-attention weights sum to 1 within each of the n_heads groups.
    print("\n[3] FT channel attention weights ---")
    a = out["attn"].view(B, 4, D // 4)
    sums = a.sum(dim=-1)              # (B, n_heads)
    _check("attn sums to 1 per head",
           torch.allclose(sums, torch.ones_like(sums), atol=1e-5),
           detail=f"head-sum mean={sums.mean().item():.4f}")
...
def test_gridcell_rnn():
    ...
    _check("Cell.attn shape (B, C, 1, 1) — channel attention",
           tuple(out["attn"].shape) == (B, st_ch, 1, 1))
