#!/usr/bin/env bash
set -euo pipefail
if [[ "$#" -ne 5 ]]; then echo "usage: $0 ARCHIVE EXPECTED_SHA CELL RUN_ROOT SOURCE_DIR" >&2; exit 2; fi
ARCHIVE="$1"; EXPECTED_SHA="$2"; CELL="$3"; RUN_ROOT="$4"; SOURCE_DIR="$5"
case "$CELL" in
 sensitivity_loc0|sensitivity_loc3) TASK=luo2015_sensitivity ;;
 criterion_loc0|criterion_loc3) TASK=luo2015_criterion ;;
...
PY
then echo "refusing non-empty source dir" >&2; exit 11; fi
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON_BIN" -m pytest -q \
 ...
 tests/test_luo2015_corrected_grid2_memnoise032_runpod.py \
 tests/test_luo2015_fresh_gamma070_reward_matrix_runpod.py
CANARY="/workspace/canary_luo_gamma070_${CELL}"; rm -rf "$CANARY"
env CELL="$CELL" RUN_ROOT="$CANARY" PYTHON_BIN="$PYTHON_BIN" ITERS=1 SAVE_EVERY=1 DEVICE=cuda \
 bash experiments/luo2015_episodic/fresh_grid2_memnoise032_gamma070_reward_matrix_runpod/launch_cell.sh \
 > "/workspace/canary_${CELL}.log" 2>&1
FINAL="$CANARY/rvit_paper_${TASK}_final.pt"; test -s "$FINAL"; CANARY_SHA="$(sha256sum "$FINAL" | cut -d' ' -f1)"
mkdir -p "$RUN_ROOT"
printf '{"source_archive_sha256":"%s","cell":"%s","grid":[2,2],"memory_noise_std":0.32,"gamma":0.7,"initialization":"fresh","canary_checkpoint_sha256":"%s"}\n' "$ACTUAL_SHA" "$CELL" "$CANARY_SHA" > "$RUN_ROOT/deployment_contract.json"
(
 set +e
 env CELL="$CELL" RUN_ROOT="$RUN_ROOT" PYTHON_BIN="$PYTHON_BIN" ITERS=20000 SAVE_EVERY=50 DEVICE=cuda \
  bash experiments/luo2015_episodic/fresh_grid2_memnoise032_gamma070_reward_matrix_runpod/launch_cell.sh
...
printf 'BOOTSTRAP_OK cell=%s gamma=0.7 source_sha256=%s canary_sha256=%s supervisor_pid=%s run_root=%s\n' "$CELL" "$ACTUAL_SHA" "$CANARY_SHA" "$pid" "$RUN_ROOT"
