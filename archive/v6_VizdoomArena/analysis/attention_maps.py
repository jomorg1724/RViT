"""
Attention interpretability suite for a trained V6 arena model.
...
Usage:
    .venv/bin/python v6_VizdoomArena/analysis/attention_maps.py \
        --checkpoint ~/rvit_plus_checkpoints/v6_vizdoom_arena/v6_latest.pt \
...
from v6_VizdoomArena.env import OBS_H, OBS_W, VizdoomArenaEnv
...
def main(argv=None) -> int:
    ...
    enc_layout = model.encoder.key_layout()
    ...
    with torch.no_grad():
        for _ep in range(args.episodes):
            ...
            for _t in range(args.max_steps):
                ...
                enc_grids = []
                for k in range(L):
                    grid, groups = _grid_attention(out["enc_attn"][k].cpu(), enc_layout, gh, gw)
                ...
                if mask.any():
                    ...
                    if args.causal:
                        ...
                        for sign, tag in ((+1.0, "boost"), (-1.0, "suppress")):
                            bias_vec = torch.zeros(model.encoder.n_keys, device=device)
                            for name, (lo, hi) in enc_layout.items():
                                if hi - lo == gh * gw:          # patch-aligned spans
                                    bias_vec[lo:hi][flat] = sign * args.bias
                            outb = model.rl_step(
                                x, f, prev_states,
                                attn_bias={"enc": [bias_vec] * L})
