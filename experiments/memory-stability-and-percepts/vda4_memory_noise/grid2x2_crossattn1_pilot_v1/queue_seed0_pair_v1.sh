...
# Preflight both registered production conditions before training either one.
...
export VDA_PAIR_ID="production_$("$PYTHON_BIN" -c 'from datetime import datetime,timezone; import uuid; print(datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")+"_"+uuid.uuid4().hex[:12])')"
export VDA_PAIR_RUNTIME_SHA256="$(runtime_fingerprint)"
printf 'QUEUE_PREFLIGHT_PASS|run_kind=production|conditions=2|seed=0|pair_id=%s|runtime_sha256=%s|no_result_dependent_stopping=true\n' \
  "$VDA_PAIR_ID" "$VDA_PAIR_RUNTIME_SHA256"

# This loop is unconditional with respect to training metrics or scientific results.
# set -e may stop only when a launcher/trainer engineering contract fails.
for MEMORY_NOISE_STD in 0.0 0.5; do
  CURRENT_RUNTIME_SHA256="$(runtime_fingerprint)"
  [[ "$CURRENT_RUNTIME_SHA256" == "$VDA_PAIR_RUNTIME_SHA256" ]] || {
    printf 'ERROR: runtime/GPU fingerprint changed inside production pair\n' >&2
    exit 2
  }
...
CURRENT_RUNTIME_SHA256="$(runtime_fingerprint)"
[[ "$CURRENT_RUNTIME_SHA256" == "$VDA_PAIR_RUNTIME_SHA256" ]] || {
  printf 'ERROR: runtime/GPU fingerprint changed after production pair\n' >&2
  exit 2
}
printf 'QUEUE_COMPLETE|run_kind=production|conditions=2|seed=0|pair_id=%s|heldout_pending=true\n' "$VDA_PAIR_ID"
