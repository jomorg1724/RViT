...
try:
    ...
    from model import RViTPlusV12Part3Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # Gate ("energy") cost (v12_part3): penalize the write-gate openness mean(gate)
    # (gate_seq), so the default is a CLOSED gate (codebook = resting C_base) and
    # opening it to write costs. WARMED IN — silent for the first gate_warmup_iters
    # (writes flow freely to ground the readout), then ramped to gate_coef by
    # gate_ramp_end_iter; once content has consolidated into C_base, this closes the
    # gate so the write relaxes. 0 disables.
    gate_coef: float = 1.0e-3
    gate_warmup_iters: int = 2000
    gate_ramp_end_iter: int = 4000

    # Codebook EMA consolidation rate (v12_part3): the codebook C_base is a BUFFER
    # moved only by a slow VQ-VAE-style EMA toward the WRITTEN codebook C_t,
    # C_base ← (1−r)·C_base + r·mean(C_t) = C_base + r·mean(gate⊙H3), once per update.
    # VERY SMALL (JEPA-like). 0 disables (codebook frozen at init). No-op for models
    # lacking 'Ct_seq' / encoder.ema_update_codebook.
    codebook_ema_rate: float = 2.0e-3
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV12Part3Model] = None,
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_gate": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0, "gate_open": 0.0, "codebook_drift": 0.0,
        "n_updates": 0.0, "n_loss_evals": 0.0, "n_skipped": 0.0, "max_grad_elem": 0.0,
        "per_episode_priority": None,
    }
    Ct_for_ema = None   # last epoch's detached Ct_seq (B,T,N,d) — the EMA target C_t
    # WARMED-IN gate ("energy") cost: silent until gate_warmup_iters, then ramps to
    # gate_coef by gate_ramp_end_iter (lets the write flow freely to ground early).
    _w0, _w1 = cfg.gate_warmup_iters, max(cfg.gate_ramp_end_iter, cfg.gate_warmup_iters + 1)
    eff_gate_coef = cfg.gate_coef * min(max((global_iter - _w0) / (_w1 - _w0), 0.0), 1.0)
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        gate_seq = out.get("gate_seq", None)        # (B, T) differentiable mean(gate) or None
        Ct_for_ema = out.get("Ct_seq", None)        # (B,T,N,d) detached written codebook — keep last epoch's
        ...
        # ── Gate ("energy") cost (v12_part3), plain valid mask m, NOT PER mw ───
        # WARMED-IN penalty on the write-gate openness mean(gate) — silent early
        # (eff_gate_coef ramps from 0 so the write flows and grounds the readout),
        # then closes the gate once content has consolidated into C_base, so the
        # write relaxes (C_t → C_base). Gradient flows into the write stream + gate.
        denom_m = m.sum().clamp(min=1.0)
        loss_gate = logits_t.new_zeros(())
        if gate_seq is not None and cfg.gate_coef > 0.0:
            gate_seq = torch.where(m.bool(), gate_seq, torch.zeros_like(gate_seq))  # NaN-safe pad
            loss_gate = (gate_seq * m).sum() / denom_m
            accumulated["loss_gate"] += float(loss_gate.detach().item())
            accumulated["gate_open"] = max(
                accumulated["gate_open"], float((gate_seq * m).max().detach().item()))

        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive + eff_gate_coef * loss_gate)
    ...
    # ── Codebook EMA consolidation (v12_part3) ─────────────────────────────
    # AFTER the gradient epochs, slowly move the codebook (a buffer) toward the
    # WRITTEN codebook C_t: C_base ← (1−r)·C_base + r·mean_{valid B,T}(C_t) =
    # C_base + r·mean(gate⊙H3) (VQ-VAE EMA toward C_t — not the write alone — so the
    # codebook keeps its scale and accumulates the gated write). Non-gradient; uses
    # the last epoch's detached C_t, masked to valid steps.
    ema_fn = getattr(getattr(model, "encoder", None), "ema_update_codebook", None)
    if Ct_for_ema is not None and cfg.codebook_ema_rate > 0.0 and ema_fn is not None:
        with torch.no_grad():
            vmask = batch.valid_mask.unsqueeze(-1).unsqueeze(-1)          # (B,T,1,1)
            denom_steps = batch.valid_mask.sum().clamp(min=1.0)
            mean_Ct = (Ct_for_ema * vmask).sum(dim=(0, 1)) / denom_steps  # (N,d) target C_t
            # Finite-guard the buffer write (the gradient step is isfinite-skipped,
            # but the EMA writes the codebook IN-PLACE + into the checkpoint).
            if torch.isfinite(mean_Ct).all():
                increment = ema_fn(mean_Ct, cfg.codebook_ema_rate)        # C_base ← (1-r)C_base + r·mean(C_t)
                accumulated["codebook_drift"] = float(increment.abs().mean().item())
            else:
                accumulated["n_skipped"] += 1

    n_evals = max(accumulated["n_loss_evals"], 1.0)
    LOSS_KEYS = {"loss_policy", "loss_value", "loss_entropy", "loss_contrastive",
                 "loss_gate", "loss_total", "approx_kl"}
    result = {}
...
def train(
    ...
    *,
    ...
    if start_iter > 0:
        print(f"[resume] continuing from global iter {start_iter} — burn-in "
              f"{'SKIPPED (already past it)' if start_iter >= cfg.burn_in_iters else 'partial'}")
    t_start = time.time()
    for it in range(n_iterations):
        g = start_iter + it                      # global (cumulative) iteration across resumes
        in_burn_in = g < cfg.burn_in_iters       # → no re-burn-in when resuming a trained model
        ...
        log = {"iter": g, **rollout_stats, **update_stats,
        ...
        if (it + 1) % log_every == 0:
            ...
            eng_str = ""
            if cfg.gate_coef > 0:
                eng_str += f"  gate={log.get('gate_open', 0.0):.3f} L_gate={log.get('loss_gate', 0.0):.4f}"
            if cfg.codebook_ema_rate > 0:
                eng_str += f" cbΔ={log.get('codebook_drift', 0.0):.2e}"
            print(
                f"[{phase}{g:5d} | {elapsed:6.1f}s] "
                ...
                f"H={-log['loss_entropy']:.3f}  KL={log['approx_kl']:.4f}{eng_str}  "
                f"upd={int(n_upd)}  gmax={max_g:.2e}{buf_str}{skip_flag}"
            )

        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            os.makedirs(checkpoint_dir, exist_ok=True)
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v12_part3_rl_latest.pt")
            torch.save({"iter": g, "model_state_dict": model.state_dict()}, ckpt_path)
