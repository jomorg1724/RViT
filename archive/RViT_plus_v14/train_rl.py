...
Usage:
    .venv/bin/python RViT_plus_v14/train_rl.py
    .venv/bin/python RViT_plus_v14/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v14.config.loader import (
...
from RViT_plus_v14.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--ss-coef", type=float, default=cfg_get(cfg, "ppo.ss_coef", 1.0),
                   help="Weight on the concurrent self-supervised predictive-coding loss (Module 1). "
                        "Detaches route it ONLY into Module 1. 0 disables.")
    p.add_argument("--pc-horizon", type=int, default=cfg_get(cfg, "model.pc_horizon", 1),
                   help="Predict H1 this many steps ahead in the JEPA latent loss.")
    ...
    model_kwargs = dict(
        ...
        d_latent=cfg_get(cfg, "model.d_latent", args.d_model), pc_horizon=args.pc_horizon,
    )
    model = RViTPlusV14Model(**model_kwargs).to(device)
    ...
    # v14 trains BOTH modules CONCURRENTLY (no two-phase freeze): the RL loss trains
    # Module 2 (+heads) and the self-supervised predictive-coding loss (ss_coef) trains
    # Module 1, firewalled by the encoder's cross-detachments.
    print(f"[v14] CONCURRENT training: Module 1 (self-supervised PC, ss_coef={args.ss_coef:g}, "
          f"horizon={args.pc_horizon}) ⟂ Module 2 (RL). Cross-detached gradients.")
    ...
    ppo_cfg = PPOConfig(
        ...
        value_coef=args.value_coef, entropy_coef=args.entropy_coef, ss_coef=args.ss_coef,
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v14_rl_final.pt")
