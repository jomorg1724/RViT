...
try:
    ...
    from model import RViTPlusV14Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # v14 CONCURRENT self-supervised predictive coding (Module 1). Adds
    # ss_coef·L_SS (JEPA latent prediction on H1) to the loss. The encoder's
    # cross-detachments route L_SS ONLY into Module 1 and L_RL ONLY into Module 2,
    # so the two train concurrently from one loss. 0 disables. No-op if the model's
    # forward_rl_sequence omits 'H1_seq'.
    ss_coef: float = 1.0
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV14Model] = None,
    ...
    accumulated = {
        ...
        "loss_contrastive": 0.0, "loss_ss": 0.0, "loss_total": 0.0,
        "approx_kl": 0.0, "surprise": 0.0,
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        # ── v14 CONCURRENT self-supervised predictive coding (Module 1) ─────
        # JEPA latent prediction on the H1 sequence. The encoder detaches Z2 inside
        # T1, so this gradient flows ONLY into Module 1 (T1, LSTM1, predictor,
        # patch_embed) — never into Module 2 or the heads. Computed on the same
        # re-encoded batch as the RL loss → concurrent training.
        loss_ss = logits_t.new_zeros(())
        H1_seq = out.get("H1_seq", None)
        if H1_seq is not None and cfg.ss_coef > 0.0:
            loss_ss, surprise_seq, _parts = model.encoder.predictive_loss(H1_seq, batch.valid_mask)
            accumulated["loss_ss"] += float(loss_ss.detach().item())
            accumulated["surprise"] = max(accumulated["surprise"],
                                          float((surprise_seq * batch.valid_mask).sum().detach()
                                                / batch.valid_mask.sum().clamp(min=1.0)))

        loss = (loss_policy + cfg.value_coef * loss_value + cfg.entropy_coef * loss_entropy
                + cfg.contrastive_coef * loss_contrastive + cfg.ss_coef * loss_ss)
...
def train(
    ...
    *,
    ...
    # Only the trainable params (Phase 2 freezes the predictive-coding encoder).
    ...
    for it in range(n_iterations):
        ...
        if (it + 1) % log_every == 0:
            ...
            ss_str = (f"  L_ss={log.get('loss_ss', 0.0):.3f} surp={log.get('surprise', 0.0):.4f}"
                      if cfg.ss_coef > 0 else "")
            print(
                ...
                f"H={-log['loss_entropy']:.3f}  KL={log['approx_kl']:.4f}{ss_str}  "
                f"upd={int(n_upd)}  gmax={max_g:.2e}{buf_str}{skip_flag}"
            )

        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            os.makedirs(checkpoint_dir, exist_ok=True)
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v14_rl_latest.pt")
