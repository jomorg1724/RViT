...
QA_ROOT = VDA_ROOT / "qa"
SNAPSHOT_ROOT = VDA_ROOT / "review_snapshots" / "final_candidate_20260712_v4"
SNAPSHOT_MANIFEST = SNAPSHOT_ROOT / "SNAPSHOT_MANIFEST.json"
VISUAL_QA = QA_ROOT / "2026-07-12_matched_width_integrated_manuscript_visual_audit.md"
REVIEW_ATTESTATIONS = QA_ROOT / "2026-07-12_final_review_attestations.json"
...
def validate_snapshot_inventory(path: Path) -> dict:
    ...
    for candidate in root.rglob("*"):
        ...
        if stat.S_ISREG(mode) and candidate != path:
...
def validate_final_reviews(source_sha: str, pdf_sha: str, snapshot_sha: str) -> dict:
    ...
    candidate = attestations.get("candidate", {})
    expected_candidate = {
        "snapshot": "final_candidate_20260712_v4",
    ...
    if candidate != expected_candidate:
        raise RuntimeError(f"Review candidate identity mismatch: {candidate}")
    required_verdicts = {
        "feynman": "PASS",
        "hawking": "PASS",
        "tyson": "PASS",
    ...
    reviews = attestations.get("reviews")
    if not isinstance(reviews, list):
        raise RuntimeError("Review attestations must contain a review list")
    by_role = {review.get("role"): review for review in reviews}
    if set(by_role) != set(required_verdicts) or len(by_role) != len(reviews):
        raise RuntimeError("Final review roles are missing, duplicated, or unexpected")
    identities = [review.get("reviewer_identity") for review in reviews]
    if None in identities or len(set(identities)) != len(identities):
        raise RuntimeError("Final reviewer identities must be present and distinct")
    v15_manifest = VDA_ROOT / "matched_width_20260712_production_v15" / "MANIFEST.json"
    for role, verdict in required_verdicts.items():
        ...
        for key, expected in (
            ...
            if review.get(key) != expected:
                raise RuntimeError(f"{role} review has stale {key}: {review.get(key)!r}")
        record_name = review.get("record_path")
        if not isinstance(record_name, str):
            raise RuntimeError(f"{role} review lacks a record path")
        record_relative = Path(record_name)
        if record_relative.is_absolute() or ".." in record_relative.parts:
            raise RuntimeError(f"{role} review record path is unsafe: {record_name!r}")
        record = ROOT / record_relative
        if not record.is_file() or review.get("record_sha256") != sha256(record):
            raise RuntimeError(f"{role} review record is missing or hash-mismatched")
        marker = f"VERDICT[{role}]={verdict}"
        if marker not in record.read_text(encoding="utf-8"):
            raise RuntimeError(f"{role} review record lacks approving marker {marker!r}")
        if role == "artifact" and review.get("artifact_manifest_sha256") != sha256(v15_manifest):
            raise RuntimeError("Artifact review does not bind the current v15 manifest")
    return attestations
...
def main() -> None:
    ...
    artifacts = [
        ...
        VISUAL_QA,
        QA_ROOT / "2026-07-12_integrated_prose_trigger_lint.txt",
        QA_ROOT / "2026-07-12_integrated_prose_trigger_disposition.md",
    ...
    manifest = {
        ...
        "verification": {
            ...
            "final_review_roles": sorted(review["role"] for review in reviews["reviews"]),
        },
