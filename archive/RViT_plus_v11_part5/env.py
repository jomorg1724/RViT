...
class ChangeDetectionEnv(gym.Env):
    def __init__(
        ...
        # FRAME-REPEAT / temporal-dilation (see the standard env): self.t counts PHYSICAL
        # steps; the rendered frame is the LOGICAL frame L = self.t // frame_repeat, held for
        # frame_repeat steps with FROZEN pixels. change_time / distractor_time are LOGICAL.
        # frame_repeat=1 + n_frames=29 reproduces the original distractor env exactly.
        self.n_frames = int(n_frames)
        self.frame_repeat = max(1, int(frame_repeat))
        self.T = self.n_frames * self.frame_repeat
    ...
    def _next_observation(self):
        # FREEZE: render each logical frame once, return identical pixels across its held steps.
    ...
    def _render_frame(self, L):
        ...
        # Per-quadrant orientation offsets active at this LOGICAL frame.
        offsets = [0.0, 0.0, 0.0, 0.0]
        if L >= self.change_time and self.change_true == 1:
            offsets[self.change_index] += self.orientation_change
        if L >= self.distractor_time and self.distractor_true == 1:
