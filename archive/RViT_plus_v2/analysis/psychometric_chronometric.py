"""
Psychometric & chronometric curves for the RViT+ V2 model.
...
Usage:
    .venv/bin/python RViT_plus_v2/analysis/psychometric_chronometric.py \
        --checkpoint RViT_plus_v2/checkpoints/rvit_plus_rl_latest.pt \
...
from RViT_plus_v2.analysis import _behav_utils as bu
...
def plot_exp1(results, out_path, args):
    ...
    fig.suptitle(
        f"V2 signal-strength psychometric / chronometric  ·  ring validity={args.valid_ring}  ·  "
...
def plot_exp2(rows, out_path, args):
    ...
    axR.grid(alpha=0.3)
    fig.suptitle(
        f"V2 validity scaling (cued)  ·  |Δθ|={args.scaling_mag}  ·  change@t={args.change_time}  ·  n={args.n_trials}/cell",
        fontsize=12,
    )
    fig.tight_layout()
    fig.savefig(out_path, dpi=130, bbox_inches="tight")
...
def plot_exp3(rows, out_path, args):
    ...
    axR.grid(alpha=0.3)
    fig.suptitle(
        f"V2 value scaling (cued)  ·  ring={args.valid_ring}  ·  |Δθ|={args.scaling_mag}  ·  n={args.n_trials}/cell",
        fontsize=12,
    )
    fig.tight_layout()
    fig.savefig(out_path, dpi=130, bbox_inches="tight")
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--checkpoint", default="RViT_plus_v2/checkpoints/rvit_plus_rl_latest.pt")
    ap.add_argument("--config", default=None)
    ap.add_argument("--out-dir", default="RViT_plus_v2/analysis/figures")
