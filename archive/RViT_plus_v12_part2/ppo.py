...
try:
    ...
    from model import RViTPlusV12Part2Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # Energetic maintenance cost (v12_part2): penalize the codebook deviation
    # mean‖D_t‖² returned as energy_seq by the model. This is the "cost to
    # maintain" an adapted codebook — with it the agent is incentivized to fall
    # back on the static learned codebook unless deforming it earns enough reward.
    # 0 disables (the energy module can then deform freely). No-op for models
    # whose forward_rl_sequence does not return 'energy_seq'.
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV12Part2Model] = None,
    ...
    for _epoch in range(cfg.n_epochs):
        ...
        energy_seq = out.get("energy_seq", None)    # (B, T) mean‖D_t‖² or None
        ...
        # ── Energetic maintenance cost (v12_part2) ─────────────────────────
        # Penalize the codebook deviation mean‖D_t‖² (energy_seq) averaged over
        # valid steps. Applied in BOTH burn-in and acting phases (it regularizes
        # the encoder dynamics, independent of the actor). Gradient flows into the
        # energy-modulation transformer, pulling the codebook back to static.
        # Use the PLAIN valid mask m (not the PER-weighted mw): ‖D_t‖² is an
        # intrinsic property of the encoder on a trajectory, not a TD error, so it
        # should not be scaled by replay importance-sampling weights.
        loss_energy = logits_t.new_zeros(())
        if energy_seq is not None and cfg.energy_coef > 0.0:
            energy_seq = torch.where(m.bool(), energy_seq, torch.zeros_like(energy_seq))  # NaN-safe pad
            denom_m = m.sum().clamp(min=1.0)
            loss_energy = (energy_seq * m).sum() / denom_m
            accumulated["loss_energy"] += float(loss_energy.detach().item())
            accumulated["energy_dev"] = max(
                accumulated["energy_dev"], float((energy_seq * m).max().detach().item()))
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if (it + 1) % log_every == 0:
            ...
            eng_str = (f"  L_eng={log['loss_energy']:.4f} dev={log.get('energy_dev', 0.0):.3f}"
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v12_part2_rl_latest.pt")
