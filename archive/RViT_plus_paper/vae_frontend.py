...
class VAEPatchFrontEnd(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=2, padding=1)   # 25→13
    ...
    def _encode_patch(self, o: torch.Tensor) -> torch.Tensor:
        """o: (B,1,25,25) → ô (B,128) = o_flat,2 (encoder components 1–4)."""
    ...
    def forward(self, x: torch.Tensor, t: int) -> torch.Tensor:
        ...
        B = x.shape[0]
        # → grayscale luminance (paper is C=1); our env is RGB
        g = x.mean(dim=1, keepdim=True)                                     # (B,1,50,50)
