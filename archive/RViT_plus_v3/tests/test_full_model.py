...
from RViT_plus_v3.latent import VAELatentSampler
from RViT_plus_v3.decoder import IterativeReconstructionDecoder
from RViT_plus_v3.losses import kl_to_unit_gaussian, pixel_recon_loss, slowness_loss
...
def test_encoder():
    ...
    # V3b: multi-head spatial-softmax attention. Per-layer attn is
    # (B, n_heads, H_ℓ, W_ℓ) — one spatial softmax map per head over the grid.
    _check("attn_per_iter[0][0] shape (B, 32, 12, 12)",
           tuple(out["attn_per_iter"][0][0].shape) == (B, 32, 12, 12))
    _check("attn_per_iter[0][1] shape (B, 32, 6, 6)",
           tuple(out["attn_per_iter"][0][1].shape) == (B, 32, 6, 6))
    _check("attn_per_iter[0][2] shape (B, 32, 3, 3)",
           tuple(out["attn_per_iter"][0][2].shape) == (B, 32, 3, 3))
...
def test_full_model():
    ...
    # V3b: multi-head spatial-softmax attention. Layer-0 attn is (B, 32, 12, 12).
    _check("encoder_attn_per_frame[0][0][0] shape (B, 32, 12, 12)",
           tuple(out.encoder_attn_per_frame[0][0][0].shape) == (B, 32, 12, 12))
...
def test_rl_heads_gradient_flow():
    ...
    from RViT_plus_v3.rl_heads import quantile_huber_loss
    B, T = 2, 3
