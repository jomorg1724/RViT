...
\begin{{abstract}}
This analysis tests a precise hypothesis about recurrent visual attention: whether a stronger memory decay is associated with more dynamic learned attention maps. Two preserved VDA4 cross-attention checkpoints were evaluated on identical valid and deliberately forced-invalid videos. ``Activity'' was fixed in advance as frame-to-frame total variation over every query-to-key attention distribution, because total softmax mass is constant by construction. The standard checkpoint averaged {f4(primary['standard_mean'])}; the high-decay checkpoint averaged {f4(primary['high_decay_mean'])}. The paired high-minus-standard difference was {signed(primary['mean_difference'])} (95\% trial-bootstrap interval {interval(primary)}; $n={primary['n']}$ event trials). Thus the high-decay checkpoint was less, not more, dynamic under the primary metric. Behavioral curves, image-versus-memory allocation, selectivity, spatial reorientation, response-stratified traces, and raw query-level maps provide convergent description. The comparison remains associational: the high-decay checkpoint was frozen 5,351 iterations earlier, and only one training seed exists per condition.
\end{{abstract}}
...
\boundary{{The standard checkpoint completed 20,000 training iterations. The high-decay snapshot was frozen at iteration 14,649 while its training process was active; that process later completed, but the requested frozen snapshot was retained. One training seed per checkpoint means the intervals quantify evaluation-trial sampling only, not variation across training runs.}}

\section{{Frozen checkpoints and matched protocol}}
\begin{{table}}[ht]
\centering\small
\caption{{Frozen model identities. Both checkpoints validate as VDA4, xLSTM, \texttt{{crossattn1}}, $d_{{mem}}=128$, convolutional front end, and $2\times2$ geometry.}}
...
def main() -> int:
    ...
    for name in FIGURES:
        copy_exact(run_root / "figures" / name, output / "figures" / name)
