...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON_BIN" -m pytest -q \
 ...
 tests/test_luo2015_dualstream_dmem128_curriculum_runpod.py tests/test_luo2015_protocol.py \
 tests/test_luo2015_scientific_assay.py tests/test_luo2015_attention_allocation.py \
 tests/test_training_checkpoint.py
CANARY="/workspace/canary_luo_dualstream_dmem128_${CELL}"; rm -rf "$CANARY"
env CELL="$CELL" RUN_ROOT="$CANARY" PYTHON_BIN="$PYTHON_BIN" ITERS=1 START_ITERATION=20 SAVE_EVERY=1 DEVICE=cuda bash "$EXPERIMENT/launch_cell.sh" > "/workspace/canary_dualstream_dmem128_${CELL}.log" 2>&1
...
assert int(c['model_kwargs']['d_mem'])==128
...
printf 'BOOTSTRAP_OK cell=%s d_mem=128 curriculum=true dual_streams=true bc_alpha=0.0 gamma=1.0 memory_noise=0.075 source_sha256=%s canary_sha256=%s pid=%s root=%s\n' "$CELL" "$ACTUAL_SHA" "$CANARY_SHA" "$pid" "$RUN_ROOT"
