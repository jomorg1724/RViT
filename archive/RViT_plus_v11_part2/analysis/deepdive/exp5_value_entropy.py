...
Usage:
  .venv/bin/python -m RViT_plus_v11_part2.analysis.deepdive.exp5_value_entropy \
...
from RViT_plus_v11_part2.analysis import _behav_utils as bu  # noqa: E402,F401
...
def main(argv=None):
    ...
    # ── Part 3: attention → value/entropy (all-heads TOP-DOWN-key bias sweep) ──
    # v11_part2 SPLIT readout: the critic reads ONLY the top-down stream (Q=X over
    # H2). Biasing the top-down keys is the direct lever on the value function —
    # the natural "attention distorts value" manipulation for this model.
    sweep = []
    for b in args.biases:
        bias = None
        if b != 0:
            bias = dd.make_attn_bias(model, device, stream="td", head=None,
