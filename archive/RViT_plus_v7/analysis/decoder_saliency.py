"""
DECODER gradient-saliency for RViT+ v7.

v5's decoder finding ("the actor CLS attention is a bottom-up change detector")
was read off attention weights. v7's decoder is a 1D-CONV head with NO
...
Usage:
    .venv/bin/python RViT_plus_v7/analysis/decoder_saliency.py \
...
from RViT_plus_v7.analysis import _behav_utils as bu
from RViT_plus_v7.analysis.avg_attention_maps import (
...
def saliency_rollout(model, envs, obs0, device):
    """Forced-wait rollout; per frame compute |∂press/∂H2| per token position for
    the actor (press logit) and critic (mean press Q). Returns (T, 2, gh, gw)
    [row0=actor, row1=critic] mean-over-trials, and an obs example."""
    B = len(envs)
    model.eval()
    ...
    while t <= T and not done.all():
        ...
        H2 = rec[-1]                                            # (B, N, d_mem), in graph
        logits = model.actor_head(rec)                          # (B, 2)
        q = model.critic_head(rec)                              # (B, 2, n_quantiles)
        press = logits[:, 1].sum()
        q_press = q[:, 1, :].mean(dim=1).sum()                  # Σ_B mean-over-quantiles Q(s,press)
        g_a = torch.autograd.grad(press, H2, retain_graph=True)[0]      # (B, N, d_mem)
        g_c = torch.autograd.grad(q_press, H2, retain_graph=True)[0]
        ...
        t += 1
    return np.stack(frames, axis=0), np.stack(obs_example, axis=0)   # (T,2,gh,gw), (T,50,50,3)


def main(argv=None) -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--checkpoint", default="RViT_plus_v7/checkpoints/rvit_plus_rl_latest.pt")
    ap.add_argument("--config", default=None)
    ap.add_argument("--out-dir", default="RViT_plus_v7/analysis/figures_decoder")
    ...
    for cond in args.conditions:
        ...
        strip, obs_ex = saliency_rollout(model, envs, obs0, device)
        safe = label.replace(" ", "").replace("·", "_").replace("(", "").replace(")", "")
        # peak quadrant of the actor saliency in the post-change window
        from RViT_plus_v7.analysis.avg_attention_maps import quad_aggregate
        ...
        plot_head_strip(strip, obs_ex,
                        ...
                        title=f"decoder ∂press/∂H2 saliency (row0=actor, row1=critic) · {label} · n={args.n_trials}",
