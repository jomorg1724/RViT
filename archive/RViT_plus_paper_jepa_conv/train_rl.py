...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--proportions", type=float, nargs="+", default=[0.25, 0.5, 0.75, 1.0],
                   help="cue-validity set the env samples P(change at cued) from. Default = full set. "
                        "Warmup use: `--proportions 1.0` = every change at the CUED patch (no foil-change traps), "
                        "to bootstrap the declare policy before annealing foils back in (Krauzlis exploration deadlock).")
    ...
    p.add_argument("--conv-recurrent", action="store_true",
                   help="add a conv-format ConvMGU short-timescale recurrence in the conv front-end "
                        "('recurrence in the visual cortex', V1-like). Requires --conv-frontend.")
    p.add_argument("--conv-rec-stage", type=int, default=1, choices=[1, 2],
                   help="which conv stage the ConvMGU wraps: 1=after stage1 (64ch/13×13, early/cheap, "
                        "recommended) | 2=after stage2 (128ch/7×7).")
    p.add_argument("--conv-rec-retain", type=float, default=0.3,
                   help="INIT retention of the past conv state (~one-past-step memory); the gate bias is "
                        "set to logit(retain). Biased short; the cell can LEARN a longer time constant.")
    ...
    p.add_argument("--mem-noise", type=float, default=0.0,
                   help="relative Gaussian noise on the xLSTM memory readout H (WM-capacity/bottleneck "
                        "lever; forces the model to prioritise what it maintains). 0 = off.")
    ...
    p.add_argument("--init-action-bias", type=float, nargs="+", default=[0.0, -1.5],
                   help="per-action initial output bias; length must equal --n-actions "
                        "(e.g. 0.0 -1.0 -1.0 for the 3-action Baruni discrimination task).")
    # Bottom-up image-VAE auxiliary (second backward pass on Z, memory detached; crossattn1/vda4).
    p.add_argument("--vae-aux", action="store_true",
                   help="add a bottom-up image-VAE reconstruction pass on the transformer Z "
                        "(memory gradients DETACHED → shapes perception, not the LSTM).")
    p.add_argument("--recon-coef", type=float, default=0.1,
                   help="weight on the VAE recon loss (keep < 1 so it stays weaker than RL). Only used with --vae-aux.")
    p.add_argument("--recon-beta", type=float, default=1e-3, help="KL weight inside the VAE loss.")
    p.add_argument("--vae-latent-dim", type=int, default=64)
    p.add_argument("--vae-reduce-dim", type=int, default=32, help="per-patch reducer output dim (paper spec: 32).")
...
def main() -> None:
    ...
    if args.proportions != [0.25, 0.5, 0.75, 1.0]:
        print(f"[paper] PROPORTIONS override = {tuple(args.proportions)}  (warmup/anneal for the declare policy)")
    if args.curriculum:
    ...
    model_kwargs = dict(
        ...
        conv_frontend=args.conv_frontend, mem_noise=args.mem_noise,
        conv_recurrent=args.conv_recurrent, conv_rec_stage=args.conv_rec_stage,
        conv_rec_retain=args.conv_rec_retain,
        vae_aux=args.vae_aux, vae_latent_dim=args.vae_latent_dim,
        vae_reduce_dim=args.vae_reduce_dim, vae_img_size=int(env.S),
    )
    if args.conv_recurrent and not args.conv_frontend:
        raise SystemExit("--conv-recurrent requires --conv-frontend (the ConvMGU lives in the conv front-end)")
    model = RViTPaperModel(**model_kwargs).to(device)
    if args.mem_noise > 0:
        print(f"[paper] MEM-NOISE ON: {args.mem_noise} relative Gaussian noise on the PERSISTENT xLSTM "
              f"cell state C (drift->attractors; WM-capacity bottleneck; train+eval)")
    if args.conv_frontend:
        nf = sum(p.numel() for p in model.front.parameters())
        print(f"[paper] CONV front-end: SE-ResNet (3ch colour, trained end-to-end), per-patch 25×25→128; "
              f"front params={nf:,}")
        if args.conv_recurrent:
            locus = {1: "after stage1 (64ch, 13×13)", 2: "after stage2 (128ch, 7×7)"}[args.conv_rec_stage]
            ng = sum(p.numel() for p in model.front.conv_mgu.parameters()) \
                + sum(p.numel() for p in model.front.conv_reduce.parameters())
            print(f"[paper] CONV-RECURRENT: ConvMGU {locus}, init retention={args.conv_rec_retain} "
                  f"(~1-past-step, τ≈{-1/__import__('math').log(max(args.conv_rec_retain,1e-3)):.2f} frames); "
                  f"added params={ng:,}")
    elif args.vae_color:
    ...
    if init_mode in ("warm_start", "resume"):
        ...
        # resume the curriculum where it left off: restore θ from the checkpoint (overrides --theta-start)
        if args.curriculum:
            _ck = torch.load(ckpt_path, map_location="cpu", weights_only=False)
            _th = _ck.get("theta")
            if _th is not None and _th == _th:      # not NaN
                env.theta = float(_th)
                print(f"[paper]   -> resumed curriculum θ={env.theta:.1f}° from checkpoint")
    else:
    ...
    ppo_cfg = PPOConfig(
        ...
        recon_coef=(args.recon_coef if args.vae_aux else 0.0), recon_beta=args.recon_beta,
    )
    print(f"[paper] ppo: qr_kappa={ppo_cfg.qr_kappa} (critic-particle Huber threshold, scale WITH reward) "
          f"mpo_temperature={ppo_cfg.mpo_temperature} value_coef={ppo_cfg.value_coef} "
          f"entropy_coef={ppo_cfg.entropy_coef} gamma={ppo_cfg.gamma}")
    if args.vae_aux:
        print(f"[paper] IMAGE-VAE AUX: 2nd backward on Z (memory DETACHED → bottom-up only); "
              f"reduce→{args.vae_reduce_dim}/patch, latent={args.vae_latent_dim}, "
              f"recon_coef={args.recon_coef} (< RL), KL β={args.recon_beta}")
    ...
    history = train(
        ...
        checkpoint_dir=ckpt_dir, save_every=args.save_every, model_kwargs=model_kwargs,
    )
