...
def test_first_wave_specs_fix_historical_geometry_and_opposite_corner():
    ...
    vda16 = first_wave_spec("vda16")
    assert vda16.grid == (4, 4)
    assert vda16.image_size == 100
    assert vda16.token_count == 16
    assert vda16.active_indices == tuple(range(16))
    assert vda16.cue_index == 0
    assert vda16.invalid_change_index == 15
...
def test_vda16_environment_figure_uses_current_4x4_geometry(tmp_path):
    from vda_series.first_wave_figures import build_environment_figure

    outputs = build_environment_figure("vda16", tmp_path, seed=1701)
    for path in (outputs.pdf, outputs.svg, outputs.png, outputs.metadata):
        assert path.is_file()
    metadata = json.loads(outputs.metadata.read_text())
    assert metadata["task"] == "vda16"
    assert metadata["grid"] == [4, 4]
    assert metadata["active_items"] == 16
    assert metadata["cue_index"] == 0
    assert metadata["valid_change_index"] == 0
    assert metadata["invalid_change_index"] == 15
...
def test_psychometric_conditions_use_s1_and_true_opposite_corner():
    ...
    assert psychometric_conditions("vda16")["invalid_change_index"] == 15
