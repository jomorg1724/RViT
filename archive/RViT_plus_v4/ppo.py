"""
RL trainer for RViT+ v4 — PAC actor loss + distributional QR-DQN critic +
prioritized episode replay. RL ONLY: no reconstruction auxiliary, no
predictive-coding / JEPA terms, no recon pretrain phase (v4 has no decoder and
...
try:
    ...
    from model import RViTPlusV4Model  # type: ignore[no-redef]
...
class PPOConfig:
    ...
    # acts. (This is v3's `recon_pretrain_iters`, re-pointed: v4 has no decoder,
    ...
    # Supervised-contrastive auxiliary. v4's model has NO contrastive heads
    # (that feature is scoped to v4), so this is kept OFF here. The guarded block
    # in ppo_update never runs for v4 (getattr(model,"contrastive_heads") is None).
    contrastive_coef: float = 0.0          # OFF for v4
...
def ppo_update(
    ...
    batch: RolloutBatch,
    cfg: PPOConfig,
    target_model: Optional[RViTPlusV4Model] = None,
    ...
    *,
    ...
    for it in range(n_iterations):
        ...
        if checkpoint_dir is not None and (it + 1) % save_every == 0:
            ...
            ckpt_path = os.path.join(checkpoint_dir, "rvit_plus_v4_rl_latest.pt")
