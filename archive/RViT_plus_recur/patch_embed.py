...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conv_embed = bool(conv_embed)
        ...
        if self.conv_embed:
            # Medium per-patch CONV stem: each (C, p, p) patch → d_model via a 3-layer
            # strided ConvNet + a linear projection. Applied identically to every patch
            # (shared weights). Replaces the conv-free MLP. For p=25: 25→13→7→4, 128ch
            # → flat 2048 → Linear(d_model). The Linear is sized dynamically from p.
            self.conv_stem = nn.Sequential(
                nn.Conv2d(in_channels, 32, kernel_size=3, stride=2, padding=1), nn.GELU(),
                nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1), nn.GELU(),
                nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1), nn.GELU(),
            )
            with torch.no_grad():
                _flat = self.conv_stem(torch.zeros(1, in_channels, patch_size, patch_size)).flatten(1).shape[1]
            self.conv_fc = nn.Linear(_flat, d_model)
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.conv_embed:
            patches = x.reshape(B * gh * gw, C, p, p)         # each patch keeps its 2D layout
            emb = self.conv_fc(self.conv_stem(patches).flatten(1))   # (B*n_tokens, d_model)
            emb = emb.reshape(B, gh * gw, self.d_model)
        else:
            patches = x.reshape(B, gh * gw, C * p * p)        # (B, n_tokens, patch_dim)
            emb = self.proj(patches)
        tokens = self.norm(emb) + self.pos_emb               # (B, n_tokens, d_model)
        return tokens
