...
decision reads".
    ...
    .venv/bin/python v6_VizdoomArena/analysis/render_attention_grid.py
    .venv/bin/python v6_VizdoomArena/analysis/render_attention_grid.py \
...
def _load_checkpoint(path: str, device: torch.device):
    """One retry — the trainer rewrites v6_latest.pt every few minutes."""
    for attempt in (0, 1):
        try:
...
def main(argv=None) -> int:
    ...
    L = model.enc_layers
    enc_spans = _patch_aligned_spans(model.encoder.key_layout(), gh * gw)
            ...
            if render_this:
                ...
                for li in range(L):
                    canvas.paste(first_col[li] if li < len(first_col) else frame_img,
                                 (0, y0 + li * PANEL_H * s))
                    grids = _per_head_grids(out["enc_attn"][li].cpu(), enc_spans, gh, gw)
