"""
Render the most recent V10 checkpoint playing ONE arena episode.
...
HUD line and red boxes around labeled monsters) next to the checkpoint:
    ...
    .venv/bin/python v10_VizdoomArena/analysis/render_episode.py
    .venv/bin/python v10_VizdoomArena/analysis/render_episode.py --greedy --scale 3
    .venv/bin/python v10_VizdoomArena/analysis/render_episode.py --attn      # + encoder attention panels
    .venv/bin/python v10_VizdoomArena/analysis/render_episode.py --window    # watch live instead
...
def _load_checkpoint(path: str, device: torch.device):
    """Load with one retry — the trainer rewrites v10_latest.pt every few minutes."""
    for attempt in (0, 1):
        try:
...
def main(argv=None) -> int:
    ...
    layouts = [model.encoder.key_layout(k) for k in range(model.enc_layers)]
        ...
        for _t in range(args.max_steps):
            ...
            if args.attn:
                ...
                for k, aw in enumerate(out["enc_attn"]):
                    grid = _attn_grid(aw.cpu(), layouts[k], gh, gw)
