"""
Encoder for RViT+ v11_part3: PARALLEL dual-stream cross-attention (Tμ SALIENCE +
...
with the current patch tokens X:
  ...
  Tμ — SALIENCE (bottom-up):   Q = X ,  K = V = [H1 ‖ H2] ,  residual = X
      ...
      (v11_part3's change vs v11: H2 added to the salience K/V, each source tagged).
      → feeds the ACTOR.
...
class DualStreamEncoder(nn.Module):
    ...
    LSTMs. v11_part3: Tμ reads K=V=[H1‖H2]; TQ reads K=V=H2; single head each; the
    ...
    def __init__(
        ...
        if int(n_lstm) != 2:
            raise ValueError(
                "v11_part3 dual-stream needs exactly n_lstm=2 (H1 = sensory memory, H2 = deep memory)."
            )
