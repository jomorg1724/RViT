...
set -euo pipefail
CELL="${CELL:?set CELL}"
GAMMA="${GAMMA:?set GAMMA to 0.8, 0.9, or 1.0}"
case "$GAMMA" in 0.8|0.9|1.0) ;; *) echo "invalid GAMMA: $GAMMA" >&2; exit 2;; esac
case "$CELL" in
...
cmd=("$PYTHON_BIN" -u train_rl.py --T 7
 ...
 --memory-decay 1.0 --memory-noise-std 0.32 --noise 5.0 --conv-frontend
 --jepa-coef 0.5 --d-mem 32 --episodes-per-iter 8 --gamma "$GAMMA"
 ...
 --experiment-launcher experiments/luo2015_episodic/fresh_grid2_memnoise032_gamma_ladder_runpod/launch_cell.sh
...
mkdir -p "$RUN_ROOT"
cp experiments/luo2015_episodic/fresh_grid2_memnoise032_gamma_ladder_runpod/experiment_manifest.json "$RUN_ROOT/experiment_manifest.json"
printf 'cell=%s\ngamma=%s\ninitialization=fresh\ntask=%s\ncondition_loc=%s\nreward_scale=%s\ntheta=65.0\ncurriculum=false\nrun_root=%s\n' "$CELL" "$GAMMA" "$TASK" "$HIGH_LOC" "$REWARD_SCALE" "$RUN_ROOT" > "$RUN_ROOT/launch_contract.txt"
exec "${cmd[@]}"
