...
def test_vda16_decoder_uses_a_high_dimensional_convergence_ceiling():
    classifier = C._classifier(matched_dimensions=None, seed=17)
    assert classifier.named_steps["logit"].max_iter == 4000


def test_zero_variance_recurrent_state_is_declared_at_exact_chance_without_pca_warning():
    n = 240
    labels = np.resize(np.arange(3), n)
    cells = np.zeros((n, 1, 2, 128), dtype=float)
    fold_ids = C.stratified_fold_ids(labels, np.ones(n, dtype=bool), seed=17)

    with warnings.catch_warnings():
        warnings.simplefilter("error")
        native = C.decode_timecourses(
            cells, labels, fold_ids, matched_dimensions=None
        )
        matched = C.decode_timecourses(
            cells, labels, fold_ids, matched_dimensions=128
        )

    np.testing.assert_allclose(native, [1.0 / 3.0], atol=0.0, rtol=0.0)
    np.testing.assert_allclose(matched, [1.0 / 3.0], atol=0.0, rtol=0.0)
