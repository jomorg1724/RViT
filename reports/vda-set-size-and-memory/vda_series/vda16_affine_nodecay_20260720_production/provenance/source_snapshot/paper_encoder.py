...
class RecurrentViTxLSTM(nn.Module):
    ...
    def forward_step(self, X: torch.Tensor, state, return_attn: bool = False,
        ...
        if self.feedback == "dualmem":
            ...
            H1, C1, N1, M1 = self.lstm(
                Z1, *s1, inject_memory_noise=inject_memory_noise
            )                                                                # Z1 → H1
            H2, C2, N2, M2 = self.lstm2(
                Z2, *s2, inject_memory_noise=inject_memory_noise
            )                                                                # Z2 → H2
        ...
        if self.feedback == "affine_cascade":
            ...
            H1, C1, N1, M1 = self.lstm(
                Z1, H1p, C1, N1, M1, inject_memory_noise=inject_memory_noise
            )                                                                # H1 ← LSTM1(Z1)
            Z2, aw2 = self.attn2(Z1, H2p, return_attn=return_attn, attn_clamp=c2) # T2: input Z1, feedback H2
            H2, C2, N2, M2 = self.lstm2(
                Z2, H2p, C2, N2, M2, inject_memory_noise=inject_memory_noise
            )                                                                # H2 ← LSTM2(Z2)
        ...
        if self.two_lstm:
            ...
            H2, C2, N2, M2 = self.lstm2(
                lstm2_in, *s2, inject_memory_noise=inject_memory_noise
            )                                                                # LSTM2 → H2
            return ((H1, C1, N1, M1), (H2, C2, N2, M2)), H2, attn            # heads read H2
