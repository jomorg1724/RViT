...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON_BIN" -m pytest -q \
 ...
 tests/test_luo2015_fresh_memnoise0106667_gamma080_runpod.py
CANARY="/workspace/canary_luo_memnoise0106667_gamma080_${CELL}"; rm -rf "$CANARY"
env CELL="$CELL" RUN_ROOT="$CANARY" PYTHON_BIN="$PYTHON_BIN" ITERS=1 SAVE_EVERY=1 DEVICE=cuda bash experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma080_reward_matrix_runpod/launch_cell.sh > "/workspace/canary_memnoise0106667_${CELL}.log" 2>&1
...
printf '{"source_archive_sha256":"%s","cell":"%s","grid":[2,2],"memory_noise_std":0.10666666666666666,"sensory_noise_std":5.0,"gamma":0.8,"initialization":"fresh","canary_checkpoint_sha256":"%s"}\n' "$ACTUAL_SHA" "$CELL" "$CANARY_SHA" > "$RUN_ROOT/deployment_contract.json"
(
 set +e
 env CELL="$CELL" RUN_ROOT="$RUN_ROOT" PYTHON_BIN="$PYTHON_BIN" ITERS=20000 SAVE_EVERY=50 DEVICE=cuda bash experiments/luo2015_episodic/fresh_grid2_memnoise0106667_gamma080_reward_matrix_runpod/launch_cell.sh
...
printf 'BOOTSTRAP_OK cell=%s gamma=0.8 memory_noise=0.10666666666666666 source_sha256=%s canary_sha256=%s pid=%s root=%s\n' "$CELL" "$ACTUAL_SHA" "$CANARY_SHA" "$pid" "$RUN_ROOT"
