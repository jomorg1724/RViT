...
from RViT_plus_v3.config.loader import (
...
from RViT_plus_v3.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    # ── Discrete-JEPA variant of the PC auxiliary ──
    p.add_argument("--pc-kind", choices=["cosine", "discrete_jepa", "discrete_latent"],
                   default=cfg_get(cfg, "ppo.pc_kind", "cosine"),
                   help="PC objective: 'cosine' (legacy 1−cos regression), "
                        "'discrete_jepa' (categorical code as a prediction target only), "
                        "or 'discrete_latent' (hard one-hot code REPLACES C₂ as the "
                        "agent's latent, DreamerV3-style; predictors = dynamics model).")
    p.add_argument("--pc-groups", type=int,
                   default=cfg_get(cfg, "ppo.pc_groups", 8),
                   help="discrete_jepa: K categorical groups per spatial position.")
    p.add_argument("--pc-classes", type=int,
                   default=cfg_get(cfg, "ppo.pc_classes", 16),
                   help="discrete_jepa: V classes per categorical group.")
    p.add_argument("--pc-kl-dyn", type=float,
                   default=cfg_get(cfg, "ppo.pc_kl_dyn", 0.5),
                   help="discrete_jepa: weight on KL(sg(post)‖prior) (DreamerV3 β_dyn).")
    p.add_argument("--pc-kl-rep", type=float,
                   default=cfg_get(cfg, "ppo.pc_kl_rep", 0.1),
                   help="discrete_jepa: weight on KL(post‖sg(prior)) (DreamerV3 β_rep).")
    p.add_argument("--pc-anchor-coef", type=float,
                   default=cfg_get(cfg, "ppo.pc_anchor_coef", 0.1),
                   help="discrete_jepa: weight on the −MI InfoMax anchor (collapse guard).")
    p.add_argument("--pc-free-bits", type=float,
                   default=cfg_get(cfg, "ppo.pc_free_bits", 0.0),
                   help="discrete_jepa: per-group free bits in nats (KL below this is not penalized).")
    p.add_argument("--pc-detach-c2", action=argparse.BooleanOptionalAction,
                   default=bool(cfg_get(cfg, "ppo.pc_detach_c2", True)),
                   help="discrete_jepa: detach C₂ into the tokenizer (isolates actor/critic branches).")
    ...
    print_resolved_config(cfg, used_keys=[
        ...
        "ppo.pc_kind", "ppo.pc_groups", "ppo.pc_classes",
        "ppo.pc_kl_dyn", "ppo.pc_kl_rep", "ppo.pc_anchor_coef",
        "ppo.pc_free_bits", "ppo.pc_detach_c2",
    ...
    model = RViTPlusModel(
        ...
        pc_kind=args.pc_kind,
        pc_groups=args.pc_groups,
        pc_classes=args.pc_classes,
    ).to(device)
    print(f"[diag] actor init bias: {list(args.init_action_bias)}")
    print(f"[diag] split_c3: {args.split_c3}")
    print(f"[diag] PC coefs: actor={args.pc_actor_coef}, critic={args.pc_critic_coef}")
    if args.pc_kind in ("discrete_jepa", "discrete_latent"):
        extra = ("  [latent: code REPLACES C₂ at the heads, STE-anchored]"
                 if args.pc_kind == "discrete_latent"
                 else f"  detach_c2={args.pc_detach_c2}")
        print(f"[diag] PC kind: {args.pc_kind}  codes={args.pc_groups}×{args.pc_classes} "
              f"(K×V)  kl_dyn={args.pc_kl_dyn} kl_rep={args.pc_kl_rep} "
              f"anchor={args.pc_anchor_coef} free_bits={args.pc_free_bits}{extra}")
    else:
        print(f"[diag] PC kind: cosine (legacy)")
    ...
    cfg = PPOConfig(
        ...
        pc_groups=args.pc_groups,
        pc_classes=args.pc_classes,
        pc_kl_dyn=args.pc_kl_dyn,
        pc_kl_rep=args.pc_kl_rep,
        pc_anchor_coef=args.pc_anchor_coef,
        pc_free_bits=args.pc_free_bits,
        pc_detach_c2=args.pc_detach_c2,
    ...
    torch.save({
        ...
        "model_kwargs": {
            "in_channels": 3, "image_h": 50, "image_w": 50,
            "stem_out_channels": 64, "state_channels": (64, 96, 128),
            "n_FR": args.n_FR, "n_heads": 32, "seq_len": args.seq_len,
            "upsample_out_channels": 32, "cnn_hidden": 64,
            "enable_skips": True, "skip_scale": 0.3,
            "enable_actor": True, "enable_critic": True,
            "n_actions": args.n_actions, "n_quantiles": args.n_quantiles,
