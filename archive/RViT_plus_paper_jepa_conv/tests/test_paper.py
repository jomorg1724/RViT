...
def test_vae_aux_detaches_memory_gradient():
    """Image-VAE aux (crossattn1): the recon backward shapes the BOTTOM-UP path (conv front-end +
    attention projections + VAE head) but pushes NO gradient through the memory/LSTM."""
    m = RViTPaperModel(feedback="crossattn1", cell="xlstm", d_mem=128, conv_frontend=True,
                       seq_len=7, vae_aux=True, vae_latent_dim=32, vae_reduce_dim=32, vae_img_size=50)
    def gsum(mod): return sum(p.grad.abs().sum().item() for p in mod.parameters() if p.grad is not None)
    vid = torch.randn(2, 7, 50, 50, 3).clamp(-1, 1)
    m.zero_grad(set_to_none=True)
    loss, mse, kl = m.recon_loss(vid, beta=1e-3)
    loss.backward()
    assert gsum(m.encoder.lstm) == 0.0                       # NO gradient through the LSTM (the contract)
    assert gsum(m.front) > 0 and gsum(m.encoder.attn) > 0 and gsum(m.vae_head) > 0
    recon, mu, logvar = m.vae_head(torch.randn(2, 7, 4, 140))
    assert recon.shape == (2, 7, 3, 50, 50)                  # reconstructs the full frame
    # and the RL path still trains the LSTM
    m.zero_grad(set_to_none=True)
    m.forward_rl_sequence(vid)["q_dist_seq"].sum().backward()
    assert gsum(m.encoder.lstm) > 0.0
