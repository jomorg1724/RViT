...
Usage:
    .venv/bin/python RViT_plus_v11/train_rl.py
    .venv/bin/python RViT_plus_v11/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v11.config.loader import (
    DEFAULT_CONFIG_PATH, cfg_get, load_checkpoint_weights, load_config,
    print_resolved_config,
)
from RViT_plus_v11.env import ChangeDetectionEnv
from RViT_plus_v11.model import RViTPlusV11Model
from RViT_plus_v11.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    model_kwargs = dict(
        ...
        conv_embed=args.conv_embed,
    )
    model = RViTPlusV11Model(**model_kwargs).to(device)

    _embed = (f"CONV stem (3 conv → flat {model.patch_embed.conv_fc.in_features}) → d_model"
              if args.conv_embed else f"reshape+MLP {args.patch_hidden} → d_model")
    print(f"[diag] patch_size={args.patch_size} → n_tokens={model.n_tokens} "
          f"(raw patch {model.patch_embed.patch_dim} → {_embed}; "
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v9_rl_final.pt")
