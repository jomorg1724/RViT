...
def load_config(path: Optional[str] = None) -> dict:
    if path is None:
        path = os.path.join(_PROJECT_ROOT, "RViT_plus_v9", "config", "rvit_plus_config.json")
    with open(path) as f:
        return json.load(f)


def build_model(cfg: dict, device: torch.device) -> RViTPlusV9Model:
    """Construct the v5_part2 model exactly as train_rl.py does, from the config."""
    m = cfg["model"]
    rl = m["rl"]
    model = RViTPlusV9Model(
...
def load_checkpoint(model: RViTPlusV9Model, ckpt_path: str, device: torch.device) -> int:
