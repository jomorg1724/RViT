...
Usage:
    .venv/bin/python RViT_plus_v15/train_rl.py
    .venv/bin/python RViT_plus_v15/train_rl.py --iters 500 --patch-size 5
"""
...
from RViT_plus_v15.config.loader import (
...
from RViT_plus_v15.ppo import PPOConfig, train
...
def main(argv=None) -> int:
    ...
    p.add_argument("--energy-coef", type=float, default=cfg_get(cfg, "ppo.energy_coef", 0.01),
                   help="VQ-VAE commitment / energy cost: weight on mean‖energy‖²=‖Z−C‖². "
                        "Higher → energized Z stays near codebook C → reduces to v12. 0 disables.")
    p.add_argument("--codebook-ema-rate", type=float, default=cfg_get(cfg, "ppo.codebook_ema_rate", 1.0e-3),
                   help="EMA the codebook toward the energized Z: C += r·mean(energy). SMALL. "
                        "0 = pure v12 + energy (no consolidation).")
    ...
    model = RViTPlusV15Model(**model_kwargs).to(device)
    ...
    ppo_cfg = PPOConfig(
        ...
        energy_coef=args.energy_coef, codebook_ema_rate=args.codebook_ema_rate,
    ...
    final_path = os.path.join(args.checkpoint_dir, "rvit_plus_v15_rl_final.pt")
